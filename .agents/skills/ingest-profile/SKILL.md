...
2. **(b) Compensation floor choice** — Ask one plain question first: **"Should I screen jobs by guaranteed pay, or by what you need to make in a full year including tips, commission, or cash bonuses?"** Offer **Guaranteed pay** and **Total annual earnings** as clickable choices when the surface supports them; typed answers mean the same thing.
   ...
   - **Total annual earnings** → ask: **"What's the least you need to make in a full year, including wages, tips, commission, or cash bonuses? Don't count equity or benefits."** Save `minimum_annual_earnings` with `careerrat gate comp-annual-floor <N> --write --confirm`.
     - If the candidate answers in wage-shaped terms, do not ask them to annualize it. Ask how their base pay is set up first: an hourly wage, a flat weekly amount, or a flat monthly amount. Gather only the missing pieces for that one shape, one at a time (hourly wage plus paid hours per week; or a flat weekly amount; or a flat monthly amount), plus optional expected cash per shift from tips/commission/cash bonuses and shifts per week, and weeks per year for hourly pay. Treat a stated amount "per night" as cash per shift. Never infer overtime, shift differential, tips, commission, or bonuses they did not state. A candidate gives exactly one base pay shape; if they give more than one, ask which one to use.
     - Calculate the annual base first: `hourly wage × paid hours per week × weeks per year` for hourly pay, `flat weekly amount × 52` for weekly pay, or `flat monthly amount × 12` for monthly pay. Add `cash per shift × shifts per week × weeks per year` on top when the candidate gave shift cash. Show the arithmetic and the derived annual cash amount in the response. If weeks per year was not stated for hourly pay, show the 52-week assumption explicitly so the candidate can edit it. Confirm the annual amount before writing `minimum_annual_earnings`; the wage inputs are a worksheet, not new profile fields.
