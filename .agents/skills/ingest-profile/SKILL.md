...
2. **(b) Compensation floor choice** — Ask one plain question first: **"Should I screen jobs by guaranteed pay, or by what you need to make in a full year including tips, commission, or cash bonuses?"** Offer **Guaranteed pay** and **Total annual earnings** as clickable choices when the surface supports them; typed answers mean the same thing.
   ...
   - **Total annual earnings** → ask: **"What's the least you need to make in a full year, including wages, tips, commission, or cash bonuses? Don't count equity or benefits."** Save `minimum_annual_earnings` with `careerrat gate comp-annual-floor <N> --write --confirm`.
     - If the candidate answers in wage-shaped terms, do not ask them to annualize it. Gather only the missing pieces, one at a time: hourly wage, paid hours per week, expected cash per shift from tips/commission/cash bonuses, shifts per week, and weeks per year. Treat a stated amount "per night" as cash per shift. Never infer overtime, shift differential, tips, commission, or bonuses they did not state.
     - Calculate `(hourly wage × paid hours per week + cash per shift × shifts per week) × weeks per year`. Show the arithmetic and the derived annual cash amount in the response. If weeks per year was not stated, show the 52-week assumption explicitly so the candidate can edit it. Confirm the annual amount before writing `minimum_annual_earnings`; the wage inputs are a worksheet, not new profile fields.
