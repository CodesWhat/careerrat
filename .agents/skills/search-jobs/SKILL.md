...
For each entry in `prompts`, run `WebSearch` using that prompt's text (or a close paraphrase) as the query. Use at most 2 `WebSearch` calls per saved prompt and at most 4 job-posting `WebFetch` calls per saved prompt. Stop earlier when the searches repeat the same roles or four readable postings have been checked. For every promising result — a real job-posting URL, not an aggregator/search-results page — run `WebFetch` on the URL to pull the actual JD text. Prefer postings you can read directly; drop a result rather than guessing at a JD you couldn't fetch.
