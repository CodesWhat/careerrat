...
For each entry in `prompts`, run `WebSearch` using that prompt's text (or a close paraphrase) as the query. Use at most 2 `WebSearch` calls per saved prompt and at most 4 job-posting `WebFetch` calls per saved prompt. Search broadly before spending the fetch budget. Keep credible, role-specific leads from direct postings, employer career pages, specialist boards, and useful aggregators. A result does not disappear just because its full JD needs a browser session, blocks automated fetching, or lives on a results page. For up to four promising direct posting URLs, use `WebFetch` to improve the capture. If the full body is still unavailable, return the title, company, URL, visible location/comp/date, and factual search-result evidence with `body_text: null` and `body_partial: true`. The server will preserve it as an explicitly unverified lead; the later Evaluate step owns liveness and full-JD verification. Drop only results that lack a specific role and employer, violate the candidate's hard filters, duplicate another result, or are known to be expired.
