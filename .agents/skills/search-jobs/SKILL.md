...
- `rejected_postings[]` — up to four successfully fetched exact posting URLs that are not in `roles[]`, each as `{url, reason}`. Use the exact fetched URL and one short factual reason. Return an empty array when every successful exact fetch is kept or no exact posting was fetched.
