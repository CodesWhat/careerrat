...
Open `candidate/targeting.yml`. Append accepted signals under `keep_signals` or `cut_signals` (preserve all existing entries). Save. Run `careerrat tracker --verify`.
...
Run `careerrat tracker --verify` immediately after.
