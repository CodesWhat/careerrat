#!/usr/bin/env bash
# Retire the legacy npm package that careerrat@0.5.2 replaced.
#
...
LEGACY_PACKAGE="role""ster"
LEGACY_MSG="Renamed to careerrat. Install careerrat instead: npm i -g careerrat"
...
if ! WHO="$(npm whoami 2>/dev/null)"; then
  ...
  if [ ! -t 0 ]; then
    ...
    echo
    echo "        npm login && bash scripts/retire-legacy-npm-package.sh"
    echo
    exit 1
  fi
...
echo "==> Deprecating the legacy package (all versions)"
npm deprecate "$LEGACY_PACKAGE" "$LEGACY_MSG" || fail=1
...
echo "    legacy@0.5.2:    $(npm view "$LEGACY_PACKAGE@0.5.2" deprecated 2>/dev/null || echo '(not deprecated)')"
