...
// ---------------------------------------------------------------------------
// Parity with the client-side companion (apps/web/src/lib/commLinks.js).
// That module hand-copies the recipient rule and the mailto shape because the
// web app doesn't import server modules; this suite fails the moment the two
// implementations drift apart, so "kept in sync by hand" is enforced by a
// test instead of trusted.
// ---------------------------------------------------------------------------

describe("parity with apps/web/src/lib/commLinks.js", () => {
  it("picks the same recipient for the same participants", async () => {
    const { firstParticipantEmail } = await import("../apps/web/src/lib/commLinks.js");
    const cases = [
      [{ name: "Avery", email: "avery@acme.test" }],
      [{ name: "No Email" }, { name: "Second", email: "second@acme.test" }],
      [
        { name: "Bad", email: "not-an-email" },
        { name: "Good", email: "good@acme.test" },
      ],
      [{ name: "Bad", email: "still@bad" }],
      [],
    ];
    for (const participants of cases) {
      const server = resolveRecipient({ participants });
      const client = firstParticipantEmail(participants);
      assert.equal(
        server.state === "ready" ? server.to : null,
        client,
        `recipient drift for ${JSON.stringify(participants)}`
      );
    }
  });

  it("builds the same mailto link for the same input", async () => {
    const { buildMailtoLink } = await import("../apps/web/src/lib/commLinks.js");
    const input = {
      to: "avery@acme.test",
