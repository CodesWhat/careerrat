...
test("two applications for the same company and role land on distinct output filenames, never sharing a stem", async () => {
  // exportPacketArtifacts' reservation (src/core/packet/exports.mjs) only
  // ever protects one application's own artifacts from its own outBase
  // choices; it does not reserve identities across different applications.
  // That is safe only because generatePacket's sourceBase (src/core/packet/
  // generate.mjs) always folds the application id into the output
  // filename -- `${company}-${role}-${appId}` -- so two applications
  // sharing a company and role still can never collide on the same
  // tailored/ file, even though every export writes into that single
  // shared directory. This is the regression that proves that premise: if
  // sourceBase ever stopped including the application id, this test would
  // start failing well before any cross-application reservation gap could
  // bite in production.
  const repoRoot = tempRepo();
  seedPacketReadyApp(repoRoot, { id: "app-packet-one" });
  seedPacketReadyApp(repoRoot, { id: "app-packet-two" });

  const server = await bootServer(repoRoot, validPacketCalls());
  try {
    const first = await postJson(server, "/api/packet/generate", {
      appId: "app-packet-one",
      applyIntent: true,
      formats: ["pdf"],
    });
    assert.equal(first.status, 200);
    const second = await postJson(server, "/api/packet/generate", {
      appId: "app-packet-two",
      applyIntent: true,
      formats: ["pdf"],
    });
    assert.equal(second.status, 200);

    const appOne = readApp(repoRoot, "app-packet-one");
    const appTwo = readApp(repoRoot, "app-packet-two");

    for (const key of ["resumeSource", "coverLetterSource", "packetManifest", "resumePdf"]) {
      assert.notEqual(
        appOne.artifacts[key],
        appTwo.artifacts[key],
        `${key} must not collide across applications sharing a company and role`
      );
    }

    // Both sets of files genuinely exist, distinctly, side by side in the
    // same tailored/ directory -- neither export silently renamed over
    // the other's output.
    for (const app of [appOne, appTwo]) {
      for (const key of ["resumeSource", "coverLetterSource", "packetManifest", "resumePdf"]) {
        assert.ok(
          existsSync(join(repoRoot, app.artifacts[key])),
          `${key} should point at a local artifact file`
        );
      }
    }
    assert.match(
      readFileSync(join(repoRoot, appOne.artifacts.resumeSource), "utf8"),
      /Alex Rivera|production AI/i
    );
    assert.match(
      readFileSync(join(repoRoot, appTwo.artifacts.resumeSource), "utf8"),
      /Alex Rivera|production AI/i
    );
  } finally {
    await closeServer(server);
