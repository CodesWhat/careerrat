...
test("GET /app/jobs (M10 Jobs route) falls back to the same index.html", async () => {
  const repoRoot = tempRepo();
  writeFakeDist(repoRoot);
  const dev = await bootServer(repoRoot);
  try {
    const res = await fetch(`${baseUrl(dev)}/app/jobs`);
    assert.equal(res.status, 200);
    const body = await res.text();
    assert.match(body, /id="root"/);
  } finally {
...
test("GET /app/calendar (M10 Calendar route) falls back to the same index.html", async () => {
  const repoRoot = tempRepo();
  writeFakeDist(repoRoot);
  const dev = await bootServer(repoRoot);
  try {
    const res = await fetch(`${baseUrl(dev)}/app/calendar`);
    assert.equal(res.status, 200);
    const body = await res.text();
    assert.match(body, /id="root"/);
  } finally {
