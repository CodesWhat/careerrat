...
try {
  const softgardenModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/softgarden.mjs")).href
  );
  ...
  if (
    resolveWidgetUrl({ api: "https://renk-group.softgarden.io/de/widgets/jobs" }) ===
    "https://renk-group.softgarden.io/de/widgets/jobs"
  )
    pass("softgarden.resolveWidgetUrl() keeps explicit widget URLs");
  else fail("softgarden.resolveWidgetUrl() should keep the widget URL");
  if (
    resolveWidgetUrl({ careers_url: "https://acme.softgarden.io/en/vacancies" }) ===
    "https://acme.softgarden.io/en/widgets/jobs"
  )
    pass("softgarden.resolveWidgetUrl() defaults other tenant URLs to the lang widget");
  else
    fail(
      `softgarden.resolveWidgetUrl() default wrong: ${resolveWidgetUrl({ careers_url: "https://acme.softgarden.io/en/vacancies" })}`
    );
  if (
    softgarden.detect({ careers_url: "https://evil.com/x.softgarden.io" }) === null &&
    softgarden.detect({ careers_url: "https://softgarden.io.evil.com/x" }) === null
  ) {
  ...
  if (
    parseSoftgardenDate("04.07.26") === Date.UTC(2026, 6, 4) &&
    parseSoftgardenDate("7/4/26") === Date.UTC(2026, 6, 4) &&
    parseSoftgardenDate("junk") === undefined
  ) {
  ...
  const sgHtml =
    "<html>" +
    sgCard("111", "Fachkraft (m/w/d) f&#252;r Export &amp; Zoll", ["Hannover"]) +
    sgCard("222", "SAP Consultant", ["Augsburg", "M&#252;nchen"]) +
    "</html>";
  ...
  if (sgRows[0]?.title === "Fachkraft (m/w/d) für Export & Zoll")
    pass("softgarden.parseWidget() decodes entities in titles");
  else fail(`softgarden.parseWidget() title wrong: ${JSON.stringify(sgRows[0]?.title)}`);
  if (sgRows[0]?.url === "https://renk-group.softgarden.io/job/111/slug-111?jobDbPVId=9111&l=de")
    pass("softgarden.parseWidget() resolves ../../job/ hrefs against the widget path");
  else fail(`softgarden.parseWidget() url wrong: ${JSON.stringify(sgRows[0]?.url)}`);
  if (sgRows[1]?.location === "Augsburg / München" && sgRows[0]?.postedAt === Date.UTC(2026, 6, 4))
    pass("softgarden.parseWidget() joins cities and parses the date");
  else fail(`softgarden.parseWidget() fields wrong: ${JSON.stringify(sgRows[1])}`);
  if (
    parseWidget("<html>none</html>", "https://x.softgarden.io/de/widgets/jobs").length === 0 &&
    parseWidget(undefined, "https://x").length === 0
  ) {
  ...
  // text while still decoding valid entities.
  {
    ...
    try {
      badRows = parseWidget(
        "<html>" + sgCard("333", badTitle, ["Berlin"]) + "</html>",
        "https://renk-group.softgarden.io/de/widgets/jobs"
      );
    } catch (e) {
      badThrew = e;
    ...
    if (badThrew)
      fail(
        `softgarden.parseWidget() threw ${badThrew.name} on a malformed numeric entity: ${badThrew.message}`
      );
    else if (badRows.length === 1 && badRows[0].title === expected)
      pass(
        "softgarden.parseWidget() degrades out-of-range / lone-surrogate / mixed-radix entities to literal text while still decoding valid ones"
      );
    else
      fail(
        `softgarden.parseWidget() malformed-entity handling wrong: ${JSON.stringify(badRows?.[0]?.title)}`
      );
  }

  // fetch — single widget request, jobs normalized.
  const sgCtx = { fetchText: async () => sgHtml };
  const sgJobs = await softgarden.fetch(
    { name: "Renk", api: "https://renk-group.softgarden.io/de/widgets/jobs" },
    sgCtx
  );
  if (
    sgJobs.length === 2 &&
    sgJobs[0].company === "Renk" &&
    sgJobs.every((j) => j.url.startsWith("https://renk-group.softgarden.io/job/"))
  ) {
