...
try {
  const smartrecruitersModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/smartrecruiters.mjs")).href
  );
  ...
  const hitCareers = sr.detect({
    name: "Adyen",
    careers_url: "https://careers.smartrecruiters.com/adyen",
  });
  if (
    hitCareers &&
    hitCareers.url.startsWith("https://api.smartrecruiters.com/v1/companies/adyen/postings")
  ) {
  ...
  if (
    hitJobs &&
    hitJobs.url.startsWith("https://api.smartrecruiters.com/v1/companies/x/postings")
  ) {
  ...
  if (
    hitApi &&
    hitApi.url.startsWith("https://api.smartrecruiters.com/v1/companies/Continental/postings")
  ) {
  ...
  if (jobs[1]?.location === "Paris, France, Remote") {
    pass(
      "parseSmartRecruitersResponse builds location from city/country/remote when no fullLocation"
    );
  } else {
  ...
  const malformedRef = parseSmartRecruitersResponse(
    {
      content: [
        { id: "zz-9", name: "Odd Role", ref: "https://api.smartrecruiters.com/v1/companies/sgs" },
      ],
    },
    "SGS"
  );
  ...
  const noRefNoId = parseSmartRecruitersResponse({ content: [{ name: "Stranded Role" }] }, "X");
  ...
  if (
    sr.detect({
      name: "Spoof",
      careers_url: "https://evil.example/careers.smartrecruiters.com/slug",
    }) === null
  ) {
  ...
  const bogusRef = parseSmartRecruitersResponse(
    {
      content: [
        { id: "X1", name: "Strange Role", ref: "https://evil.example/v1/companies/x/postings/X1" },
      ],
    },
    "TestCo"
  );
  ...
  if (
    noName[0]?.url === "https://jobs.smartrecruiters.com/sgs/N1" &&
    ...
    { name: "PagedCo", careers_url: "https://careers.smartrecruiters.com/paged" },
    {
      ...
      fetchJson: async (url) => {
        ...
        if (offset === 100) {
          ...
          return {
            content: Array.from({ length: 50 }, (_, i) => ({ id: `P2-${i}`, name: `Role 2-${i}` })),
          };
        }
  ...
  if (pageRequests === 2 && pagedJobs.length === 150) {
    ...
    fail(
      `pagination: pageRequests=${pageRequests}, total=${pagedJobs.length} (expected 2 requests / 150 results)`
    );
  }
  ...
  if (
    ...
    extractDescription({}) === "" &&
    extractDescription({ jobAd: {} }) === "" &&
    extractDescription({ jobAd: { sections: "nope" } }) === ""
  ) {
  ...
  if (joined === "Acme builds & ships widgets You will build robots 5+ years Sponsorship: no") {
    pass(
      "extractDescription() joins the four known sections in order, strips HTML, decodes entities"
    );
  } else {
    ...
    const enriched = await sr.fetch(
      ...
      },
      {
        fetchJson: async (url) => {
          ...
          return {
            jobAd: {
              sections: id === "A1" ? { jobDescription: { text: "<p>Body of A1</p>" } } : {}, // B2 has no usable sections → no description key
            },
          };
        },
      }
      ...
      },
      {
        fetchJson: async (url) => {
          if (url.includes("/postings?"))
            return { content: bigBoardIds.map((id) => ({ id, name: `Role ${id}` })) };
          bigBoardDetailCalls++;
          return {
            jobAd: { sections: { jobDescription: { text: `<p>body ${bigBoardDetailCalls}</p>` } } },
          };
        },
      }
    );
      ...
      },
      {
        fetchJson: async (url) => {
          if (url.includes("/postings?")) {
            return {
              content: [
                { id: "OK-1", name: "Fine Role" },
                { id: "BAD-2", name: "Doomed Role" },
              ],
            };
          }
          resilientCalls++;
