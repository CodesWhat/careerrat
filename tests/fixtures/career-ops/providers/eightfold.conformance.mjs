...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/eightfold.mjs")).href
  );
  ...
  if (
    hit &&
    hostOf(hit.url) === "acme.eightfold.ai" &&
  ...
  const withDomain = ef.detect({
    name: "Acme",
    careers_url: "https://acme.eightfold.ai/careers?domain=acme.example",
  });
  ...
  const badEntries = [
    ...
    {},
    { name: "B" },
    { name: "B", careers_url: null },
    { name: "B", careers_url: 42 },
    { name: "B", careers_url: "not a url" },
  ];
  const detectLeaks = badEntries.filter((e) => ef.detect(e) !== null);
  if (detectLeaks.length === 0) {
    pass(
      `eightfold.detect() returns null for all ${badEntries.length} untrusted / unusable entries (SSRF pin)`
    );
  } else {
  ...
  if (
    hostOf(fallbackUrl) === "acme.eightfold.ai" &&
    new URL(fallbackUrl).searchParams.get("pid") === "99"
  ) {
  ...
  const fixture = {
    ...
    positions: [
      ...
      // 3: DROP — no title at all
      {
        id: 1004,
        location: "Berlin,Germany",
        canonicalPositionUrl: "https://careers.acme.example/careers/job/1004",
      },
      ...
      // 5: DROP — non-https canonical URL and no id to fall back on
      {
        name: "Insecure Role",
        location: "Berlin",
        canonicalPositionUrl: "http://careers.acme.example/careers/job/1005",
      },
      // 6: not an object
      null,
    ],
  ...
  if (parsed.length === 3) {
    ...
    fail(
      `drop rules: expected 3 kept, got ${parsed.length}: ${JSON.stringify(parsed.map((j) => j.title))}`
    );
  }

  const first = parsed[0];
  if (
    first?.title === "Staff Backend Engineer" &&
  ...
  if (parsed[1]?.url === buildJobUrl(T, "1002")) {
    pass(
      "parseEightfoldResponse() falls back to the tenant posting URL when canonicalPositionUrl is absent"
    );
  } else {
  ...
  if (parsed[2]?.title === "Product Manager" && parsed[2]?.postedAt === undefined) {
    pass(
      "parseEightfoldResponse() reads posting_name as a title fallback and omits postedAt when undated"
    );
  } else {
  ...
  const jobs = await ef.fetch(
    { name: "Acme", careers_url: "https://acme.eightfold.ai/careers" },
    ctx
  );
  ...
  if (
    calls.every(
      (c) => c.opts?.headers?.["User-Agent"] && c.opts.headers.Accept === "application/json"
    )
  ) {
  ...
  await ef.fetch(
    { name: "Big", careers_url: "https://big.eightfold.ai/careers" },
