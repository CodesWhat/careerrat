...
try {
  const bamboohrModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/bamboohr.mjs")).href
  );
  ...
  if (apiHit && apiHit.url === "https://acme.bamboohr.com/careers/list")
    pass("bamboohr.detect() honours explicit api: URL");
  ...
  if (
    bamboohr.detect({ name: "X", careers_url: null }) === null &&
    bamboohr.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    bamboohr.detect({
      name: "Spoof",
      careers_url: "https://evil.example/acme.bamboohr.com/foo",
    }) === null
  ) {
  ...
  const sample = {
    ...
    result: [
      {
        id: "15",
        jobOpeningName: "IT Security Engineer",
        location: { city: "Mayfair", state: "London, City of" },
        isRemote: null,
      },
      {
        id: 22,
        jobOpeningName: "Android Engineer",
  ...
  if (jobs.length === 2)
    pass("parseBambooHRResponse keeps rows with non-empty id + title, drops the rest");
  else fail(`parseBambooHRResponse returned ${jobs.length} jobs (expected 2)`);

  if (!jobs.some((j) => j.title === "Blank ID Role"))
    pass("parseBambooHRResponse drops blank/whitespace-id rows (no /careers/ URL)");
  ...
  if (jobs[0]?.location === "Mayfair, London, City of")
    pass("parseBambooHRResponse joins city + state");
  else fail(`parseBambooHRResponse location[0] was ${JSON.stringify(jobs[0]?.location)}`);

  if (jobs[1]?.location === "Bengaluru, Karnataka, Remote")
    pass("parseBambooHRResponse appends Remote when isRemote is set");
  else fail(`parseBambooHRResponse location[1] was ${JSON.stringify(jobs[1]?.location)}`);

  if (jobs[1]?.url === "https://acme.bamboohr.com/careers/22")
    pass("parseBambooHRResponse coerces numeric id to URL");
  else fail(`parseBambooHRResponse numeric-id url was ${jobs[1]?.url}`);

  // empty / malformed payloads → []
  if (parseBambooHRResponse({}, "X", "https://x.bamboohr.com").length === 0)
    pass("parseBambooHRResponse empty {} → []");
  else fail("parseBambooHRResponse should return [] for {}");
  if (parseBambooHRResponse({ result: null }, "X", "https://x.bamboohr.com").length === 0)
    pass("parseBambooHRResponse result:null → []");
  ...
  const fetched = await bamboohr.fetch(
    { name: "Acme", careers_url: "https://acme.bamboohr.com/careers" },
    mockCtx
  );
  if (
    fetchedUrl === "https://acme.bamboohr.com/careers/list" &&
    ...
    fail(
      `bamboohr.fetch() url=${fetchedUrl} redirect=${JSON.stringify(fetchedOpts)} jobs=${fetched.length}`
    );
  }
} catch (e) {
