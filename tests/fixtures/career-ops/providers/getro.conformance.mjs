...
try {
  const getro = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/getro.mjs")).href)
  ).default;
  const { extractCollectionId } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/getro.mjs")).href
  );
  ...
  const okEntry = {
    name: "ExampleVC",
    careers_url: "https://jobs.examplevc.example",
    getro_collection: 4283,
  };

  const getHit = getro.detect(okEntry);
  if (getHit && getHit.url === "https://api.getro.com/api/v2/collections/4283/search/jobs")
    pass("getro.detect() claims an explicit collection");
  ...
  if (getro.detect({ name: "X", careers_url: "https://jobs.examplevc.example" }) === null) {
    pass(
      "getro.detect() returns null without an explicit getro_collection (auto-resolve entries opt in via provider: getro instead)"
    );
  } else {
  ...
  try {
    await getro.fetch(
      { name: "Plaintext", careers_url: "http://jobs.examplevc.example", getro_collection: 4283 },
      {
        fetchJson: async () => {
    ...
    getroNonHttpsThrew = /https/.test(e.message);
  }
  ...
  const nextDataHtml =
    '<html><script id="__NEXT_DATA__" type="application/json">{"props":{"pageProps":{"network":{"id":9911}}}}</script></html>';
  if (extractCollectionId(nextDataHtml) === "9911")
    pass("extractCollectionId() reads network.id from __NEXT_DATA__");
  else fail(`extractCollectionId() returned ${JSON.stringify(extractCollectionId(nextDataHtml))}`);
  if (extractCollectionId("<html>no next data here</html>") === null)
    pass("extractCollectionId() returns null when __NEXT_DATA__ is missing");
  ...
  const reorderedNextDataHtml =
    '<html><script type="application/json" nonce="abc123" id="__NEXT_DATA__">{"props":{"pageProps":{"network":{"id":9911}}}}</script></html>';
  if (extractCollectionId(reorderedNextDataHtml) === "9911")
    pass("extractCollectionId() tolerates reordered/extra script attributes");
  else
    fail(
      `extractCollectionId() with reordered attributes returned ${JSON.stringify(extractCollectionId(reorderedNextDataHtml))}`
    );

  // Single quotes and spaced-out `id = '...'` are valid HTML too.
  const singleQuoteNextDataHtml =
    '<html><script id = \'__NEXT_DATA__\' type=\'application/json\'>{"props":{"pageProps":{"network":{"id":9911}}}}</script></html>';
  if (extractCollectionId(singleQuoteNextDataHtml) === "9911")
    pass("extractCollectionId() tolerates single-quoted / spaced id attribute");
  else
    fail(
      `extractCollectionId() with single quotes returned ${JSON.stringify(extractCollectionId(singleQuoteNextDataHtml))}`
    );
  ...
  const dataIdOnlyHtml =
    '<html><script data-id="__NEXT_DATA__" type="application/json">{"props":{"pageProps":{"network":{"id":9911}}}}</script></html>';
  if (extractCollectionId(dataIdOnlyHtml) === null)
    pass("extractCollectionId() does not false-match a data-id attribute");
  else
    fail(
      `extractCollectionId() should return null for data-id-only, got ${JSON.stringify(extractCollectionId(dataIdOnlyHtml))}`
    );

  // fetch() auto-resolves collection_id from careers_url when getro_collection is absent.
  let fetchTextUrl = null;
  const autoJobs = await getro.fetch(
    { name: "Auto Fund", careers_url: "https://careers.examplevc.example/jobs" },
    {
      fetchText: async (url) => {
        fetchTextUrl = url;
        return nextDataHtml;
      ...
      fetchJson: async (url) => {
        if (url !== "https://api.getro.com/api/v2/collections/9911/search/jobs")
          throw new Error(`unexpected collection in URL: ${url}`);
        return {
          results: {
            count: 1,
            jobs: [
              {
                title: "PM",
                url: "https://careers.examplevc.example/x",
  ...
  if (fetchTextUrl === "https://careers.examplevc.example/jobs")
    pass("getro.fetch() auto-resolves collection_id by fetching careers_url");
  else fail(`getro.fetch() fetched careers_url = ${JSON.stringify(fetchTextUrl)}`);
  if (autoJobs.length === 1 && autoJobs[0].company === "Acme")
    pass("getro.fetch() (auto-resolve) normalizes a job row");
  ...
  await getro.fetch(okEntry, {
    fetchText: async () => {
      overrideFetchTextCalled = true;
      return nextDataHtml;
    },
    fetchJson: async () => ({ results: { count: 0, jobs: [] } }),
  });
  if (!overrideFetchTextCalled)
    pass("getro.fetch() skips the careers_url fetch when getro_collection is set");
  else
    fail(
      "getro.fetch() should not fetch careers_url when getro_collection already resolves the id"
    );
  ...
  const getroResolveRecovered = await getro.fetch(
    { name: "Flaky Fund", careers_url: "https://careers.examplevc.example/jobs" },
    {
      sleep: async () => {},
      fetchText: async () => {
  ...
  if (getroCollectionResolveAttempts === 3 && getroResolveRecovered.length === 1) {
    pass(
      `getro.fetch() retries the collection_id auto-resolve fetch and recovers (${getroCollectionResolveAttempts} attempts)`
    );
  } else {
    fail(
      `getro.fetch() collection_id resolve retry: ${getroCollectionResolveAttempts} attempts, ${getroResolveRecovered.length} jobs`
    );
  }
  ...
  try {
    await getro.fetch(
      { name: "Broken Fund", careers_url: "https://careers.example.com/jobs" },
      {
        fetchText: async () => "<html>no next data</html>",
        fetchJson: async () => {
          throw new Error("should not reach the search API");
        },
      }
    );
  } catch (e) {
    unresolvedThrew = /could not resolve collection_id/.test(e.message);
  }
  if (unresolvedThrew)
    pass("getro.fetch() throws an actionable error when collection_id cannot be resolved");
      ...
      return {
        results: {
          ...
          jobs: [
            {
              title: "Principal",
              url: "https://jobs.examplevc.example/x",
              organization: { name: "Acme" },
              locations: ["Zurich"],
            },
          ],
  ...
  if (getroOpts?.headers?.referer === "https://jobs.examplevc.example/")
    pass("getro.fetch() sends a referer derived from careers_url");
  else fail(`getro.fetch() referer header = ${JSON.stringify(getroOpts?.headers?.referer)}`);
  if (
    getroJobs.length === 1 &&
    getroJobs[0].company === "Acme" &&
    getroJobs[0].location === "Zurich"
  )
    pass("getro.fetch() normalizes a job row");
    ...
    fetchJson: async () => ({
      results: {
        ...
        jobs: [
          {
            title: "Eng",
            url: "https://jobs.examplevc.example/loc",
            organization: { name: "Acme" },
  ...
  if (getroLocations[0]?.location === "Zurich, Bern, Remote")
    pass("getro.fetch() joins multiple locations and appends a Remote tag from work_mode");
    ...
    fetchJson: async () => ({
      results: {
        ...
        jobs: [
          {
            title: "VP Eng",
            url: "https://jobs.examplevc.example/sal",
            organization: { name: "Acme" },
            compensation_amount_min_cents: 12_000_000,
            compensation_amount_max_cents: 15_000_000,
  ...
  if (
    getroSalary[0]?.salary?.min === 120_000 &&
    getroSalary[0]?.salary?.max === 150_000 &&
    getroSalary[0]?.salary?.currency === "EUR"
  ) {
    ...
    fetchJson: async () => ({
      results: {
        ...
        jobs: [
          {
            title: "Contractor",
            url: "https://jobs.examplevc.example/hr",
            organization: { name: "Acme" },
            compensation_amount_min_cents: 5000,
  ...
  if (getroHourly[0]?.salary === undefined)
    pass("getro.fetch() ignores a non-annual compensation_period");
  else
    fail(
      `getro.fetch() should omit salary for compensation_period="hour", got ${JSON.stringify(getroHourly[0]?.salary)}`
    );
    ...
    fetchJson: async () => ({
      results: {
        ...
        jobs: [
          {
            title: "Recent",
            url: "https://jobs.examplevc.example/y",
            organization: { name: "Acme" },
  ...
  if (getroDated[0]?.postedAt === 1_900_000_000_000)
    pass("getro.fetch() converts created_at Unix seconds to epoch ms");
  ...
  let getroPages = 0;
  await getro.fetch(
    { ...okEntry, getro_max_pages: 1_000_000 },
    {
        ...
        return {
          results: {
            ...
            jobs: [
              {
                title: "T",
                url: `https://jobs.examplevc.example/${getroPages}`,
                organization: { name: "Acme" },
              },
            ],
          },
  ...
  if (getroPages <= 1500)
    pass(`getro.fetch() clamps an oversized getro_max_pages override (${getroPages} pages)`);
    ...
    fetchJson: async () => {
      ...
      return {
        results: {
          count: 100,
          jobs: [
            { title: "T", url: "https://jobs.examplevc.example/p", organization: { name: "Acme" } },
          ],
        },
      };
    },
  ...
  let getroAgePages = 0;
  const getroAged = await getro.fetch(
    { ...okEntry, getro_max_age_days: 90 },
    {
      fetchJson: async () => {
        getroAgePages++;
        return {
          results: {
            ...
            jobs: [
              {
                title: "Stale",
                url: `https://jobs.examplevc.example/s${getroAgePages}`,
                organization: { name: "Acme" },
  ...
  if (getroAged.length === 0 && getroAgePages === 1)
    pass("getro.fetch() stops pagination once postings cross getro_max_age_days");
  else
    fail(
      `getro.fetch() age-cutoff handling: ${getroAgePages} pages, ${getroAged.length} jobs kept`
    );
      ...
      return {
        results: {
          ...
          jobs: [
            {
              title: "Recovered",
              url: "https://jobs.examplevc.example/r",
              organization: { name: "Acme" },
            },
          ],
        },
  ...
  if (getroRetryAttempts === 3 && getroRecovered.length === 1)
    pass(`getro.fetch() retries a transient failure and recovers (${getroRetryAttempts} attempts)`);
  else
    fail(
      `getro.fetch() retry recovery: ${getroRetryAttempts} attempts, ${getroRecovered.length} jobs`
    );
  ...
  try {
    ...
    getroFirstPageThrew = e.message === "plain string rejection";
  }
  if (getroFirstPageThrew)
    pass(
      "getro.fetch() throws (not []) when the first page fails after exhausting retries, tolerating a non-Error rejection"
    );
        ...
        return {
          results: {
            ...
            jobs: [
              {
                title: "First",
                url: "https://jobs.examplevc.example/f",
                organization: { name: "Acme" },
              },
            ],
          },
  ...
  if (getroLaterPageFail.length === 1 && getroLaterPageFail[0].title === "First") {
    pass(
      "getro.fetch() truncates gracefully (keeps page 0) when a LATER page fails after exhausting retries"
    );
  } else {
  ...
  if (Array.isArray(getroEmpty) && getroEmpty.length === 0)
    pass("getro.fetch() tolerates an empty payload");
