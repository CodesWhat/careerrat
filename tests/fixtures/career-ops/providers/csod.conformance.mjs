...
try {
  const csodModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/csod.mjs")).href
  );
  ...
  if (resolveConfig({ api: "https://career-x.csod.com/other/path" }) === null)
    pass("csod.resolveConfig() rejects non-careersite paths");
  ...
  if (
    resolveConfig({ api: "http://career-ohb.csod.com/ux/ats/careersite/4/home?c=career-ohb" }) ===
    null
  )
    pass("csod.resolveConfig() rejects http:// (cookies must never go out in clear)");
  ...
  if (
    ...
    pass("csod.detect() matches *.csod.com careersite URLs");
  else fail("csod.detect() should match a careersite URL");
  if (csod.detect({ careers_url: "https://evil.com/x.csod.com/ux/ats/careersite/4/home" }) === null)
    pass("csod.detect() rejects path-spoofed host");
  else fail("csod.detect() should reject path-spoofed host");
  if (csod.detect({ careers_url: "https://csod.com.evil.com/ux/ats/careersite/4/home" }) === null)
    pass("csod.detect() rejects suffix-spoofed host");
  else fail("csod.detect() should reject suffix-spoofed host");

  // extractToken — anonymous JWT embedded in the bootstrap home page.
  if (extractToken('x{"token":"eyJab.c-d_e"}y') === "eyJab.c-d_e")
    pass("csod.extractToken() pulls the bearer token");
  else
    fail(`csod.extractToken() wrong: ${JSON.stringify(extractToken('x{"token":"eyJab.c-d_e"}y'))}`);
  if (extractToken("<html>no token</html>") === "" && extractToken(undefined) === "")
    pass('csod.extractToken() returns "" when absent');
  else fail('csod.extractToken() should return "" when absent');

  // parseCsodDate — US M/D/YYYY; junk → undefined.
  if (parseCsodDate("7/3/2026") === Date.UTC(2026, 6, 3))
    pass("csod.parseCsodDate() reads M/D/YYYY");
  else fail(`csod.parseCsodDate() wrong: ${parseCsodDate("7/3/2026")}`);
  if (
    parseCsodDate("13/40/2026") === undefined &&
    parseCsodDate("garbage") === undefined &&
    parseCsodDate(null) === undefined
  ) {
  ...
  if (cleanLocations([{ city: "Bremen", state: "Bremen", country: "DE" }]) === "Bremen, DE")
    pass('csod.cleanLocations() renders "City, CC"');
  else
    fail(
      `csod.cleanLocations() wrong: ${JSON.stringify(cleanLocations([{ city: "Bremen", country: "DE" }]))}`
    );
  if (
    cleanLocations([
      { city: "A", country: "DE" },
    ...
    pass("csod.cleanLocations() joins multiple locations");
  else fail('csod.cleanLocations() should join multiple locations with " / "');
  if (cleanLocations(undefined) === "" && cleanLocations([null]) === "")
    pass("csod.cleanLocations() tolerates missing/null input");
      ...
      requisitions: [
        {
          ...
          displayJobTitle: "IT Specialist (m/w/d)",
  ...
  const reqs = parseRequisitions(reqJson, {
    origin: "https://career-ohb.csod.com",
    siteId: 4,
    corpName: "career-ohb",
  });
  ...
  if (
    reqs[0]?.url ===
    "https://career-ohb.csod.com/ux/ats/careersite/4/home/requisition/8410?c=career-ohb"
  )
    pass("csod.parseRequisitions() builds the requisition detail URL");
  else fail(`csod.parseRequisitions() url wrong: ${JSON.stringify(reqs[0]?.url)}`);
  if (reqs[0]?.postedAt === Date.UTC(2026, 6, 3) && reqs[0]?.location === "Bremen, DE")
    pass("csod.parseRequisitions() maps postedAt and location");
  ...
  const searchPages = [
    { data: { totalCount: 27, requisitions: fullPage } },
    {
      data: {
        totalCount: 27,
        requisitions: [mkReq(25, "Job 25 dup"), mkReq(26, "Job 26"), mkReq(27, "Job 27")],
      },
    },
  ];
  let searchCalls = 0;
  ...
  const mockCtx = {
    ...
    fetchJson: async (url, opts) => {
      ...
      if (body.pageSize !== 25 || body.pageNumber !== searchCalls + 1)
        throw new Error("unexpected paging body");
      return searchPages[searchCalls++] ?? { data: { totalCount: 27, requisitions: [] } };
    },
  };
  const fetched = await csod.fetch(
    { name: "OHB", api: "https://career-ohb.csod.com/ux/ats/careersite/4/home?c=career-ohb" },
    mockCtx
  );
  if (fetched.length === 27 && new Set(fetched.map((j) => j.url)).size === 27 && searchCalls === 2)
    pass("csod.fetch() paginates via totalCount and dedups across pages");
  ...
  await csod
    .fetch(
      { name: "X", api: "https://x.csod.com/ux/ats/careersite/1/home?c=x" },
      { ...mockCtx, fetchText: async () => "<html/>" }
    )
    .catch((e) => {
      tokenErr = e.message;
    });
  if (/no anonymous token/.test(tokenErr))
    pass("csod.fetch() throws when the home page carries no token");
  ...
  const mkCookieCtx = (setCookies) => {
    ...
    return {
      ...
      sleep: async () => {},
      fetchText: async () => {
        throw new Error("fetchText must not be used when fetchResponse exists");
      },
  ...
  const cookieCtx = mkCookieCtx([
    "ASP.NET_SessionId=abc123; path=/; HttpOnly",
    "csod_tenant=kln; Secure; SameSite=None",
  ]);
  const cookieJobs = await csod.fetch(
    { name: "KLN", api: "https://careers-kln.csod.com/ux/ats/careersite/14/home?c=careers-kln" },
    cookieCtx
  );
  ...
  if (sentCookie === "ASP.NET_SessionId=abc123; csod_tenant=kln")
    pass("csod.fetch() replays bootstrap Set-Cookie as a cookie header (attributes stripped)");
  ...
  if (cookieCtx.calls.bootstrapOpts[0]?.redirect === "error")
    pass("csod.fetch() refuses redirects on the bootstrap request");
  else
    fail(
      `csod.fetch() bootstrap must pass redirect:'error', got ${JSON.stringify(cookieCtx.calls.bootstrapOpts[0]?.redirect)}`
    );
  ...
  if (bareCtx.calls.cookieHeaders[0] === undefined)
    pass("csod.fetch() omits the cookie header when the tenant sets none");
  else
    fail(
      `csod.fetch() should omit cookie header, got ${JSON.stringify(bareCtx.calls.cookieHeaders[0])}`
    );
  ...
  const legacyCtx = {
    ...
    fetchText: async (_url, opts) => {
      if (opts?.redirect !== "error")
        throw new Error(
          `legacy bootstrap must pass redirect:'error', got ${JSON.stringify(opts?.redirect)}`
        );
      return '{"token":"tok.legacy"}';
    },
    fetchJson: async (_url, opts) => {
      if (opts?.headers?.cookie !== undefined)
        throw new Error("legacy path must not send a cookie header");
      return { data: { totalCount: 1, requisitions: [mkReq(1, "Job 1")] } };
    },
  };
  const legacyJobs = await csod.fetch(
    { name: "L", api: "https://l.csod.com/ux/ats/careersite/2/home?c=l" },
    legacyCtx
  );
