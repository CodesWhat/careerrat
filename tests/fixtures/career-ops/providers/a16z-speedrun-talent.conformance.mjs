...
import { fetchJsonWithRetry } from "../../../../src/core/providers/career-ops/vendor/_http.mjs";
...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/a16z-speedrun-talent.mjs")).href
  );
  const provider = mod.default;
  const { normalizeSpeedrunJob } = mod;

  if (provider.id === "a16z-speedrun-talent")
    pass('a16z-speedrun-talent.id is "a16z-speedrun-talent"');
  ...
  const remoteLoc = normalizeSpeedrunJob({
    title: "R",
    url: "https://speedrun-talent-network.com/jobs/r",
    location: "SF Bay Area",
    remote: true,
  });
  const remoteOnly = normalizeSpeedrunJob({
    title: "R",
    url: "https://speedrun-talent-network.com/jobs/r2",
    location: null,
    remote: true,
  });
  if (remoteLoc?.location === "SF Bay Area, Remote" && remoteOnly?.location === "Remote") {
    ...
    fail(
      `normalizeSpeedrunJob remote locations = ${JSON.stringify({ a: remoteLoc?.location, b: remoteOnly?.location })}`
    );
  }

  // company fallbacks: entry name, then "a16z speedrun talent network".
  const coEntry = normalizeSpeedrunJob(
    { title: "T", url: "https://speedrun-talent-network.com/jobs/c1", company: "" },
    "Entry Name"
  );
  const coDefault = normalizeSpeedrunJob({
    title: "T",
    url: "https://speedrun-talent-network.com/jobs/c2",
  });
  const coBlank = normalizeSpeedrunJob(
    { title: "T", url: "https://speedrun-talent-network.com/jobs/c3" },
    "   "
  );
  if (
    coEntry?.company === "Entry Name" &&
    coDefault?.company === "a16z speedrun talent network" &&
    coBlank?.company === "a16z speedrun talent network"
  ) {
    pass('normalizeSpeedrunJob falls back company → entry name → "a16z speedrun talent network"');
  } else {
    fail(
      `normalizeSpeedrunJob company fallbacks = ${JSON.stringify({ a: coEntry?.company, b: coDefault?.company, c: coBlank?.company })}`
    );
  }

  // postedAt omitted when published_at absent/unparseable.
  const noDate = normalizeSpeedrunJob({
    title: "T",
    url: "https://speedrun-talent-network.com/jobs/nd",
    published_at: null,
  });
  const badDate = normalizeSpeedrunJob({
    title: "T",
    url: "https://speedrun-talent-network.com/jobs/bd",
  ...
  if (drops.every((r) => r === null)) {
    pass(
      "normalizeSpeedrunJob host-locks url to speedrun-talent-network.com and drops off-host/non-https/no-url/empty-title/non-object"
    );
  } else {
  ...
  const mk = (i) => ({
    title: `Role ${i}`,
    url: `https://speedrun-talent-network.com/jobs/x${i}`,
    company: `Co ${i}`,
    location: "New York, NY",
    remote: false,
  ...
  const page0 = {
    jobs: Array.from({ length: 50 }, (_, i) => mk(i)),
    total: 53,
    page: 0,
    page_size: 50,
    total_pages: 2,
  ...
  const page1 = {
    jobs: [mk(50), mk(51), { title: "", url: "https://speedrun-talent-network.com/jobs/bad" }],
    total: 53,
    page: 1,
    page_size: 50,
    total_pages: 2,
  ...
  const jobs = await provider.fetch(
    { name: "a16z speedrun talent network", max_pages: 5, q: "engineer" },
    ctx
  );
  const urlsOk =
    calls.length === 2 &&
  ...
  if (urlsOk && jobs.length === 52 && jobs[0].title === "Role 0" && jobs[51].title === "Role 51") {
    pass(
      "fetch() paginates 0-based with source=career-ops + q passthrough, stops after total_pages, drops invalid rows"
    );
  } else {
  ...
  const capCtx = {
    fetchJson: async (url) => {
      capCalls.push(url);
      return { jobs: Array.from({ length: 50 }, (_, i) => mk(i)), total_pages: 50 };
    },
  };
  const capped = await provider.fetch({ max_pages: 2 }, capCtx);
  if (capCalls.length === 2 && capped.length === 100)
    pass("fetch() honors max_pages ahead of total_pages");
  ...
  const livePagesOk =
    liveShapeCalls.length === 2 &&
    new URL(liveShapeCalls[0]).searchParams.get("page") === "0" &&
    new URL(liveShapeCalls[1]).searchParams.get("page") === "1";
  if (livePagesOk && liveShape.length === 100)
    pass("fetch() keeps paginating past a full 50-row page (live feed shape)");
  else
    fail(
      `live-shape pagination = ${JSON.stringify({ calls: liveShapeCalls, jobs: liveShape?.length })}`
    );
  if (liveWarnings.some((w) => w.includes("truncated at max_pages=2")))
    pass("fetch() warns when max_pages truncates the live-shape feed");
  ...
  if (bareCalls.length === 2 && bare.length === 99)
    pass("fetch() stops on a short page via the PER_PAGE=50 fallback when page_size is absent");
  ...
  const emptyCtx = {
    fetchJson: async (url) => {
      emptyCalls.push(url);
      return { jobs: [], total: 0, page: 0, page_size: 50, total_pages: 0 };
    },
  };
  const empty = await provider.fetch({ max_pages: 3 }, emptyCtx);
  if (emptyCalls.length === 1 && empty.length === 0)
    pass("fetch() returns [] after one call on an empty feed");
  ...
  const kwCtx = {
    fetchJson: async (url) => {
      kwCalls.push(url);
      return { jobs: [mk(0)], total_pages: 1 };
    },
  };
  await provider.fetch({ keywords: ["machine", "", "learning"] }, kwCtx);
  if (new URL(kwCalls[0]).searchParams.get("q") === "machine learning")
    pass("fetch() falls back to joined keywords[] when q: is absent");
  ...
  const defCtx = {
    fetchJson: async (url) => {
      ...
      defCalls.push(p);
      return {
        jobs: Array.from({ length: 50 }, (_, i) => mk(p * 50 + i)),
        total: 16830,
        page: p,
        page_size: 50,
        total_pages: 337,
  ...
  if (defPagesOk && defJobs.length === 300 && defDistinct)
    pass("fetch() default budget covers pages 0..5 → 300 distinct jobs");
  else fail(`default budget = ${JSON.stringify({ calls: defCalls, jobs: defJobs?.length })}`);
  if (defWarnings.some((w) => w.includes("truncated at max_pages=6")))
    pass("fetch() warns when the default budget truncates the feed");
  ...
  if (bigPagesOk && new Set(bigJobs.map((j) => j.url)).size === 1000 * 50)
    pass("fetch() clamps max_pages to the 1000-page cap (pages 0..999, distinct jobs)");
  else fail(`cap clamp = ${JSON.stringify({ calls: bigCalls.length, jobs: bigJobs?.length })}`);
  if (warnings.some((w) => w.includes("truncated at max_pages=1000")))
    pass("fetch() warns when the cap truncates the feed");
  ...
  if (atCapCalls.length === 1000 && overCapCalls.length === 1000)
    pass("fetch() cap boundary: max_pages=1000 runs unclamped, 1001 clamps to 1000");
  else
    fail(`cap boundary = ${JSON.stringify({ at: atCapCalls.length, over: overCapCalls.length })}`);
  ...
  for (const [label, bad] of [
    ...
    if (badCalls.length === 6)
      pass(`fetch() falls back to the 6-page default for invalid max_pages=${label}`);
    ...
    jobs: [
      {
        ...
        company_name: "Acme",
        url: "https://speedrun-talent-network.com/jobs/a1",
      },
    ],
    total_pages: 1,
  };

  {
    ...
    if (attempts === 2 && jobs.length === 1)
      pass("fetch() retries a transient 5xx and still returns the page (#2506)");
    else fail(`transient 5xx not retried: attempts=${attempts}, jobs=${jobs.length}`);
    if (slept.length === 1 && slept[0] > 0)
      pass("fetch() backs off between attempts via ctx.sleep (#2506)");
...

  {
    ...
    try {
      await provider.fetch({}, deadCtx);
    } catch {
      persistentThrew = true;
    }
    if (persistentThrew && attempts === 3)
      pass("fetch() gives up loudly after 3 bounded attempts, never a silent partial (#2506)");
...

  {
    ...
    try {
      await fetchJsonWithRetry(
        ...
        {},
        { retries: 5, baseDelayMs: 400, maxDelayMs }
      );
    } catch {
      /* expected */
    } finally {
    ...
    if (slept.every((ms) => ms >= 0 && ms <= maxDelayMs))
      pass("retry backoff never exceeds maxDelayMs once jitter is added (#2506)");
    else fail(`a jittered delay left [0, maxDelayMs]: ${JSON.stringify(slept)}`);
    if (capped.length > 1 && new Set(capped).size > 1)
      pass("jitter survives AT THE CAP, so concurrent retries de-synchronise (#2506)");
...

  {
    ...
    try {
      await fetchJsonWithRetry(
        ...
        "https://example.test/x",
        {},
        { retries: 3, baseDelayMs: 400, maxDelayMs: 100 }
      );
    } catch {
      /* expected */
    }
...

  {
    ...
    try {
      await provider.fetch({}, badReqCtx);
    } catch {
      fastFailed = true;
    }
