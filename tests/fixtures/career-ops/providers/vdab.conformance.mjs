...
try {
  const vdabModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/vdab.mjs")).href
  );
  ...
  if (
    ...
    cfg.keywords[0] === "python" &&
    cfg.keywords[1] === "data engineer"
  ) {
  ...
  if (
    deduped.keywords.length === 2 &&
    deduped.keywords[0] === "Backend" &&
    deduped.keywords[1] === "Python"
  ) {
  ...
  const detailsCfg = parseVdabConfig({
    vdab: { keywords: ["x"], fetchDetails: true, detailLimit: 999 },
  });
  ...
  if (
    normalizeJob({ vacaturefunctie: { naam: "No id" } }) === null &&
  ...
  if (
    extractDescription({
      functie: { omschrijving: { markdown: " **Build** things ", plainText: "Fallback" } },
    }) === "**Build** things" &&
    extractDescription({ functie: { omschrijving: { plainText: " Plain text " } } }) ===
      "Plain text" &&
    extractDescription({}) === ""
  ) {
  ...
  if (fetched.length === 1 && !("id" in fetched[0]))
    pass("vdab.fetch() dedups by id and strips id from output");
  else fail(`vdab.fetch() returned ${JSON.stringify(fetched)}`);
  if (sentKey === "b277002f-e1fa-4fc5-868a-fdab633c3851")
    pass("vdab.fetch() sends the vej-key-monitor header");
  ...
  // fetch() — detail enrichment is opt-in, bounded, batched, and fail-open.
  {
    ...
    const detailed = await vdab.fetch(
      {
        name: "VDAB",
        vdab: { keywords: ["python"], size: 100, fetchDetails: true, detailLimit: 2 },
      },
      ctx
    );
    ...
    if (
      ...
      pass(
        "vdab.fetch() opt-in detail enrichment adds descriptions, respects detailLimit, and fails open per detail"
      );
    } else {
      fail(
        `vdab.fetch() detail enrichment result=${JSON.stringify(detailed)}, calls=${JSON.stringify(detailCalls)}`
      );
    }
  }
  ...
  // cap, and every job across the batch boundary must still get enriched.
  {
    ...
    const batched = await vdab.fetch(
      {
        name: "VDAB",
        vdab: { keywords: ["python"], size: 100, fetchDetails: true, detailLimit: 25 },
      },
      {
        fetchJson: async (url) => {
          if (url.includes("/vacatures/")) {
    ...
    if (
      detailIds.length === 7 &&
      new Set(detailIds).size === 7 &&
      batched.every((j) => j.description === `Description ${j.url.split("/").pop()}`)
    ) {
      pass(
        "vdab.fetch() enriches every job across a DETAIL_BATCH boundary, not just the first batch"
      );
    } else {
      fail(
        `vdab.fetch() cross-batch enrichment: detailIds=${JSON.stringify(detailIds)}, batched=${JSON.stringify(batched)}`
      );
    }
  }
  ...
  // rather than trusting the short-page heuristic alone.
  {
    ...
    const neverEndingPage = await vdab.fetch(
      { name: "VDAB", vdab: { keywords: ["python"], size: 1 } },
      {
        fetchJson: async () => {
          pageRequests++;
          return { resultaten: [job(pageRequests, `Job ${pageRequests}`)] };
        },
      } // always a "full" page (length === size)
    );
    if (pageRequests === 50 && neverEndingPage.length === 50) {
      pass(
        "vdab.fetch() caps real-scan pagination at MAX_PAGES_PER_KEYWORD even when every page is full"
      );
    } else {
      fail(
        `vdab.fetch() unbounded-pagination guard: pageRequests=${pageRequests}, results=${neverEndingPage.length}`
      );
    }
  }
  ...
  // instead of throwing (e.g. TypeError on a non-iterable).
  {
    ...
    if (!malformedThrew && Array.isArray(malformedResult) && malformedResult.length === 0) {
      ...
      fail(
        `vdab.fetch() malformed response: threw=${malformedThrew}, result=${JSON.stringify(malformedResult)}`
      );
    }
    ...
    try {
      await vdab.fetch(
        { name: "VDAB", vdab: { keywords: ["python"] } },
        { fetchJson: async () => ({ resultaten: null }) }
      );
    } catch {
      nullThrew = true;
    }
    ...
    try {
      await vdab.fetch(
        { name: "VDAB", vdab: { keywords: ["python"] } },
        { fetchJson: async () => ({ resultaten: "oops" }) }
      );
    } catch {
      nonArrayThrew = true;
    }
    if (!nullThrew && !nonArrayThrew) {
      ...
      fail(
        `vdab.fetch() malformed resultaten: nullThrew=${nullThrew}, nonArrayThrew=${nonArrayThrew}`
      );
    }
  }
  ...
  // probing) propagates with its original identity intact.
  {
    ...
    const paged = await vdab.fetch(
      { name: "VDAB", vdab: { keywords: ["python"], size: 1 } },
      {
        ...
        }, // always a "full" page (length===size) — would paginate forever without the cap
      }
    );
    ...
    if (caught instanceof FakeSentinel) {
      pass(
        "vdab.fetch() propagates a per-keyword error unwrapped while probing (ctx.maxPages set)"
      );
    } else {
    ...
    if (
      scanCaught &&
      !(scanCaught instanceof FakeSentinel) &&
      /all 2 keyword/.test(scanCaught.message)
    ) {
      pass(
        "vdab.fetch() keeps recall-first summary-error behavior for a real scan (no ctx.maxPages)"
      );
    } else {
    ...
    if (
      detailCalls === 0 &&
      probedWithDetails.length === 1 &&
      !("description" in probedWithDetails[0])
    ) {
      pass("vdab.fetch() skips detail enrichment entirely while probing");
    } else {
      fail(
        `vdab.fetch() probe detail skip: detailCalls=${detailCalls}, result=${JSON.stringify(probedWithDetails)}`
      );
    }
  }
  ...
  // from the live bundle, then the retry succeeds with the fresh key.
  {
    ...
    if (
      Array.isArray(healed) &&
      healed.length === 0 &&
      fetchTextCalls === 2 &&
      keysSent.length === 2 &&
      keysSent[1] === FRESH_KEY
    ) {
      pass(
        "vdab.fetch() self-heals a rotated key: re-derives once from the live bundle and retries"
      );
    } else {
      fail(
        `self-heal: healed=${JSON.stringify(healed)}, fetchTextCalls=${fetchTextCalls}, keysSent=${JSON.stringify(keysSent)}`
      );
    }
  }
  ...
  // from the probing case already covered above.
  {
    ...
    try {
      ...
      threwNoBundleMatch = true;
      caughtNoBundleMatch = err;
    }
    if (
      threwNoBundleMatch &&
      /HTTP 403/.test(caughtNoBundleMatch.message) &&
      !/network error/.test(caughtNoBundleMatch.message)
    ) {
      pass(
        "vdab.fetch() surfaces the original 403 unchanged when the live bundle URL cannot be found"
      );
    } else {
      fail(
        `give-up (no bundle match): threw=${threwNoBundleMatch}, error=${caughtNoBundleMatch?.message}`
      );
    }
    ...
    try {
      ...
      threwNoKeyMatch = true;
      caughtNoKeyMatch = err;
    }
    if (
      threwNoKeyMatch &&
      /HTTP 403/.test(caughtNoKeyMatch.message) &&
      !/network error/.test(caughtNoKeyMatch.message)
    ) {
      pass(
        "vdab.fetch() surfaces the original 403 unchanged when the bundle has no matching key literal"
      );
    } else {
    ...
    try {
      await vdab.fetch(
        { name: "VDAB", vdab: { keywords: ["python"] } },
        {
          ...
          }, // deriveKeyFromBundle itself rejects
        }
      );
    } catch (err) {
      threwFetchTextError = true;
      caughtFetchTextError = err;
    }
    if (
      threwFetchTextError &&
      /HTTP 403/.test(caughtFetchTextError.message) &&
      !/network error/.test(caughtFetchTextError.message)
    ) {
      pass(
        "vdab.fetch() surfaces the original 403 unchanged when fetchText itself rejects during re-derivation"
      );
    } else {
      fail(
        `give-up (fetchText throws): threw=${threwFetchTextError}, error=${caughtFetchTextError?.message}`
      );
    }
  }
  ...
  // multiple keywords, and does not loop forever if the key never validates.
  {
    ...
    if (stillOutageThrew && jsonCalls === 3 && fetchTextCalls === 2) {
      pass(
        "vdab.fetch() attempts self-heal derivation only once per fetch() call, not per keyword"
      );
    } else {
      fail(
        `self-heal-once: threw=${stillOutageThrew}, jsonCalls=${jsonCalls}, fetchTextCalls=${fetchTextCalls}`
      );
    }
  }
  ...
  // fetch() — a non-403 error never triggers self-heal (fetchText untouched)
  {
    ...
    let threw500 = false;
    try {
      await vdab.fetch({ name: "VDAB", vdab: { keywords: ["a"] } }, ctx);
    } catch {
      threw500 = true;
    }
    if (threw500 && fetchTextCalls === 0)
      pass("vdab.fetch() does not attempt self-heal on a non-403 error");
