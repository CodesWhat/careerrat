...
try {
  const larajobsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/larajobs.mjs")).href
  );
  ...
  if (hit && hit.url === "https://larajobs.com/feed")
    pass("larajobs.detect() resolves provider:larajobs → feed URL");
  else fail(`larajobs.detect() returned ${JSON.stringify(hit)}`);
  if (larajobs.detect({ name: "X" }) === null)
    pass("larajobs.detect() returns null without provider:larajobs");
  ...
  if (
    jobs[0]?.title === "Laravel & Vue Developer" &&
    jobs[0]?.url === "https://larajobs.com/job/3899"
  ) {
  ...
  if (
    jobs[1]?.company === "Only Creator Co" &&
    jobs[1]?.location === "" &&
  ...
  const bareJobs = parseLarajobsFeed(
    "<item><title>Bare</title><link>https://larajobs.com/job/1</link></item>",
    "FallbackName"
  );
  if (bareJobs[0]?.company === "FallbackName")
    pass("parseLarajobsFeed falls back to the entry name for company");
  ...
  const fetchJobs = await larajobs.fetch(
    { name: "LaraJobs", provider: "larajobs" },
    {
      ...
      fetchText: async (url, options) => {
        if (url !== "https://larajobs.com/feed")
  ...
  if (fetchJobs.length === 2)
    pass("larajobs.fetch() hits the feed with redirect:error and returns parsed jobs");
