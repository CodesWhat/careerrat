...
try {
  const sj = (
    await import(
      pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/solidjobs.mjs")).href
    )
  ).default;
  ...
  if (
    sj.detect({
      name: "X",
      careers_url: "https://evil.example/solid.jobs/public-api/offers/it",
    }) === null
  ) {
  ...
  const fakeJobs = {
    jobs: [
      {
        title: "Senior Dev",
        url: "https://solid.jobs/o/abc123/career-ops",
        company: "Acme Corp",
        locations: ["Warszawa", "Remote"],
      },
      {
        title: "Junior Dev",
        url: "https://solid.jobs/o/def456/career-ops",
        company: "Beta Inc",
        locations: ["Kraków"],
      },
    ],
  ...
  if (parsed[0].company === "Acme Corp") pass("solidjobs.fetch() uses j.company from API response");
    ...
    fail(`solidjobs.fetch() company is ${JSON.stringify(parsed[0].company)}, expected "Acme Corp"`);

  if (parsed[0].location === "Warszawa, Remote") pass("solidjobs.fetch() joins locations array");
  else
    fail(
      `solidjobs.fetch() location is ${JSON.stringify(parsed[0].location)}, expected "Warszawa, Remote"`
    );

  if (
    parsed[0].title === "Senior Dev" &&
    parsed[0].url === "https://solid.jobs/o/abc123/career-ops"
  ) {
  ...
  const noCompanyJobs = {
    jobs: [{ title: "Tester", url: "https://solid.jobs/o/xyz/career-ops", locations: [] }],
  };
  ...
  if (fallback[0].company === "SolidJobs IT")
    pass("solidjobs.fetch() falls back to entry.name when j.company missing");
  else fail(`solidjobs.fetch() fallback company is ${JSON.stringify(fallback[0].company)}`);

  // fetch() handles empty locations array
  if (fallback[0].location === "")
    pass("solidjobs.fetch() returns empty string for empty locations array");
  else
    fail(`solidjobs.fetch() location for empty array is ${JSON.stringify(fallback[0].location)}`);
  ...
  if (filtered.length === 1 && filtered[0].title === "Has URL")
    pass("solidjobs.fetch() filters out jobs with empty/missing url");
  else
    fail(
      `solidjobs.fetch() should filter empty URLs, got ${filtered.length} jobs: ${JSON.stringify(filtered)}`
    );

  // fetch() handles string locations (non-array)
  const stringLocJobs = {
    jobs: [
      { title: "Dev", url: "https://solid.jobs/o/2/career-ops", company: "X", locations: "Warsaw" },
    ],
  };
  ...
  if (strLoc[0].location === "Warsaw") pass("solidjobs.fetch() handles string locations");
    ...
    fail(
      `solidjobs.fetch() string location is ${JSON.stringify(strLoc[0].location)}, expected "Warsaw"`
    );
  ...
  if (capturedOpts && capturedOpts.redirect === "error")
    pass('solidjobs.fetch() passes redirect:"error" to fetchJson');
  ...
  const malformedMembers = {
    jobs: [null, 7, { title: "OK", url: "https://solid.jobs/o/3/career-ops", company: "Z" }],
  };
