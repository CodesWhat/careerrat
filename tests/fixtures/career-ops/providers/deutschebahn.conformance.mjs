...
try {
  const dbModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/deutschebahn.mjs")).href
  );
  ...
  if (dbCfg && dbCfg.searchBase === "https://db.jobs/service/search/de-de/5441588")
    pass("deutschebahn.resolveConfig() pins the search id from the URL");
  else fail(`deutschebahn.resolveConfig() wrong: ${JSON.stringify(dbCfg)}`);
  if (
    db.detect({ careers_url: "https://evil.com/x.db.jobs" }) === null &&
    db.detect({ careers_url: "https://db.jobs.evil.com/x" }) === null
  ) {
  ...
  const dbHtml =
    "<html>" +
    dbHit("630365", "Teilprojektleiter:in Tunnel / Logistik", "München, Deutschland") +
  ...
  if (
    dbRows[0]?.title === "Teilprojektleiter:in Tunnel / Logistik" &&
    dbRows[0]?.location === "München, Deutschland"
  )
    pass("deutschebahn.parseHits() extracts the title span and the Arbeitsort location");
  else fail(`deutschebahn.parseHits() fields wrong: ${JSON.stringify(dbRows[0])}`);
  if (
    dbRows[0]?.url ===
    "https://db.jobs/de-de/Suche/Teilprojektleiter-in-Tunnel-Logistik-1396630365?jobId=630365"
  )
    pass("deutschebahn.parseHits() builds the absolute posting URL");
  else fail(`deutschebahn.parseHits() url wrong: ${JSON.stringify(dbRows[0]?.url)}`);
  if (
    dbParseHits("<html>no hits</html>", "https://db.jobs").length === 0 &&
    dbParseHits(undefined, "https://db.jobs").length === 0
  )
    pass("deutschebahn.parseHits() returns [] for hit-less / non-string input");
  ...
  const badHtml =
    "<html>" + dbHit("700001", "Bad&#xD800;Entity", "Berlin, Deutschland") + "</html>";
  const badRows = dbParseHits(badHtml, "https://db.jobs");
  if (badRows.length === 1 && badRows[0].title === "Bad&#xD800;Entity")
    pass("deutschebahn.parseHits() tolerates an invalid numeric entity (no RangeError crash)");
  else
    fail(
      `deutschebahn.parseHits() should degrade a malformed entity to literal text, got: ${JSON.stringify(badRows)}`
    );

  // fetch — paginates ?pageNum=N, stops on the first no-fresh-ids page.
  const dbPages = [
    dbHtml,
    "<html>" + dbHit("700000", "C", "Berlin, Deutschland") + "</html>",
    "<html></html>",
  ];
  let dbCalls = 0;
  const dbSeen = [];
  const dbCtx = {
    ...
    fetchText: async (url) => {
      dbSeen.push(url);
      return dbPages[dbCalls++] ?? "<html></html>";
    },
  };
  const dbJobs = await db.fetch(
    { name: "Deutsche Bahn", api: "https://db.jobs/service/search/de-de/5441588" },
    dbCtx
  );
  if (dbJobs.length === 3 && dbCalls === 3)
    pass("deutschebahn.fetch() paginates and stops on the first empty page");
  else fail(`deutschebahn.fetch() returned ${dbJobs.length} jobs after ${dbCalls} calls`);
  if (dbSeen[0]?.includes("pageNum=0") && dbSeen[1]?.includes("pageNum=1"))
    pass("deutschebahn.fetch() pages via pageNum=N (0-based)");
  else
    fail(
      `deutschebahn.fetch() paged wrong: ${JSON.stringify(dbSeen.map((u) => u.match(/pageNum=\d+/)?.[0]))}`
    );
  ...
  const capCtx = {
    ...
    fetchText: async () => {
      capCalls++;
      return dbHit(String(700100 + capCalls), `Job ${capCalls}`, "Berlin, Deutschland");
    },
  };
  const cappedJobs = await db.fetch(
    { name: "Deutsche Bahn", api: "https://db.jobs/service/search/de-de/5441588", max_pages: 3 },
    capCtx
  );
  if (cappedJobs.length === 3 && capCalls === 3)
    pass("deutschebahn.fetch() honors entry.max_pages and stops even with more pages available");
  else
    fail(
      `deutschebahn.fetch() max_pages cap wrong: ${cappedJobs.length} jobs after ${capCalls} calls`
    );
  ...
  const fallbackCtx = {
    ...
    fetchText: async () => {
      fallbackCalls++;
      return fallbackCalls <= 5
        ? dbHit(String(700200 + fallbackCalls), `Job ${fallbackCalls}`, "Berlin, Deutschland")
        : "<html></html>";
    },
  };
  const fallbackJobs = await db.fetch(
    { name: "Deutsche Bahn", api: "https://db.jobs/service/search/de-de/5441588", max_pages: 0 },
    fallbackCtx
  );
  if (fallbackJobs.length === 5 && fallbackCalls === 6)
    pass("deutschebahn.fetch() falls back to the default page cap for a non-positive max_pages");
  else
    fail(
      `deutschebahn.fetch() max_pages fallback wrong: ${fallbackJobs.length} jobs after ${fallbackCalls} calls`
    );
} catch (e) {
