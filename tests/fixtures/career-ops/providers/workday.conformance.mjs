...
import { captureConsoleErrors, fail, pass, ROOT, run, warn } from "../helpers.mjs";

console.log("\nProvider — workday");

try {
  const workdayModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/workday.mjs")).href
  );
  ...
  const hitNoLocale = workday.detect({
    name: "Test",
    careers_url: "https://test.wd5.myworkdayjobs.com/TestBoard",
  });
  if (
    hitNoLocale &&
    hitNoLocale.url === "https://test.wd5.myworkdayjobs.com/wday/cxs/test/TestBoard/jobs"
  ) {
  ...
  if (
    hitFallthrough &&
    hitFallthrough.url === "https://acme.wd12.myworkdayjobs.com/wday/cxs/acme/acme-jobs/jobs"
  ) {
  ...
  if (
    workday.detect({
      name: "Spoof",
      careers_url: "https://evil.example/test.wd5.myworkdayjobs.com/en-US/board",
    }) === null
  ) {
  ...
  if (
    workday.detect({ name: "X", careers_url: null }) === null &&
    workday.detect({ name: "X" }) === null
  ) {
    ...
    jobPostings: [
      {
        ...
        externalPath: "/job/board/Senior-QA-Engineer_JR001",
        locationsText: "Berlin, Germany",
        postedOn: "Posted 2 Days Ago",
      },
      {
        title: "Lead Developer",
        externalPath: "/job/board/Lead-Developer_JR002",
  ...
  if (parsed.length === 2)
    pass("parseWorkdayResponse extracts 2 valid jobs (skips missing title/path)");
  ...
  if (
    parsed[0].url.includes("acme-jobs") &&
    parsed[0].url.includes("/job/board/Senior-QA-Engineer_JR001")
  ) {
  ...
  const noLocEntry = {
    name: "Globex",
    careers_url: "https://globex.wd103.myworkdayjobs.com/globexcareers",
  };
  const noLocJson = {
    jobPostings: [
      ...
      {
        title: "Test Lead",
        externalPath: "/job/Remote-Poland/Test-Lead_ATCI-456",
        locationsText: "",
      }, // empty locationsText
      {
        title: "QA Analyst",
        externalPath: "/job/Remote-Hungary/QA-Analyst_ATCI-789",
        locationsText: "Remote, Hungary",
      }, // has locationsText — use it
    ],
  };
  const noLocParsed = parseWorkdayResponse(noLocJson, noLocEntry);
  if (noLocParsed[0]?.location === "Mumbai")
    pass("parseWorkdayResponse falls back to URL path location when locationsText absent");
  else
    fail(
      `parseWorkdayResponse path fallback: expected "Mumbai", got ${JSON.stringify(noLocParsed[0]?.location)}`
    );
  if (noLocParsed[1]?.location === "Remote Poland")
    pass("parseWorkdayResponse falls back to URL path location when locationsText empty");
  else
    fail(
      `parseWorkdayResponse path fallback empty: expected "Remote Poland", got ${JSON.stringify(noLocParsed[1]?.location)}`
    );
  if (noLocParsed[2]?.location === "Remote, Hungary")
    pass("parseWorkdayResponse prefers locationsText over URL path when present");
  else
    fail(
      `parseWorkdayResponse locationsText priority: expected "Remote, Hungary", got ${JSON.stringify(noLocParsed[2]?.location)}`
    );
  ...
  try {
    ...
    if (malformedParsed.length === 2 && malformedParsed[1].location === "Berlin") {
      pass(
        "parseWorkdayResponse tolerates malformed percent-encoding without dropping other records"
      );
    } else {
  ...
  const fetchedJobs = await workday.fetch(
    entry,
    mkWorkdayCtx(async (_url, opts) => {
      postRequests++;
  ...
  if (postRequests === 2 && fetchedJobs.length === 30) {
    ...
    fail(
      `fetch pagination: requests=${postRequests}, total=${fetchedJobs.length} (expected 2 requests / 30 jobs)`
    );
  }
  ...
  // skipped, not crash
  const sparseWorkday = {
    jobPostings: [null, undefined, { title: "Real Job", externalPath: "/job/board/real-job" }],
  };
  try {
  ...
  if (hugeWorkdayRequests === 100 && fetchedHugeWorkday.length === 2000) {
    ...
    fail(
      `workday fetch pagination cap: requests=${hugeWorkdayRequests}, total=${fetchedHugeWorkday.length} (expected 100/2000)`
    );
  }
  if (capturedWarnings.some((w) => /truncated at max_pages=\d+/.test(w))) {
    ...
    fail(
      `workday fetch cap: expected a truncation warning, got ${JSON.stringify(capturedWarnings)}`
    );
  }
  ...
  const capEntry = {
    name: "CappedCo",
    careers_url: "https://cappedco.wd5.myworkdayjobs.com/careers",
    max_pages: 3,
  };
  ...
  const runCapCase = async (ctxExtra) => {
    const { errors } = await captureConsoleErrors(() =>
      workday.fetch(
        capEntry,
        mkWorkdayCtx(async () => capPage(), ctxExtra)
      )
    );
  ...
  if (capFullRun.some((w) => ADVICE.test(w))) {
    ...
    fail(
      `workday cap warning (full run): expected the raise-max_pages advice, got ${JSON.stringify(capFullRun)}`
    );
  }
  ...
  if (capSinceRun.some((w) => ADVICE.test(w))) {
    ...
    fail(
      `workday cap warning (--since run): expected the raise-max_pages advice, got ${JSON.stringify(capSinceRun)}`
    );
  }
  ...
  if (
    capReverseScan.some((w) => /truncated at 3 pages/.test(w)) &&
    !capReverseScan.some((w) => ADVICE.test(w))
  ) {
    pass(
      "workday.fetch() cap warning: a reverse scan (synthesized entries) stays terse, no advice"
    );
  } else {
    fail(
      `workday cap warning (reverse scan): expected the terse line without advice, got ${JSON.stringify(capReverseScan)}`
    );
  }
  ...
  const suspectEntry = {
    name: "SuspectCo",
    careers_url: "https://suspectco.wd5.myworkdayjobs.com/careers",
    max_pages: 3,
  };
  ...
  if (suspectWarnings.some((w) => /total may be Workday-capped, not real/.test(String(w)))) {
    pass(
      "workday.fetch() cap warning: the Workday-capped-total tag survives on the reverse-scan line"
    );
  } else {
    fail(
      `workday cap warning (suspect total): expected the capped-total tag, got ${JSON.stringify(suspectWarnings)}`
    );
  }
  ...
  const bigWorkdayEntry = {
    name: "BigCo",
    careers_url: "https://bigco.wd5.myworkdayjobs.com/careers",
    max_pages: 80,
  };
  const fetchedOverriddenWorkday = await workday.fetch(
    bigWorkdayEntry,
    mkWorkdayCtx(async () => {
  ...
  if (overriddenWorkdayRequests === 80 && fetchedOverriddenWorkday.length === 1600) {
    ...
    fail(
      `workday fetch max_pages override: requests=${overriddenWorkdayRequests}, total=${fetchedOverriddenWorkday.length} (expected 80/1600)`
    );
  }
  ...
  const absurdWorkdayEntry = {
    name: "AbsurdCo",
    careers_url: "https://absurdco.wd5.myworkdayjobs.com/careers",
    max_pages: 100_000,
  };
  const fetchedAbsurdWorkday = await workday.fetch(
    absurdWorkdayEntry,
    mkWorkdayCtx(async () => {
  ...
  if (absurdWorkdayRequests === 1500 && fetchedAbsurdWorkday.length === 30_000) {
    ...
    fail(
      `workday fetch max_pages hard cap: requests=${absurdWorkdayRequests}, total=${fetchedAbsurdWorkday.length} (expected 1500/30000)`
    );
  }
  ...
  for (const invalidMaxPages of [-5, 0, "abc", NaN, null]) {
    ...
    const invalidEntry = {
      name: "InvalidCo",
      careers_url: "https://invalidco.wd5.myworkdayjobs.com/careers",
      max_pages: invalidMaxPages,
    };
    const fetchedInvalid = await workday.fetch(
      invalidEntry,
      mkWorkdayCtx(async () => {
        invalidRequests++;
        return {
    ...
    if (invalidRequests === 100 && fetchedInvalid.length === 2000) {
      ...
      fail(
        `workday fetch invalid max_pages=${label}: requests=${invalidRequests}, total=${fetchedInvalid.length} (expected 100/2000)`
      );
    }
  }
    ...
    workday.fetch(
      ...
      mkWorkdayCtx(async (_url, opts) => {
        ...
        return {
          ...
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `Job p${page}-${i}`,
            externalPath: `/job/board/p${page}-${i}`,
          })),
        };
      })
    )
  ...
  if (flakyWorkdayRequests === 6 && flakyWorkdayJobs.length === 40) {
    ...
    fail(
      `workday fetch partial failure: requests=${flakyWorkdayRequests}, total=${flakyWorkdayJobs.length} (expected 6/40)`
    );
  }
  if (flakyWarnings.some((w) => /truncated at \d+ of \d+ pages after 4 attempts/.test(w))) {
    pass(
      "workday.fetch() warns (console.error) when a page fetch fails after exhausting retries, with the attempt count"
    );
  } else {
    fail(
      `workday fetch page failure: expected a fetch-failed warning, got ${JSON.stringify(flakyWarnings)}`
    );
  }
  ...
  if (
    flakyWarnings.some((w) =>
      /truncated at 3 of 4 pages after 4 attempts \(40 of 80 jobs\): HTTP 503/.test(w)
    )
  ) {
  ...
  if (!flakyWarnings.some((w) => /raise max_pages/.test(w))) {
    ...
    fail(
      `workday fetch-error stop should not also warn about max_pages: ${JSON.stringify(flakyWarnings)}`
    );
  }
  ...
  // pattern — no per-tenant logging beyond the existing truncation warning).
  {
    ...
    if (jobs.workdayTruncated === true && jobs.length === 20) {
      ...
      fail(
        `expected workdayTruncated tag on 20 partial jobs, got tag=${jobs.workdayTruncated} len=${jobs.length}`
      );
    }
  }
  ...
  const retryEntry = {
    name: "RetryCo",
    careers_url: "https://retryco.wd5.myworkdayjobs.com/careers",
  };
  const retryJobs = await workday.fetch(
    retryEntry,
    mkWorkdayCtx(
      async () => {
        retryAttempts++;
        ...
        return {
          total: 1,
          jobPostings: [{ title: "Recovered Job", externalPath: "/job/board/recovered" }],
        };
      },
      {
        sleep: async (ms) => {
  ...
  if (retrySleepCalls[0] === 1000) {
    ...
    fail(
      `workday retry-after: expected first backoff delay 1000ms, got ${JSON.stringify(retrySleepCalls)}`
    );
  }
  ...
  const hostileRetrySleepCalls = [];
  let hostileRetryAttempts = 0;
  const hostileRetryEntry = {
    name: "HostileRetryCo",
    careers_url: "https://hostileretryco.wd5.myworkdayjobs.com/careers",
  };
  await workday.fetch(
    hostileRetryEntry,
    mkWorkdayCtx(
      ...
      {
        sleep: async (ms) => {
          hostileRetrySleepCalls.push(ms);
        },
      }
    )
  );
  if (hostileRetrySleepCalls[0] === 32_000) {
    ...
    fail(
      `workday retry-after clamp: expected 32000ms, got ${JSON.stringify(hostileRetrySleepCalls)}`
    );
  }
  ...
  if (non429Attempts === 2 && non429Jobs.length === 20) {
    ...
    fail(
      `workday non-retryable 4xx: attempts=${non429Attempts}, jobs=${non429Jobs.length} (expected 2/20)`
    );
  }
  ...
  if (non429Warnings.some((w) => /truncated at 2 of \d+ pages after 1 attempts/.test(w))) {
    pass(
      "workday.fetch() truncation warning reports the real attempt count for a non-retryable failure (1, not the retry-cap upper bound)"
    );
  } else {
    fail(
      `workday non-retryable 4xx warning: expected "after 1 attempts", got ${JSON.stringify(non429Warnings)}`
    );
  }
        ...
        async (_url, opts) => {
          ...
          if (page === 0) {
            ...
            return {
              ...
              jobPostings: Array.from({ length: 20 }, (_, i) => ({
                title: `Fresh ${i}`,
                externalPath: `/job/board/fresh-${i}`,
                postedOn: "Posted Today",
              })),
            };
          }
          ...
          return {
            ...
            jobPostings: Array.from({ length: 20 }, (_, i) => ({
              title: `Stale ${i}`,
              externalPath: `/job/board/stale-${i}`,
  ...
  if (earlyStopRequests === 2 && earlyStopJobs.length === 40) {
    ...
    fail(
      `workday early-stop: requests=${earlyStopRequests}, jobs=${earlyStopJobs.length} (expected 2/40)`
    );
  }
  if (!earlyStopWarnings.some((w) => /truncated at/.test(w))) {
    ...
    fail(
      `workday early-stop should not warn about max_pages: ${JSON.stringify(earlyStopWarnings)}`
    );
  }
  ...
  let wideRequests = 0;
  const wideJobs = await workday.fetch(
    entry,
    mkWorkdayCtx(
      async (_url, opts) => {
        wideRequests++;
    ...
    workday.fetch(
      ...
      mkWorkdayCtx(
        async () => ({
          ...
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `NoDate ${i}`,
            externalPath: `/job/board/nodate-${i}`,
          })), // no postedOn
        }),
  ...
  if (noDateWarnings.some((w) => /truncated at \d+ pages/.test(w))) {
    pass(
      "workday.fetch() cap-hit warning fires in reverse-scan context (tenant has no dates, includeUndated on)"
    );
  } else {
  ...
  if (!noDateWarnings.some((w) => /raise max_pages|portal entry|portals\.yml/.test(w))) {
    pass(
      "workday.fetch() reverse-scan cap warning gives no inactionable advice (no portal entry to edit)"
    );
  } else {
    fail(
      `workday reverse-scan cap warning should not suggest editing a portal entry: ${JSON.stringify(noDateWarnings)}`
    );
  }
  ...
  ); // includeUndated intentionally omitted — the default, falsy case
  if (skipRequests === 1 && skipJobs.length === 20) {
    pass(
      "workday.fetch() skips pagination after page 1 when includeUndated is off and the tenant has no dated postings"
    );
  } else {
  ...
  if (skipJobs.workdayNoDateSkip === true) {
    ...
    fail(
      `workday no-date-skip tag missing: jobs.workdayNoDateSkip = ${JSON.stringify(skipJobs.workdayNoDateSkip)}`
    );
  }
  if (skipWarnings.length === 0) {
    pass(
      "workday.fetch() does not console.error per-company on a no-date-skip (aggregated instead)"
    );
  } else {
  ...
  const noSkipJobs = await workday.fetch(
    entry,
    mkWorkdayCtx(
      async (_url, opts) => {
  ...
  if (noSkipWithIncludeUndatedRequests === 2 && noSkipJobs.length === 40) {
    ...
    fail(
      `workday no-date-skip gate: requests=${noSkipWithIncludeUndatedRequests}, jobs=${noSkipJobs.length} (expected 2/40)`
    );
  }
  ...
  if (!datedCapWarnings.some((w) => /Workday-capped/.test(w))) {
    pass(
      "workday.fetch() cap-hit warning omits the suspected-Workday-cap tag when total is far above the window"
    );
  } else {
    fail(
      `workday cap-hit warning should not suspect a Workday-side cap here (total=1,000,000, window=2,000): ${JSON.stringify(datedCapWarnings)}`
    );
  }
    ...
    workday.fetch(
      ...
      mkWorkdayCtx(
        async () => ({
          ...
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `Suspect ${i}`,
            externalPath: `/job/board/suspect-${i}`,
            postedOn: "Posted Today",
          })),
        }),
  ...
  if (suspectCapWarnings.some((w) => /\(total may be Workday-capped, not real\)/.test(w))) {
    pass(
      "workday.fetch() cap-hit warning flags a suspected Workday-side total cap when total === maxPages*PAGE_SIZE"
    );
  } else {
  ...
  const fallbackJobs = await workday.fetch(
    entry,
    mkWorkdayCtx(async (_url, opts) => {
      fallbackRequests++;
      const body = JSON.parse(opts.body);
      if (body.offset === 0)
        return {
          jobPostings: Array.from({ length: 20 }, (_, i) => ({
            title: `FB P1-${i}`,
            externalPath: `/job/board/fb-${i}`,
          })),
        };
      return {
        jobPostings: Array.from({ length: 5 }, (_, i) => ({
          title: `FB P2-${i}`,
          externalPath: `/job/board/fb2-${i}`,
        })),
      };
    })
  );
  if (fallbackRequests === 2 && fallbackJobs.length === 25) {
    pass(
      "workday.fetch() fallback (no total): paginates sequentially, stops on short page (20+5=25)"
    );
  } else {
    fail(
      `fallback pagination: requests=${fallbackRequests}, jobs=${fallbackJobs.length} (expected 2/25)`
    );
  }
  ...
  if (probeRequests === 1 && probeJobs.length === 20) {
    ...
    fail(
      `workday probe cap: requests=${probeRequests} (expected 1), jobs=${probeJobs?.length} (expected 20)`
    );
  }
