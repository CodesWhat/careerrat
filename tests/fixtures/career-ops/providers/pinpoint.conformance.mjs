...
try {
  const pinpointModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/pinpoint.mjs")).href
  );
  ...
  const hit = pinpoint.detect({
    name: "Pinpoint",
    careers_url: "https://workwithus.pinpointhq.com",
  });
  ...
  const hitWithPath = pinpoint.detect({
    name: "X",
    careers_url: "https://acme.pinpointhq.com/en/jobs",
  });
  ...
  if (
    pinpoint.detect({ name: "X", careers_url: null }) === null &&
    pinpoint.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    pinpoint.detect({
      name: "Spoof",
      careers_url: "https://evil.example/acme.pinpointhq.com/foo",
    }) === null
  ) {
  ...
  if (
    pinpoint.detect({ name: "Trailing", careers_url: "https://acme-.pinpointhq.com" }) === null &&
  ...
  if (
    pinpoint.detect({ name: "Mid", careers_url: "https://acme-co.pinpointhq.com" })?.url ===
  ...
  if (jobs.length === 3)
    pass(
      "parsePinpointResponse keeps 3 valid postings (drops empty-title / no-url / non-absolute / non-https)"
    );
  ...
  if (
    jobs[0]?.title === "Senior Product Manager" &&
    jobs[0]?.url === "https://workwithus.pinpointhq.com/en/postings/abc-123" &&
    jobs[0]?.company === "Pinpoint" &&
    jobs[0]?.location === "Remote"
  ) {
    pass(
      "parsePinpointResponse maps title/url, sets company from entry name, prefers location.name"
    );
  } else {
  ...
  if (
    ...
    fail(
      `parsePinpointResponse row 1 title/url = ${JSON.stringify({ title: jobs[1]?.title, url: jobs[1]?.url })}`
    );
  }
  ...
  if (jobs[1]?.location === "Berlin, Berlin") {
    ...
    fail(
      `parsePinpointResponse row 1 location = ${JSON.stringify(jobs[1]?.location)}, expected "Berlin, Berlin"`
    );
  }
  ...
  if (parsePinpointResponse({}, "X").length === 0)
    pass("parsePinpointResponse: empty {} → empty result");
  ...
  if (badEntryThrew)
    pass("pinpoint.fetch() throws when the careers_url is not a pinpointhq.com host");
