...
try {
  const tencent = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/tencent.mjs")).href)
  ).default;
  const { parseTencentResponse } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/tencent.mjs")).href
  );

  if (tencent.id === "tencent") pass('tencent.id is "tencent"');
  else fail(`tencent.id is ${JSON.stringify(tencent.id)}`);

  const hit = tencent.detect({
    name: "腾讯",
  ...
  if (
    tencent.detect({ name: "X", careers_url: "https://evil.example/careers.tencent.com" }) === null
  ) {
  ...
  if (
    tencent.detect({ name: "X", careers_url: "http://careers.tencent.com/search.html" }) === null
  ) {
  ...
  if (
    j1 &&
    j1.url === "http://careers.tencent.com/jobdesc.html?postId=1001" &&
    j1.location === "中国-深圳"
  ) {
  ...
  if (
    j2 &&
    j2.url === "https://careers.tencent.com/jobdesc.html?postId=1002" &&
    j2.postedAt === undefined
  ) {
  ...
  const pagedJobs = await tencent.fetch(
    { name: "腾讯", careers_url: TENCENT_URL, keywords: ["AI"] },
    paged.ctx
  );
  ...
  const overlapJobs = await tencent.fetch(
    { name: "腾讯", careers_url: TENCENT_URL, keywords: ["AI", "大模型"] },
    overlap.ctx
  );
  if (overlapJobs.length === 1 && overlap.calls.length === 2) {
    ...
    fail(
      `tencent.fetch() cross-keyword dedup: ${overlapJobs.length} jobs, ${overlap.calls.length} requests`
    );
  }
  ...
  const capped = mkCtx(() => ({
    ...
    Data: {
      Count: 500,
      Posts: Array.from({ length: 100 }, (_, i) => mkPost(3000 + i, `岗位C${i}`)),
    },
  }));
  await tencent.fetch(
    { name: "腾讯", careers_url: TENCENT_URL, keywords: ["AI"], max_pages: 1 },
    capped.ctx
  );
  ...
  const probe = mkCtx(() => ({
    ...
    Data: {
      Count: 500,
      Posts: Array.from({ length: 100 }, (_, i) => mkPost(4000 + i, `岗位D${i}`)),
    },
  }));
  ...
  if (probe.calls.length === 1 && probe.calls[0].keyword === "" && probeJobs.length === 100) {
    pass(
      "tencent.fetch() honors the ctx.maxPages probe hint and defaults to a whole-board (empty keyword) query"
    );
  } else {
    fail(
      `tencent.fetch() ctx.maxPages=1: ${probe.calls.length} requests, keyword=${JSON.stringify(probe.calls[0] && probe.calls[0].keyword)}`
    );
  }
  ...
  const blipJobs = await tencent.fetch(
    { name: "腾讯", careers_url: TENCENT_URL, keywords: ["AI", "大模型"] },
    blip.ctx
  );
  if (blipJobs.length === 1 && blipJobs[0].title === "幸存岗位") {
    ...
    fail(
      `tencent.fetch() partial results on failure: ${JSON.stringify(blipJobs.map((j) => j.title))}`
    );
  }
  ...
  try {
    zeroThenBlipJobs = await tencent.fetch(
      { name: "腾讯", careers_url: TENCENT_URL, keywords: ["AI", "大模型"] },
      zeroThenBlip.ctx
    );
  } catch {
  ...
  if (!zeroThenBlipThrew && Array.isArray(zeroThenBlipJobs) && zeroThenBlipJobs.length === 0) {
    pass(
      "tencent.fetch() treats a later failure as a blip even when earlier keywords matched 0 jobs (board is alive)"
    );
  } else {
    fail(
      `tencent.fetch() 0-result keyword then 503: ${zeroThenBlipThrew ? "threw" : JSON.stringify(zeroThenBlipJobs)}`
    );
  }
  ...
  if (firstFailThrew) {
    pass(
      "tencent.fetch() still throws when the very first request fails (dead board reads as failure)"
    );
  } else {
