...
try {
  const museModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/themuse.mjs")).href
  );
  ...
  if (
    normalizeMuseJob({
      name: "",
      refs: { landing_page: "https://www.themuse.com/jobs/x" },
      company: { name: "X" },
      locations: [],
    }) === null
  )
  ...
  if (noLoc?.location === "")
    pass("normalizeMuseJob returns empty location when locations array is empty");
  ...
  if (noCompany?.company === "The Muse")
    pass('normalizeMuseJob falls back to "The Muse" when company.name is missing');
  ...
  const page1Result = {
    name: "Data Engineer",
    refs: { landing_page: "https://www.themuse.com/jobs/acme/de" },
    company: { name: "Acme" },
    locations: [],
  };
  ...
  if (paginatedJobs.length === 2 && paginatedJobs[1]?.title === "Data Engineer")
    ...
    fail(
      `themuse.fetch() paginated results = ${JSON.stringify(paginatedJobs.map((j) => j.title))}`
    );
  ...
  await themuse.fetch(
    { name: "X", provider: "themuse" },
    {
      fetchJson: async () => {
        cappedCalls++;
        return { results: [], page: 0, page_count: 99999 };
      },
  ...
  if (cappedCalls === 100)
    pass("themuse.fetch() clamps page_count to 100 (prevents unbounded requests)");
  ...
  await themuse.fetch(
    { name: "X", provider: "themuse" },
    {
      fetchJson: async () => {
        nonIntCalls++;
        return { results: [], page: 0, page_count: 1.5 };
      },
    }
  );
  if (nonIntCalls === 1)
    pass("themuse.fetch() ignores non-integer page_count (fetches only page 0)");
  ...
  // the caller (no jobs lost, no error surfaced) and respects Retry-After.
  {
    ...
    if (sleepCalls[0] === 1000) {
      ...
      fail(
        `themuse retry-after: expected first backoff delay 1000ms, got ${JSON.stringify(sleepCalls)}`
      );
    }
  }
  ...
  // rather than throw.
  {
      ...
      themuse.fetch(
        { name: "The Muse Board", provider: "themuse" },
        {
          fetchJson: async (url) => {
            ...
            err.status = 503;
            throw err; // every page 1+ call fails
          },
          sleep: async () => {},
        }
      )
    ...
    if (jobs.length === 1 && jobs[0].title === "Staff AI Engineer") {
      ...
      fail(
        `themuse retry exhaustion: expected 1 preserved job, got ${JSON.stringify(jobs.map((j) => j.title))}`
      );
    }
    if (
      errors.some(
        (e) =>
          /truncated at page 1 of 3/.test(e) &&
          /after 4 attempts/.test(e) &&
          /1 jobs gathered so far/.test(e)
      )
    ) {
      pass(
        "themuse.fetch() warns (does not throw) with the real exhausted attempt count (4) when a page exhausts retries"
      );
    } else {
      fail(
        `themuse retry exhaustion: expected a truncation warning, got ${JSON.stringify(errors)}`
      );
    }
  }
  ...
  // fixed MAX_RETRIES+1 ceiling that only applies to exhausted retries.
  {
    ...
    if (calls === 2 && jobs.length === 1) {
      ...
      fail(
        `themuse non-retryable: calls=${calls}, jobs=${JSON.stringify(jobs.map((j) => j.title))}`
      );
    }
    if (errors.some((e) => /after 1 attempt /.test(e) && !/attempts/.test(e))) {
      pass(
        "themuse.fetch() reports the real attempt count (1) for a non-retryable error, not the retry ceiling"
      );
    } else {
      fail(
        `themuse non-retryable attempt count: expected "after 1 attempt", got ${JSON.stringify(errors)}`
      );
    }
  }
  ...
  // result to scan.mjs's consecutive-failure detector.
  {
      ...
      await themuse.fetch(
        { name: "The Muse Board", provider: "themuse" },
        {
          fetchJson: async () => {
            ...
            const err = new Error("HTTP 503");
            err.status = 503;
            throw err; // every call fails, including page 0
          },
          sleep: async () => {},
        }
      );
    ...
    if (threw)
      pass("themuse.fetch() throws (does not return []) when page 0 fails every retry attempt");
    else
      fail("themuse.fetch() should throw on a total page-0 failure, not resolve to an empty array");
    if (thrownMessage === "HTTP 503") {
      pass("themuse.fetch() propagates the real page-0 error rather than swallowing it");
    } else {
      fail(
        `themuse page-0 failure: expected the propagated error message "HTTP 503", got ${JSON.stringify(thrownMessage)}`
      );
    }
    // page 0 retried MAX_RETRIES+1=4 times, then threw -- no page 1 was ever attempted
    if (calls === 4)
      pass("themuse.fetch() exhausts retries on page 0 before giving up (4 attempts)");
  ...
  // throw that discards jobs already gathered from earlier pages.
  {
    ...
    if (jobs.length === 1 && jobs[0].title === "Staff AI Engineer") {
      pass(
        "themuse.fetch() preserves jobs already gathered when a later page has a malformed shape"
      );
    } else {
      fail(
        `themuse malformed later page: expected 1 preserved job, got ${JSON.stringify(jobs.map((j) => j.title))}`
      );
    }
    if (
      errors.some(
        (e) => /truncated at page 1 of 3/.test(e) && /unexpected API response on page 1/.test(e)
      )
    ) {
      pass("themuse.fetch() warns (does not throw) when a later page has a malformed shape");
    } else {
      fail(
        `themuse malformed later page: expected a truncation warning, got ${JSON.stringify(errors)}`
      );
    }
  }
} catch (e) {
