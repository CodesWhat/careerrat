...
try {
  const leverModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/lever.mjs")).href
  );
  ...
  const hitEu = lever.detect({
    name: "EuCo",
    careers_url: "https://jobs.eu.lever.co/euco/some-posting",
  });
  if (hitEu && hitEu.url === "https://api.eu.lever.co/v0/postings/euco") {
    pass(
      "lever.detect() resolves jobs.eu.lever.co → api.eu.lever.co and takes the first path segment as slug"
    );
  } else {
  ...
  if (
    lever.detect({ name: "Spoof", careers_url: "https://jobs.lever.co.evil.example/acme" }) === null
  ) {
  ...
  if (
    lever.detect({ name: "PathSpoof", careers_url: "https://evil.example/jobs.lever.co/acme" }) ===
    null
  ) {
  ...
  const hitApiEu = lever.detect({
    name: "PinnedEu",
    api: "https://api.eu.lever.co/v0/postings/euco",
  });
  ...
  if (
    capturedUrl === "https://api.lever.co/v0/postings/acme" &&
  ...
  if (
    fetched[0]?.title === "Staff Platform Engineer" &&
    ...
    pass(
      "lever.fetch() maps text/hostedUrl/entry.name/categories.location/descriptionPlain/createdAt"
    );
  else fail(`lever.fetch() row 0 = ${JSON.stringify(fetched[0])}`);

  if (fetched[1]?.description === "" && fetched[1]?.postedAt === undefined)
    pass(
      'lever.fetch() coerces a non-string descriptionPlain to "" and a non-number createdAt to undefined'
    );
  ...
  if (
    ...
    fetched[2]?.url === "" &&
    fetched[2]?.location === "" &&
    fetched[2]?.company === "Acme" &&
    fetched[2]?.description === "" &&
    fetched[2]?.postedAt === undefined
  )
  ...
  if (fetched[3]?.location === "Barcelona; Madrid; Montevideo (Hybrid)")
    pass(
      "lever.fetch() folds categories.allLocations into location (multi-location postings stay visible to location_filter)"
    );
  else fail(`lever.fetch() row 3 location = ${JSON.stringify(fetched[3]?.location)}`);

  if (fetched[4]?.location === "Madrid; Sao Paulo")
    pass(
      "lever.fetch() drops blank/non-string allLocations entries and case-insensitive duplicates of the primary"
    );
  ...
  for (const body of emptyCases) {
    ...
    if (!Array.isArray(out) || out.length !== 0) {
      emptyOk = false;
      fail(`lever.fetch() body=${JSON.stringify(body)} → ${JSON.stringify(out)}`);
      break;
    }
  }
  ...
  try {
    ...
    if (/cannot derive API URL for NoBoard/.test(e.message)) {
      pass(
        'lever.fetch() throws "cannot derive API URL" before fetching for an undetectable entry'
      );
    } else {
