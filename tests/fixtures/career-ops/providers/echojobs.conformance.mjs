...
try {
  const echojobsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/echojobs.mjs")).href
  );
  ...
  if (hit && hit.url === "https://echojobs.io/api/jobs")
    pass("echojobs.detect() resolves provider:echojobs → feed URL");
  else fail(`echojobs.detect() returned ${JSON.stringify(hit)}`);
  if (echojobs.detect({ name: "X" }) === null)
    pass("echojobs.detect() returns null without provider:echojobs");
  ...
  if (
    n &&
    n.title === "Senior Go Engineer" &&
    n.company === "Acme" &&
    n.url === "https://jobs.ashbyhq.com/acme/abc-123" &&
    n.location === "San Francisco, CA, New York, NY" &&
    n.postedAt === 1783380913765
  ) {
    pass(
      "normalizeEchojobsJob maps title/company/url/location/postedAt and keeps the external ATS URL"
    );
  } else {
  ...
  const remote = normalizeEchojobsJob({
    title: "X",
    url: "https://jobs.lever.co/x/1",
    locations: [],
    remote_type: "remote",
  });
  const hybrid = normalizeEchojobsJob({
    title: "X",
  ...
  if (remote?.location === "Remote" && hybrid?.location === "Hybrid") {
    pass(
      'normalizeEchojobsJob falls back to "Remote" for a placeless remote role, "Hybrid" for a placeless hybrid one (#2258)'
    );
  } else {
  ...
  const onSiteNoPlace = normalizeEchojobsJob({
    title: "X",
  ...
  if (onSiteNoPlace?.location === "") {
    pass(
      "normalizeEchojobsJob leaves location empty for a placeless on_site role (no false Remote/Hybrid)"
    );
  } else {
  ...
  if (hybridWithCity?.location === "Berlin · Hybrid") {
    pass(
      "normalizeEchojobsJob appends the Hybrid marker to a hybrid role that lists a city (#2258)"
    );
  } else {
  ...
  const remoteWithCity = normalizeEchojobsJob({
    title: "X",
  ...
  if (remoteWithCity?.location === "Berlin") {
    pass(
      "normalizeEchojobsJob leaves a placed remote role untouched (no marker beyond the #2258 scope)"
    );
  } else {
  ...
  const hybridOddCasing = normalizeEchojobsJob({
    title: "X",
    url: "https://jobs.lever.co/x/7",
    locations: ["Berlin"],
    remote_type: " Hybrid ",
  });
  ...
  if (bare?.company === "EntryName")
    pass("normalizeEchojobsJob falls back to the entry name for company");
  else fail(`company fallback => ${JSON.stringify(bare?.company)}`);

  // drops: no title, non-https url, missing url
  if (normalizeEchojobsJob({ url: "https://x/1" }) === null)
    pass("normalizeEchojobsJob drops a title-less item");
  else fail("title-less item should be dropped");
  if (normalizeEchojobsJob({ title: "X", url: "http://insecure/1" }) === null)
    pass("normalizeEchojobsJob drops a non-https url");
  else fail("non-https url should be dropped");
  if (normalizeEchojobsJob({ title: "X" }) === null)
    pass("normalizeEchojobsJob drops an item with no url");
  ...
  if (
    networkCalls === 0 &&
    fetchErr &&
    /this feed is gone/i.test(fetchErr.message) &&
    /#2976/.test(fetchErr.message)
  ) {
    pass(
      "echojobs.fetch() throws a retirement message naming the cause (#2976) without touching the network"
    );
  } else {
    fail(
      `echojobs.fetch() should throw a retirement message with zero network calls, got: networkCalls=${networkCalls}, error=${fetchErr ? fetchErr.message : "(did not throw)"}`
    );
  }
} catch (e) {
