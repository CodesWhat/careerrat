...
try {
  ...
  const page2 = {
    jobs: [{ title: "Last", url: "https://jobs.ashbyhq.com/acme/last", company_name: "Acme" }],
  };
  const calls = [];
  const ctx = {
    transport: "http",
    fetchJson: async (url, options) => {
      ...
      if (options?.redirect !== "error")
        throw new Error(`fetchJson without redirect:'error': ${JSON.stringify(options)}`);
  ...
  if (jobs.length === 101)
    pass("echojobs.fetch() accumulates across pages and stops on the short page (100 + 1)");
  ...
  try {
    await echojobs.fetch(
      { name: "EchoJobs", provider: "echojobs" },
