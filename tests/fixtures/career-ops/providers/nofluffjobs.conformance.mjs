...
try {
  const nofluffjobsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/nofluffjobs.mjs")).href
  );
  ...
  if (
    parsed[0].title === "Frontend Engineer" &&
    parsed[0].url === "https://nofluffjobs.com/pl/job/frontend-engineer-remote"
  ) {
  ...
  if (
    parsed[0].company === "ExampleCo" &&
    parsed[0].location === "Remote, Kraków" &&
    parsed[0].postedAt === 1781270000000
  ) {
  ...
  if (
    fetched.length === 1 &&
    capturedUrl.startsWith("https://nofluffjobs.com/api/search/posting?")
  ) {
  ...
  if (capturedOpts && capturedOpts.method === "POST" && capturedOpts.redirect === "error") {
    ...
    fail(
      `nofluffjobs.fetch() should use POST and redirect:"error", got ${JSON.stringify(capturedOpts)}`
    );
  }
  ...
  try {
    await nfj.fetch(
      { name: "Evil", careers_url: "https://evil.example/pl" },
      { transport: "http", fetchJson: async () => fakeResponse, fetchText: async () => "" }
    );
  } catch (e) {
