...
try {
  const garenaModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/garena.mjs")).href
  );
  ...
  if (jobs[0].url === "https://careers.garena.com/global/careers/J02058270")
    pass("parseGarenaResponse builds the job URL from the default office + id");
  ...
  if (jobs[0].description === "Ships maps .")
    pass("parseGarenaResponse strips HTML from description");
  else fail(`job[0].description = ${JSON.stringify(jobs[0]?.description)}`);

  if (!("description" in jobs[1]))
    pass("parseGarenaResponse omits description when the payload has an empty string");
  ...
  if (withOffice[0].url === "https://careers.garena.com/sg/careers/J02058270")
    pass("parseGarenaResponse honors entry.garena.office in the job URL");
  ...
  if (
    traversal[0].url ===
    "https://careers.garena.com/sg%2F..%2F..%2Fevil/careers/J7%2F..%2F..%2Fadmin"
  )
    pass("parseGarenaResponse escapes separators in office and id");
  ...
  for (const bad of [".", ".."]) {
    ...
    try {
      parseGarenaResponse(
        { jobs: [{ id: "J8", title: "Dot office", tags: {} }] },
        { garena: { office: bad } }
      );
    } catch {
      rejected = true;
    ...
    try {
      parseGarenaResponse({ jobs: [{ id: bad, title: "Dot id", tags: {} }] }, {});
    } catch {
      idRejected = true;
    }
  ...
  const multiLoc = parseGarenaResponse(
    { jobs: [{ id: "J1", title: "Multi", tags: { location: ["Singapore", "Jakarta"] } }] },
    {}
  );
  if (multiLoc[0].location === "Singapore, Jakarta")
    pass('parseGarenaResponse joins multiple locations with ", "');
  ...
  if (dirty.length === 1 && dirty[0].title === "Good")
    pass("parseGarenaResponse drops title-less, id-less, and whitespace-only entries");
  else fail(`dirty = ${JSON.stringify(dirty.map((j) => j.title))}`);

  // Throws on unexpected shape (endpoint drift surfaces loudly).
  let drifted = false;
  try {
    parseGarenaResponse({ filters: {} }, {});
  } catch {
    drifted = true;
  }
  ...
  if (garena.detect({ careers_url: "https://careers.garena.com/global" }))
    pass("garena.detect() matches careers.garena.com");
  else fail("garena.detect() should match careers.garena.com");

  if (garena.detect({ api: "https://careers.garena.com/api/job/list" }))
    pass("garena.detect() matches via entry.api too");
  else fail("garena.detect() should match via entry.api");

  if (garena.detect({ careers_url: "https://evil.com/careers.garena.com" }) === null)
    pass("garena.detect() rejects path-spoofed host");
  else fail("garena.detect() should reject path-spoofed host");

  if (garena.detect({ careers_url: "https://careers.garena.com.evil.com/x" }) === null)
    pass("garena.detect() rejects suffix-spoofed host");
  else fail("garena.detect() should reject suffix-spoofed host");

  if (garena.detect({ careers_url: 42 }) === null && garena.detect({}) === null)
    pass("garena.detect() returns null for non-string / missing url");
  ...
  if (calledUrl === "https://careers.garena.com/api/job/list?office=global")
    pass("garena.fetch() posts to the API URL with the default office");
  else fail(`garena.fetch() called url = ${JSON.stringify(calledUrl)}`);

  if (
    calledOpts?.method === "POST" &&
    calledOpts?.body === "{}" &&
    calledOpts?.redirect === "error"
  ) {
  ...
  if (fetched.length === 1 && fetched[0].title === "Mocked Role")
    pass("garena.fetch() returns parsed jobs from the mock response");
  ...
  if (officeUrl === "https://careers.garena.com/api/job/list?office=sg")
    pass("garena.fetch() forwards a custom office to the API query string");
  else fail(`garena.fetch() custom-office url = ${JSON.stringify(officeUrl)}`);

  if (officeFetched[0]?.url === "https://careers.garena.com/sg/careers/J6")
    pass("garena.fetch() forwards a custom office to the job URL");
  ...
  if (escapedUrl === "https://careers.garena.com/api/job/list?office=sg%20team%26x%3D1")
    pass("garena.fetch() escapes the office in the API query string");
  ...
  if (dotRejected && !requested)
    pass("garena.fetch() rejects a dot-segment office before requesting");
