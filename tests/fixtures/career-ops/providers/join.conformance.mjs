...
try {
  const joinModule = await import(
    pathToFileURL(joinPath(ROOT, "src/core/providers/career-ops/vendor/join.mjs")).href
  );
  ...
  if (extractSlug("https://join.com/companies/acme-corp") === "acme-corp") {
    ...
    fail(
      `join.extractSlug() wrong: ${JSON.stringify(extractSlug("https://join.com/companies/acme-corp"))}`
    );
  }
  if (
    extractSlug("https://join.com.evil.com/companies/acme-corp") === null &&
    extractSlug("https://evil.com/join.com/companies/acme-corp") === null
  ) {
  ...
  if (extractSlug("not a url") === null && extractSlug(undefined) === null)
    pass("join.extractSlug() returns null for invalid/missing input");
  ...
  if (hit && hit.url === "https://join.com/companies/acme-corp")
    pass("join.detect() resolves a join.com careers_url");
  else fail(`join.detect() wrong: ${JSON.stringify(hit)}`);
  if (joinProvider.detect({ careers_url: "https://example.com/careers" }) === null)
    pass("join.detect() returns null for non-join.com URLs");
  ...
  const data = extractNextData(
    nextDataHtml([{ title: "X", idParam: "1", city: { cityName: "Berlin" } }], 1)
  );
  if (data?.props?.pageProps?.initialState?.jobs?.items?.length === 1)
    pass("join.extractNextData() parses the embedded __NEXT_DATA__ JSON");
  else fail(`join.extractNextData() wrong: ${JSON.stringify(data)}`);
  if (extractNextData("<html>no script here</html>") === null)
    pass("join.extractNextData() returns null when the script tag is absent");
  ...
  if (
    extractNextData(undefined) === null &&
    extractNextData(null) === null &&
    extractNextData(42) === null
  ) {
  ...
  const singleCtx = {
    fetchText: async () => {
      singleCalls++;
      return nextDataHtml(
        [{ title: "Senior AI Engineer", idParam: "abc123", city: { cityName: "Remote" } }],
        1
      );
    },
  };
  ...
  if (
    ...
    fail(
      `join.fetch() single-page wrong: ${JSON.stringify(singleJobs)} after ${singleCalls} calls`
    );
  }
  ...
  if (
    pagedJobs.length === 2 &&
    pagedCalls === 2 &&
    seenUrls[1] === "https://join.com/companies/acme-corp?page=2"
  ) {
  ...
  if (cappedCalls === 1 && cappedJobs.length === 1) {
    pass(
      "join.fetch() honors ctx.maxPages and stops after the first page even when pageCount is higher"
    );
  } else {
  ...
  const bigBoardCtx = {
    fetchText: async () => {
      bigBoardCalls++;
      return nextDataHtml(
        [{ title: `Job ${bigBoardCalls}`, idParam: String(bigBoardCalls), city: {} }],
        9999
      );
    },
  };
  ...
  if (bigBoardCalls === 50 && bigBoardJobs.length === 50) {
    pass(
      "join.fetch() caps pagination at DEFAULT_MAX_PAGES=50 even when pagination.pageCount reports far more"
    );
  } else {
    fail(
      `join.fetch() DEFAULT_MAX_PAGES cap wrong: calls=${bigBoardCalls}, jobs=${bigBoardJobs?.length}`
    );
  }
  ...
  const overrideCtx = {
    fetchText: async () => {
      overrideCalls++;
      return nextDataHtml(
        [{ title: `Job ${overrideCalls}`, idParam: String(overrideCalls), city: {} }],
        9999
      );
    },
  };
  const overrideEntry = {
    name: "Acme",
    careers_url: "https://join.com/companies/acme-corp",
    max_pages: 3,
  };
  ...
  if (overrideCalls === 3 && overrideJobs.length === 3) {
    ...
    fail(
      `join.fetch() entry.max_pages override wrong: calls=${overrideCalls}, jobs=${overrideJobs?.length}`
    );
  }
  ...
  if (throwErrored)
    pass("join.fetch() throws when __NEXT_DATA__ is missing or has an unexpected shape");
  ...
  const brokenPagePages = [
    nextDataHtml([{ title: "Job 1", idParam: "1", city: {} }], 2),
    "<html>no next data on page 2</html>",
  ];
  ...
  if (brokenPageErrored)
    pass(
      "join.fetch() throws when a paginated (non-first) page has missing/unexpected __NEXT_DATA__"
    );
  else
    fail(
      "join.fetch() should throw when a later page has missing/unexpected __NEXT_DATA__, not swallow it as empty"
    );
  ...
  try {
    await joinProvider.fetch(entry, {
      fetchText: async () => nextDataHtml({ not: "an array" }, 1),
    });
  } catch {
    nonArrayFirstPageErrored = true;
  }
  if (nonArrayFirstPageErrored)
    pass("join.fetch() throws when the first page's jobs.items is not an array");
  else fail("join.fetch() should throw when the first page's jobs.items is not an array");

  // fetch() — same check on a later page.
  const nonArrayPagePages = [
    nextDataHtml([{ title: "Job 1", idParam: "1", city: {} }], 2),
    nextDataHtml({ not: "an array" }, 2),
  ];
  let nonArrayPageCalls = 0;
  let nonArrayPageErrored = false;
  try {
    await joinProvider.fetch(entry, {
      fetchText: async () => nonArrayPagePages[nonArrayPageCalls++],
    });
  } catch {
    nonArrayPageErrored = true;
  }
  if (nonArrayPageErrored)
    pass("join.fetch() throws when a paginated (non-first) page's jobs.items is not an array");
  ...
  try {
    await joinProvider.fetch(
      { name: "Acme", careers_url: "https://example.com/careers" },
      {
        fetchText: async () => {
