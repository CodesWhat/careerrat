...
try {
  const flowxtraModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/flowxtra.mjs")).href
  );
  ...
  if (
    ...
    pass(
      "normalizeFlowxtraJob maps + trims title/url/company/location and converts date_share to epoch ms"
    );
  } else {
  ...
  const remoteJob = normalizeFlowxtraJob(
    {
      title: "R",
      urlJobApplay: "https://flowxtra.com/apply/R1",
      city_company: "Munich",
  ...
  if (remoteJob?.location === "Munich, Germany, Remote")
    pass('normalizeFlowxtraJob appends "Remote" when workplace is "Remote"');
  else fail(`normalizeFlowxtraJob remote location = ${JSON.stringify(remoteJob?.location)}`);

  // remote-only (no city/state/country) → "Remote".
  const remoteOnly = normalizeFlowxtraJob(
    { title: "R", urlJobApplay: "https://flowxtra.com/apply/R2", workplace: "Remote" },
    "X"
  );
  if (remoteOnly?.location === "Remote")
    pass(
      'normalizeFlowxtraJob yields "Remote" when workplace is "Remote" and no location fields are set'
    );
  ...
  const onSite = normalizeFlowxtraJob(
    {
      title: "O",
      urlJobApplay: "https://flowxtra.com/apply/O1",
      city_company: "Vienna",
      workplace: "On-site",
    },
    "X"
  );
  if (onSite?.location === "Vienna")
    pass('normalizeFlowxtraJob does not append "Remote" for non-remote workplace values');
  else fail(`normalizeFlowxtraJob on-site location = ${JSON.stringify(onSite?.location)}`);

  // company fallbacks: entry name, then "Flowxtra".
  const coFromEntry = normalizeFlowxtraJob(
    { title: "T", urlJobApplay: "https://flowxtra.com/apply/C1", name_company: "" },
    "Entry Name"
  );
  const coDefault = normalizeFlowxtraJob({
    title: "T",
    urlJobApplay: "https://flowxtra.com/apply/C2",
  });
  if (coFromEntry?.company === "Entry Name" && coDefault?.company === "Flowxtra") {
    ...
    fail(
      `normalizeFlowxtraJob company fallbacks = ${JSON.stringify({ a: coFromEntry?.company, b: coDefault?.company })}`
    );
  }
  ...
  if (drops.every((r) => r === null))
    pass(
      "normalizeFlowxtraJob drops empty-title / no-url / non-https / off-host / relative / non-object"
    );
  else fail(`normalizeFlowxtraJob drops = ${JSON.stringify(drops)}`);

  // missing date_share → no postedAt key.
  const noDate = normalizeFlowxtraJob({
    title: "T",
    urlJobApplay: "https://flowxtra.com/apply/ND",
  });
  if (noDate && !("postedAt" in noDate))
    pass("normalizeFlowxtraJob omits postedAt when date_share is absent");
  ...
  const pagedFetch = async (url, opts) => {
    ...
    if (page === 1)
      return {
        success: true,
        data: { data: page1, next_page_url: "https://app.flowxtra.com/api/central/jobs?page=2" },
      };
  ...
  if (
    requested.length === 2 &&
    requested[0].url ===
      "https://app.flowxtra.com/api/central/jobs?status=Live&per_page=100&page=1" &&
    requested[1].url === "https://app.flowxtra.com/api/central/jobs?status=Live&per_page=100&page=2"
  ) {
    pass(
      "flowxtra.fetch() builds ?status=Live&per_page=100&page=N URLs and stops after next_page_url is null"
    );
  } else {
  ...
  if (requested.every((r) => r.redirect === "error"))
    pass('flowxtra.fetch() passes redirect:"error" on every page (SSRF guard)');
  else fail(`flowxtra.fetch() redirect opts = ${JSON.stringify(requested.map((r) => r.redirect))}`);

  if (paged.length === 102)
    pass(
      "flowxtra.fetch() aggregates valid jobs across pages (100 + 2, dropping the empty-title row)"
    );
    ...
    { name: "Flowxtra", max_pages: 1 },
    {
      fetchJson: async (url) => {
        ...
        return {
          success: true,
          data: {
            data: Array.from({ length: 100 }, (_, i) => mk(i)),
            next_page_url: "https://app.flowxtra.com/api/central/jobs?page=2",
          },
        };
      },
    }
  ...
  if (
    capRequested.length === 1 &&
    capRequested[0] === "https://app.flowxtra.com/api/central/jobs?status=Live&per_page=100&page=1"
  ) {
