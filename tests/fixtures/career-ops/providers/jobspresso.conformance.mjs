...
try {
  const { default: jobspresso, parseJobspressoFeed } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/jobspresso.mjs")).href
  );
  ...
  if (
    jobspresso.detect({ provider: "jobspresso" })?.url === "https://jobspresso.co/?feed=job_feed"
  ) {
  ...
  const fetched = await jobspresso.fetch(
    {},
    {
      async fetchText(url, opts) {
        ...
        if (url !== "https://jobspresso.co/?feed=job_feed" || opts?.redirect !== "error") {
