...
try {
  const alibaba = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/alibaba.mjs")).href)
  ).default;
  const { parseAlibabaResponse } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/alibaba.mjs")).href
  );

  if (alibaba.id === "alibaba") pass('alibaba.id is "alibaba"');
  else fail(`alibaba.id is ${JSON.stringify(alibaba.id)}`);

  const hit = alibaba.detect({
    name: "阿里巴巴",
  ...
  if (
    alibaba.detect({ name: "X", careers_url: "https://evil.example/talent.alibaba.com" }) === null
  ) {
  ...
  if (
    alibaba.detect({
      name: "X",
      careers_url: "http://talent.alibaba.com/off-campus/position-list",
    }) === null
  ) {
  ...
  if (jobs.length === 2)
    pass("parseAlibabaResponse() keeps titled+ID posts, drops incomplete ones");
  else fail(`parseAlibabaResponse() returned ${jobs.length} jobs, expected 2`);

  const j1 = jobs[0];
  if (
    j1 &&
    j1.url === "https://talent.alibaba.com/off-campus/position-detail?positionId=100009135010" &&
    j1.location === "杭州/北京"
  ) {
    pass(
      "parseAlibabaResponse() builds a track_id-free detail URL from the id and joins workLocations"
    );
  } else {
  ...
  if (
    j1 &&
    j1.description.includes("类别: 技术类-开发") &&
    j1.description.includes("经验: 3-5年") &&
    j1.description.includes("负责大模型后训练服务。")
  ) {
    pass(
      "parseAlibabaResponse() packs categories/experience/description/requirement into description"
    );
  } else {
  ...
  const empty = parseAlibabaResponse(
    { success: true, content: { totalCount: 0, datas: null } },
    "阿里巴巴"
  );
  ...
  const pagedJobs = await alibaba.fetch(
    { name: "阿里巴巴", careers_url: ALI_URL, keywords: ["AI"] },
    paged.ctx
  );
  ...
  if (paged.sleeps.length === 1 && paged.sleeps[0] > 0) {
    pass(
      "alibaba.fetch() paces follow-up requests via ctx.sleep (no delay before the first request)"
    );
  } else {
  ...
  const overlapJobs = await alibaba.fetch(
    { name: "阿里巴巴", careers_url: ALI_URL, keywords: ["AI", "大模型"] },
    overlap.ctx
  );
  if (overlapJobs.length === 1 && overlap.calls.length === 2 && overlap.sleeps.length === 1) {
    ...
    fail(
      `alibaba.fetch() cross-keyword: ${overlapJobs.length} jobs, ${overlap.calls.length} requests, sleeps ${JSON.stringify(overlap.sleeps)}`
    );
  }
  ...
  const capped = mkCtx(() => ({
    ...
    content: {
      totalCount: 500,
      datas: Array.from({ length: 100 }, (_, i) => mkJob(5000 + i, `岗位E${i}`)),
    },
  }));
  await alibaba.fetch(
    { name: "阿里巴巴", careers_url: ALI_URL, keywords: ["AI"], max_pages: 1 },
    capped.ctx
  );
  ...
  const probe = mkCtx(() => ({
    ...
    content: {
      totalCount: 500,
      datas: Array.from({ length: 100 }, (_, i) => mkJob(6000 + i, `岗位F${i}`)),
    },
  }));
  ...
  if (probe.calls.length === 1 && probe.calls[0].key === "" && probeJobs.length === 100) {
    pass(
      "alibaba.fetch() honors the ctx.maxPages probe hint and defaults to a whole-board (empty keyword) query"
    );
  } else {
    fail(
      `alibaba.fetch() ctx.maxPages=1: ${probe.calls.length} requests, key=${JSON.stringify(probe.calls[0] && probe.calls[0].key)}`
    );
  }
  ...
  const blipJobs = await alibaba.fetch(
    { name: "阿里巴巴", careers_url: ALI_URL, keywords: ["AI", "大模型"] },
    blip.ctx
  );
  if (blipJobs.length === 1 && blipJobs[0].title === "幸存岗位") {
    ...
    fail(
      `alibaba.fetch() partial results on failure: ${JSON.stringify(blipJobs.map((j) => j.title))}`
    );
  }

  const softFail = mkCtx(({ key }) =>
    key === "大模型"
      ? { success: false, errorMsg: "rate limited" }
      : { success: true, content: { totalCount: 1, datas: [mkJob(8, "幸存岗位2")] } }
  );
  const softFailJobs = await alibaba.fetch(
    { name: "阿里巴巴", careers_url: ALI_URL, keywords: ["AI", "大模型"] },
    softFail.ctx
  );
  ...
  if (softDeadThrew) {
    pass(
      "alibaba.fetch() throws when the very first response reports success:false (dead board, not empty board)"
    );
  } else {
  ...
  if (firstFailThrew) {
    pass(
      "alibaba.fetch() still throws when the very first request fails (dead board reads as failure)"
    );
  } else {
