...
try {
  const ibmModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/ibm.mjs")).href
  );
  ...
  if (countryTerm?.term.field_keyword_05 === "Germany" && catTerm?.bool.should.length === 2) {
    pass(
      "buildPostFilter maps country → field_keyword_05 and categories → field_keyword_08 should[]"
    );
  } else {
      ...
      hits: [
        ...
        },
        {
          _source: {
            ...
            url: "https://ibm.com/careers/2",
            field_keyword_19: "Remote",
          },
        },
      ],
    },
  ...
  if (jobs.length === 2 && jobs[0].company === "IBM")
    pass('parseIbmResponse extracts 2 jobs with company "IBM"');
  else fail(`parseIbmResponse returned ${JSON.stringify(jobs)}`);

  if (jobs[0].location === "Berlin, Germany · Hybrid")
    pass("parseIbmResponse joins location · work mode");
  else fail(`row 0 location = ${JSON.stringify(jobs[0]?.location)}`);

  if (jobs[1].location === "Remote")
    pass("parseIbmResponse omits the separator when work mode is absent");
  ...
  if (dirty.length === 1 && dirty[0].title === "Good")
    pass("parseIbmResponse drops title-less, url-less, and non-http(s) entries");
  ...
  if (calls === 2 && fetched.length === 32)
    pass("ibm.fetch() paginates and stops on the first short page");
