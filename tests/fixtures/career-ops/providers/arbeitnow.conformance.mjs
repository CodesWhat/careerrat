...
try {
  const arbeitnowModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/arbeitnow.mjs")).href
  );
  ...
  if (
    ...
    pass(
      "normalizeArbeitnowJob maps + trims title/url/company/location and converts created_at seconds → ms"
    );
  } else {
  ...
  const remoteJob = normalizeArbeitnowJob(
    { title: "R", url: "https://www.arbeitnow.com/jobs/r", location: "Munich", remote: true },
    "X"
  );
  if (remoteJob?.location === "Munich, Remote")
    pass('normalizeArbeitnowJob appends "Remote" when remote is true');
  else fail(`normalizeArbeitnowJob remote location = ${JSON.stringify(remoteJob?.location)}`);

  // remote-only (no base location) → "Remote".
  const remoteOnly = normalizeArbeitnowJob(
    { title: "R", url: "https://www.arbeitnow.com/jobs/r2", remote: true },
    "X"
  );
  if (remoteOnly?.location === "Remote")
    pass('normalizeArbeitnowJob yields "Remote" when remote is true and location is absent');
  ...
  const coFromEntry = normalizeArbeitnowJob(
    { title: "T", url: "https://www.arbeitnow.com/jobs/c1", company_name: "" },
    "Entry Name"
  );
  ...
  if (coFromEntry?.company === "Entry Name" && coDefault?.company === "Arbeitnow") {
    ...
    fail(
      `normalizeArbeitnowJob company fallbacks = ${JSON.stringify({ a: coFromEntry?.company, b: coDefault?.company })}`
    );
  }
  ...
  if (drops.every((r) => r === null))
    pass(
      "normalizeArbeitnowJob drops empty-title / no-url / non-https / relative / off-host / non-object"
    );
  ...
  if (noDate && !("postedAt" in noDate))
    pass("normalizeArbeitnowJob omits postedAt when created_at is absent");
  ...
  const mk = (i) => ({
    title: `Role ${i}`,
    url: `https://www.arbeitnow.com/jobs/x${i}`,
    company_name: `Co ${i}`,
    location: "Berlin",
    remote: false,
    created_at: 1782693032 + i,
  });
  ...
  const pagedFetch = async (url, opts) => {
    ...
    if (page === 1)
      return {
        data: page1,
        links: { next: "https://www.arbeitnow.com/api/job-board-api?search=foo&page=2" },
        meta: {},
      };
  ...
  if (
    ...
    pass(
      "arbeitnow.fetch() builds ?page=N URLs itself (ignores links.next) and stops after the short page"
    );
  } else {
  ...
  if (requested.every((r) => r.redirect === "error"))
    pass('arbeitnow.fetch() passes redirect:"error" on every page (SSRF guard)');
  else
    fail(`arbeitnow.fetch() redirect opts = ${JSON.stringify(requested.map((r) => r.redirect))}`);

  if (paged.length === 102)
    pass(
      "arbeitnow.fetch() aggregates valid jobs across pages (100 + 2, dropping the empty-title row)"
    );
  ...
  await arbeitnow.fetch(
    { name: "Arbeitnow", max_pages: 1 },
    {
      fetchJson: async (url, opts) => {
        ...
        return { data: Array.from({ length: 100 }, (_, i) => mk(i)) };
      },
    }
  );
  if (
    capRequested.length === 1 &&
    capRequested[0] === "https://www.arbeitnow.com/api/job-board-api?page=1"
  ) {
