...
try {
  const ccj = (
    await import(
      pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/cryptocurrencyjobs.mjs")).href
    )
  ).default;
  const { parseCryptocurrencyJobsRss, splitTitle } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/cryptocurrencyjobs.mjs")).href
  );
  ...
  if (
    splitTitle("Senior Backend Engineer at Acme Protocol").company === "Acme Protocol" &&
    splitTitle("Role with no separator").company === ""
  ) {
    pass('splitTitle splits on the last " at " and handles the no-separator case');
  } else {
    fail(
      `splitTitle wrong: ${JSON.stringify(splitTitle("Senior Backend Engineer at Acme Protocol"))}`
    );
  }
  ...
  if (
    jobs[0]?.description === "We are hiring a backend engineer. 100% remote." &&
    !("description" in (jobs[1] || {}))
  ) {
    pass("parseCryptocurrencyJobsRss populates description when present and omits it when absent");
  } else {
    fail(
      `parseCryptocurrencyJobsRss description handling = ${JSON.stringify(jobs[0]?.description)}`
    );
  }

  if (
    parseCryptocurrencyJobsRss("").length === 0 &&
    parseCryptocurrencyJobsRss(null).length === 0
  ) {
  ...
  if (capturedOpts && capturedOpts.redirect === "error")
    pass('cryptocurrencyjobs.fetch() passes redirect:"error" (SSRF hardening)');
  else
    fail(
      `cryptocurrencyjobs.fetch() should pass redirect:"error", got ${JSON.stringify(capturedOpts)}`
    );
  ...
  if (threw?.message === "network down")
    pass("cryptocurrencyjobs.fetch() throws when the feed fetch fails (dead board ≠ empty board)");
