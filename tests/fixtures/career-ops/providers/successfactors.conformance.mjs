...
try {
  const successfactorsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/successfactors.mjs")).href
  );
  ...
  if (
    single.base === "https://jobs.zf.com" &&
    single.tileApi === "https://jobs.zf.com/tile-search-results/"
  ) {
  ...
  const brandSearch = resolveConfig({
    name: "Bluebeam",
    api: "https://careers.nemetschek.com/Bluebeam/search/",
  });
  if (
    ...
    pass(
      "resolveConfig: multi-brand tenant keeps the brand path in tileApi/jobsApi/searchPage (#2010)"
    );
  } else {
  ...
  const brandNoSearch = resolveConfig({
    name: "Bluebeam",
    careers_url: "https://careers.nemetschek.com/Bluebeam/",
  });
  if (brandNoSearch.base === "https://careers.nemetschek.com/Bluebeam") {
    pass(
      "resolveConfig: a brand path without a trailing /search/ segment still resolves correctly (#2010)"
    );
  } else {
  ...
  // tile-search-results/).
  const brandTileEndpoint = resolveConfig({
    name: "Bluebeam",
    api: "https://careers.nemetschek.com/Bluebeam/tile-search-results/",
  });
  if (
    brandTileEndpoint.base === "https://careers.nemetschek.com/Bluebeam" &&
    brandTileEndpoint.tileApi === "https://careers.nemetschek.com/Bluebeam/tile-search-results/"
  ) {
    pass(
      "resolveConfig: api: pointing at tile-search-results/ directly does not double the segment (#2010)"
    );
  } else {
    fail(
      `resolveConfig tile-endpoint-as-api wrong: base=${brandTileEndpoint.base} tileApi=${brandTileEndpoint.tileApi}`
    );
  }
  const brandJobsEndpoint = resolveConfig({
    name: "Bluebeam",
    api: "https://careers.nemetschek.com/Bluebeam/services/recruiting/v1/jobs",
  });
  if (
    brandJobsEndpoint.base === "https://careers.nemetschek.com/Bluebeam" &&
    brandJobsEndpoint.jobsApi ===
      "https://careers.nemetschek.com/Bluebeam/services/recruiting/v1/jobs"
  ) {
    pass(
      "resolveConfig: api: pointing at services/recruiting/v1/jobs directly does not double the segment (#2010)"
    );
  } else {
    fail(
      `resolveConfig jobs-endpoint-as-api wrong: base=${brandJobsEndpoint.base} jobsApi=${brandJobsEndpoint.jobsApi}`
    );
  }
  ...
  if (
    cityFromSlug(
      "/job/Hyderabad-Specialist-Low-Level-Driver-Development-TG-500032/1399717233/",
      "Specialist -Low Level Driver Development"
    ) === "Hyderabad"
  ) {
    pass("cityFromSlug extracts a single-word city");
  } else {
    fail(
      `cityFromSlug single-word wrong: ${cityFromSlug("/job/Hyderabad-Specialist-Low-Level-Driver-Development-TG-500032/1399717233/", "Specialist -Low Level Driver Development")}`
    );
  }
  ...
  if (
    cityFromSlug(
      "/job/Levallois-Perret-Data-Management-Engagement-Architect-92300/1400945133/",
      "Data Management Engagement Architect"
    ) === "Levallois Perret"
  ) {
    pass("cityFromSlug extracts a multi-word city");
  } else {
    fail(
      `cityFromSlug multi-word wrong: ${cityFromSlug("/job/Levallois-Perret-Data-Management-Engagement-Architect-92300/1400945133/", "Data Management Engagement Architect")}`
    );
  }
  // Accented title (Ingénieur) — unicode word matching keeps the anchor intact.
  if (
    cityFromSlug(
      "/job/Massy-Ing%C3%A9nieur-Commercial-91743/1351400755/",
    ...
    fail(
      `cityFromSlug accented wrong: ${cityFromSlug("/job/Massy-Ing%C3%A9nieur-Commercial-91743/1351400755/", "Ingénieur Commercial")}`
    );
  }
  ...
  if (j1 && j1.location === "Schweinfurt")
    pass('parseTiles reads the city-value div, not the "City" label');
  else fail(`parseTiles city wrong: ${JSON.stringify(j1 && j1.location)}`);
  if (j1 && j1.url === "https://jobs.example.com/job/Schweinfurt-Ferienarbeiter-97421/111/")
    pass("parseTiles builds an absolute URL from data-url");
  else fail(`parseTiles url wrong: ${JSON.stringify(j1 && j1.url)}`);

  const j2 = parsed.find((j) => j.url.includes("/222/"));
  if (
    j2 &&
    j2.url === "https://jobs.example.com/job/Palo-Alto-Program-&-Release-Manager-CA-94304/222/"
  ) {
    pass("parseTiles decodes &amp; in the data-url path");
  } else {
    fail(`parseTiles &amp; url wrong: ${JSON.stringify(j2 && j2.url)}`);
  }
  if (j2 && j2.location === "Palo Alto")
    pass("parseTiles falls back to slug city when no city div is present");
  ...
  if (parseTiles("<!DOCTYPE html>", jobBase).length === 0)
    pass("parseTiles returns [] for an empty fragment");
  ...
  if (extractLocales("<p>no locales here</p>").length === 0)
    pass("extractLocales returns [] when the page carries none");
  ...
  if (parseCsbDate("20.11.23") === Date.UTC(2023, 10, 20))
    pass("parseCsbDate reads European D.M.YY (dots)");
  else fail(`parseCsbDate DE wrong: ${parseCsbDate("20.11.23")}`);
  if (
    parseCsbDate("garbage") === undefined &&
    parseCsbDate("13/40/99") === undefined &&
    parseCsbDate("") === undefined
  ) {
  ...
  if (cleanCsbLocation(["Karlovy Vary, CZE, 36004<br/>"]) === "Karlovy Vary, CZE, 36004")
    pass("cleanCsbLocation strips trailing <br/>");
  else
    fail(
      `cleanCsbLocation single wrong: ${JSON.stringify(cleanCsbLocation(["Karlovy Vary, CZE, 36004<br/>"]))}`
    );
  if (cleanCsbLocation(["Munich<br/>", "Berlin<br/>"]) === "Munich / Berlin")
    pass('cleanCsbLocation joins multiple locations with " / "');
  else
    fail(
      `cleanCsbLocation multi wrong: ${JSON.stringify(cleanCsbLocation(["Munich<br/>", "Berlin<br/>"]))}`
    );
  if (cleanCsbLocation(undefined) === "" && cleanCsbLocation([]) === "")
    pass("cleanCsbLocation tolerates missing/empty location");
    ...
    jobSearchResult: [
      {
        response: {
          id: "31099",
          unifiedStandardTitle: "Analytical Lab Technician",
          unifiedUrlTitle: "Analytical-Lab-Technician",
          jobLocationShort: ["Anyang, KOR, 14058<br/>"],
          unifiedStandardStart: "6/18/26",
        },
      },
      {
        response: {
          id: "1283",
          unifiedStandardTitle: "Senior Expert Mergers & Acquisitions (m/f/d)",
          unifiedUrlTitle: "Senior-Expert-Mergers-&amp;-Acquisitions-%28mfd%29",
          jobLocationShort: ["Munich<br/>"],
          unifiedStandardStart: "4/21/26",
        },
      },
  ...
  const c1 = csbJobs[0];
  if (c1 && c1.url === "https://jobs.example.com/job/Analytical-Lab-Technician/31099-en_US")
    pass("parseCsbJobs builds {origin}/job/{slug}/{id}-{locale}");
  ...
  if (c1 && c1.postedAt === Date.UTC(2026, 5, 18))
    pass("parseCsbJobs sets postedAt from unifiedStandardStart");
  else fail(`parseCsbJobs postedAt wrong: ${JSON.stringify(c1 && c1.postedAt)}`);
  const c2 = csbJobs[1];
  if (c2 && !/[?#&]|&amp;/.test(new URL(c2.url).pathname))
    pass("parseCsbJobs sanitizes &amp; / URL-structural chars out of the slug");
  ...
  const csbBrandCfg = {
    ...
    base: "https://careers.example.com/Acme",
  };
  const csbBrandJobs = parseCsbJobs(csbJson, csbBrandCfg, "en_US");
  if (
    csbBrandJobs[0]?.url ===
    "https://careers.example.com/Acme/job/Analytical-Lab-Technician/31099-en_US"
  ) {
  ...
  // still decoding valid ones (&amp;).
  {
    ...
    let badTiles,
      badThrew = null;
    try {
      badTiles = parseTiles(badFrag2, "https://jobs.example.com");
    } catch (e) {
      badThrew = e;
    }
    if (badThrew)
      fail(
        `successfactors.parseTiles() threw ${badThrew.name} on an out-of-range numeric entity (unguarded String.fromCodePoint): ${badThrew.message}`
      );
    else if (
      badTiles.length === 1 &&
      badTiles[0].title === "Overflow &#99999999; & Hex &#xFFFFFFFF; Surrogate &#xD800;"
    )
      pass(
        "successfactors.parseTiles() tolerates out-of-range / surrogate entities, degrading them to literal text while still decoding &amp; (no RangeError crash)"
      );
  ...
  // record a failure instead of "live but empty".
  {
    ...
    try {
      await sf.fetch(
        { name: "DeadCo", careers_url: "https://careers.deadco.example", sfVariant: "csb" },
        {
          ...
          }, // locale discovery is best-effort
          ...
          fetchJson: async () => {
            csbCalls++;
            throw new Error(`jobs api down (call ${csbCalls})`);
          },
        }
      );
    } catch (err) {
    ...
    if (csbErr?.message === "jobs api down (call 1)" && csbCalls >= 2) {
      pass(
        "successfactors CSB fetch() throws the FIRST locale failure when every locale fails (dead board ≠ empty board, deterministic error)"
      );
    } else {
      fail(
        `successfactors CSB fetch() wrong: ${csbErr ? `threw "${csbErr.message}"` : "swallowed an all-locales failure into []"} after ${csbCalls} jobs-API calls`
      );
    }
  }
  ...
  // and legitimately empty, not a dead slug.
  {
    ...
    try {
      probeJobs = await sf.fetch(
        { name: "EmptyCo", careers_url: "https://jobs.emptyco.example" },
        {
          ...
          fetchJson: async () => {
            probeCalls++;
            throw new Error("no CSB endpoint on this tenant");
          },
        }
      );
    } catch (err) {
    ...
    if (!probeErr && Array.isArray(probeJobs) && probeJobs.length === 0 && probeCalls >= 1) {
      pass(
        "successfactors fetch() without sfVariant: empty RMK + failing CSB probe returns [] (healthy empty board ≠ dead slug)"
      );
    } else {
      fail(
        `successfactors fetch() empty-RMK CSB-probe wrong: ${probeErr ? `threw "${probeErr.message}"` : JSON.stringify(probeJobs)} after ${probeCalls} jobs-API calls`
      );
    }
  }
} catch (err) {
