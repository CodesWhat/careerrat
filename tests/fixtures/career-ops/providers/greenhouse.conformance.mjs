...
try {
  const greenhouseModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/greenhouse.mjs")).href
  );
  ...
  const hitEu = greenhouse.detect({
    name: "EuCo",
    careers_url: "https://job-boards.eu.greenhouse.io/euco",
  });
  ...
  if (
    greenhouse.detect({ name: "Evil", api: "https://evil.example/v1/boards/acme/jobs" }) === null
  ) {
  ...
  if (
    capturedUrl === "https://boards-api.greenhouse.io/v1/boards/acme/jobs?content=true" &&
    capturedOpts?.redirect === "error"
  ) {
    pass(
      'greenhouse.fetch() hits the derived boards-api URL with content=true and redirect:"error" (SSRF guard)'
    );
  } else {
    fail(
      `greenhouse.fetch() url=${JSON.stringify(capturedUrl)} opts=${JSON.stringify(capturedOpts)}`
    );
  }
  ...
  if (
    fetched[0]?.title === "Senior Backend Engineer" &&
  ...
  if (fetched[1]?.title === "" && fetched[1]?.location === "" && fetched[1]?.postedAt === undefined)
    pass(
      'greenhouse.fetch() defaults missing title/location to "" and omits postedAt when first_published is absent'
    );
  else fail(`greenhouse.fetch() row 1 = ${JSON.stringify(fetched[1])}`);

  if (fetched[2]?.postedAt === undefined)
    pass(
      "greenhouse.fetch() yields undefined postedAt for an unparseable first_published (NaN-safe)"
    );
    ...
    jobs: [
      {
        ...
        content:
          "&lt;div&gt;&lt;script&gt;tracking()&lt;/script&gt;&lt;style&gt;.x{}&lt;/style&gt;&lt;h2&gt;About &amp;amp; Us&lt;/h2&gt;&lt;p&gt;We offer &#39;visa sponsorship&#39; for &lt;b&gt;H-1B&lt;/b&gt; roles.&lt;/p&gt;&lt;/div&gt;",
      },
      {
        id: 302,
        title: "No Body",
  ...
  if (!("description" in (withBodies[1] || {})) && !("description" in (withBodies[2] || {})))
    ...
    fail(
      `greenhouse.fetch() description keys = ${JSON.stringify(withBodies.map((j) => "description" in j))}`
    );
        ...
        jobs: [
          {
            ...
            first_published: "1970-01-01T00:00:00.000Z",
          },
        ],
      }),
    }
  ...
  for (const body of emptyCases) {
    ...
    if (!Array.isArray(out) || out.length !== 0) {
      emptyOk = false;
      fail(`greenhouse.fetch() body=${JSON.stringify(body)} → ${JSON.stringify(out)}`);
      break;
    }
  }
  ...
  try {
    ...
    if (!untrustedFetchCalled && /untrusted hostname/.test(e.message)) {
      ...
      fail(
        `greenhouse.fetch() untrusted api: fetchCalled=${untrustedFetchCalled}, error=${e.message}`
      );
    }
  }
  ...
  if (
    isWorkModelOnly("Hybrid") &&
    isWorkModelOnly("In-Office") &&
    isWorkModelOnly("Distributed") &&
    isWorkModelOnly("Distributed; Hybrid") &&
    isWorkModelOnly(" remote ")
  )
  ...
  if (
    !isWorkModelOnly("Hybrid - London") &&
    !isWorkModelOnly("Remote (Canada)") &&
    !isWorkModelOnly("Austin, TX") &&
    !isWorkModelOnly("") &&
    !isWorkModelOnly(null)
  )
    pass(
      "isWorkModelOnly() leaves locations that already carry geography (and empty/non-string) alone"
    );
  ...
  if (
    ...
    fail(
      `officesUrlFor() = ${JSON.stringify(officesUrlFor("https://boards-api.greenhouse.io/v1/boards/acme/jobs"))}`
    );
  ...
  if (
    officeMap.get(201)?.has("Austin, TX") &&
    officeMap.get(202)?.size === 2 &&
    officeMap.get(202)?.has("Seattle, WA") &&
    !officeMap.has(203)
  )
    pass(
      "buildOfficeMap() walks nested offices/departments, unions multi-office jobs, skips unnamed"
    );
          ...
          jobs: [
            {
              ...
              absolute_url: "https://job-boards.greenhouse.io/cloudflare/jobs/201",
              location: { name: "In-Office" },
            },
            {
              id: 202,
              title: "Staff SWE",
              absolute_url: "https://job-boards.greenhouse.io/cloudflare/jobs/202",
              location: { name: "Distributed; Hybrid" },
            },
            {
              id: 999,
              title: "Unmapped",
              absolute_url: "https://job-boards.greenhouse.io/cloudflare/jobs/999",
  ...
  if (
    requested.length === 2 &&
    requested[1] === "https://boards-api.greenhouse.io/v1/boards/cloudflare/offices"
  )
  ...
  if (enriched[0]?.location === "In-Office · Austin, TX")
    ...
    fail(`greenhouse.fetch() enriched row 0 location = ${JSON.stringify(enriched[0]?.location)}`);
  ...
  if (enriched[1]?.location === "Distributed; Hybrid · Austin, TX · Seattle, WA")
    ...
    fail(`greenhouse.fetch() enriched row 1 location = ${JSON.stringify(enriched[1]?.location)}`);

  if (enriched[2]?.location === "Hybrid")
    pass("greenhouse.fetch() leaves a job absent from /offices on its bare work-model string");
  else
    fail(`greenhouse.fetch() enriched row 2 location = ${JSON.stringify(enriched[2]?.location)}`);
  ...
  if (reversed?.location === "Distributed; Hybrid · Austin, TX · Seattle, WA")
    pass(
      "greenhouse.fetch() sorts the folded offices, so /offices ordering cannot rewrite the location"
    );
  else
    fail(
      `greenhouse.fetch() office order leaked into the location: ${JSON.stringify(reversed?.location)}`
    );
          ...
          jobs: [
            {
              id: 1,
              title: "SRE",
              absolute_url: "https://job-boards.greenhouse.io/datadog/jobs/1",
  ...
  if (geoRequests.length === 1)
    ...
    fail(
      `greenhouse.fetch() made ${geoRequests.length} requests for a geo-location board (expected 1)`
    );
  ...
  if (degradedWarnings.some((w) => /greenhouse: Cloudflare .*404/.test(String(w))))
    ...
    fail(
      `greenhouse.fetch() should warn on failed enrichment, got: ${JSON.stringify(degradedWarnings)}`
    );
  ...
  for (const thrown of [null, undefined, "plain string", 42]) {
    ...
    if (
      !threw &&
      nonErrRows?.length === 1 &&
      nonErrRows[0]?.location === "In-Office" &&
      nonErrWarnings.length === 1
    )
      pass(`greenhouse.fetch() survives a non-Error /offices rejection (${label}) and still warns`);
    else
      fail(
        `greenhouse.fetch() non-Error rejection (${label}): threw=${threw?.message}, rows=${JSON.stringify(nonErrRows)}, warnings=${JSON.stringify(nonErrWarnings)}`
      );
  }
