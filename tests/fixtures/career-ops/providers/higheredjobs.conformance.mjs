...
try {
  const hejModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/higheredjobs.mjs")).href
  );
  ...
  if (
    hitDefault &&
    hitDefault.url === "https://www.higheredjobs.com/rss/categoryFeed.cfm?catID=68"
  ) {
...
<rss version="2.0">
  ...
  if (jobs.length === 2)
    pass("parseHigherEdJobsFeed keeps 2 items (drops missing/relative/off-host links)");
  ...
  if (
    jobs[0]?.title === "Director of AI Strategy" &&
    jobs[0]?.company === "Curry College" &&
    jobs[0]?.location === "Milton, MA"
  ) {
    pass(
      'parseHigherEdJobsFeed parses title + "Institution (City, ST)" description -> company/title/location'
    );
  } else {
  ...
  if (
    jobs[1]?.company === "State University System Office" &&
    jobs[1]?.location === "" &&
    jobs[1]?.title === "Dean of Engineering & Computing"
  ) {
    pass(
      "parseHigherEdJobsFeed falls back to whole description as company when no parens (empty location)"
    );
  } else {
  ...
  if (
    parseHigherEdJobsFeed("", "X").length === 0 &&
    parseHigherEdJobsFeed(null, "X").length === 0
  ) {
  ...
  if (
    fetched[0]?.company === "Curry College" &&
    fetched[0]?.title === "Director of AI Strategy" &&
    fetched[0]?.location === "Milton, MA"
  ) {
