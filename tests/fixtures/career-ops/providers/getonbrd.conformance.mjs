...
try {
  const getonbrdModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/getonbrd.mjs")).href
  );
  ...
  if (
    capturedUrl ===
    "https://www.getonbrd.com/api/v0/categories/programming/jobs?per_page=100&expand[]=company&page=1"
  )
  ...
  if (fetched[0] && Object.keys(fetched[0]).sort().join(",") === "company,location,title,url")
    ...
    fail(`getonbrd.fetch() row 0 keys = ${JSON.stringify(fetched[0] && Object.keys(fetched[0]))}`);

  if (
    fetched[0]?.title === "Staff AI Engineer" &&
  ...
  const noName = await getonbrd.fetch(
    {},
    {
      fetchJson: async () => ({
        data: [
          {
            attributes: { title: "Role", remote: true },
  ...
  if (noName[0]?.company === "Get on Board")
    pass(
      'getonbrd.fetch() defaults company to "Get on Board" when name and entry.name are both missing'
    );
  ...
  const mkJob = (i) => ({
    attributes: { title: `Role ${i}`, remote: true },
    links: { public_url: `https://www.getonbrd.com/jobs/role-${i}` },
  });
  ...
  const paged = await getonbrd.fetch(
    { name: "GOB", provider: "getonbrd" },
    {
      fetchJson: async (url, opts) => {
        ...
        return pageCalls.length === 1 ? fullPage : shortPage;
      },
    }
  );
  ...
  if (
    pageCalls.length === 2 &&
    /[?&]page=1(?:&|$)/.test(pageUrls[0]) &&
    /[?&]page=2(?:&|$)/.test(pageUrls[1])
  )
  ...
  if (paged.length === 101) pass("getonbrd.fetch() accumulates jobs across pages (100 + 1)");
