...
try {
  const joinup = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/joinup.mjs")).href)
  ).default;
  ...
  if (
    joinup.detect({ name: "J", careers_url: "https://joinup.ch/browse/jobs" })?.url ===
    "https://joinup.ch/browse/jobs"
  ) {
  ...
  if (
    joinup.detect({ name: "Spoof", careers_url: "https://evil.example/joinup.ch" }) === null &&
  ...
  let joinupOpts = null;
  const joinupJobs = await joinup.fetch(
    { name: "J", careers_url: "https://joinup.ch/browse/jobs" },
    {
      fetchText: async (_url, opts) => {
        joinupOpts = opts;
        return `<script id="__NEXT_DATA__" type="application/json">${nextData}</script>`;
      },
    }
  );
  ...
  if (
    joinupJobs.length === 1 &&
    joinupJobs[0].url === "https://joinup.ch/job/acme-hof" &&
    joinupJobs[0].company === "Acme"
  ) {
  ...
  if (joinupJobs[0]?.postedAt === Date.parse("2026-01-02"))
    pass("joinup.fetch() maps an ISO `created` to postedAt");
  ...
  try {
    await joinup.fetch(
      { name: "J", careers_url: "https://joinup.ch/browse/jobs" },
      { fetchText: async () => "<html>no data here</html>" }
    );
  } catch (e) {
    joinupThrew = /__NEXT_DATA__/.test(e.message);
  }
  ...
  try {
    ...
    joinupBadJson = /__NEXT_DATA__/.test(e.message);
  }
  ...
  const emptyNext = JSON.stringify({
    props: {
      pageProps: { serverState: { initialResults: { jobs: { results: [{ hits: [] }] } } } },
    },
  });
  const joinupEmpty = await joinup.fetch(
    { name: "J", careers_url: "https://joinup.ch/browse/jobs" },
    {
      fetchText: async () =>
        `<script id="__NEXT_DATA__" type="application/json">${emptyNext}</script>`,
    }
  );
  if (Array.isArray(joinupEmpty) && joinupEmpty.length === 0)
    pass("joinup.fetch() returns [] for an empty board");
