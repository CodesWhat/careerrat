...
try {
  const hkModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/hecklerkoch.mjs")).href
  );
  ...
  if (
    hkListUrl({ careers_url: "https://www.heckler-koch.com/en/Career" }) ===
    "https://www.heckler-koch.com/de/Karriere/Stellenangebote"
  )
    pass("hecklerkoch.resolveListUrl() defaults to the Stellenangebote list");
  else
    fail(
      `hecklerkoch.resolveListUrl() default wrong: ${hkListUrl({ careers_url: "https://www.heckler-koch.com/en/Career" })}`
    );
  if (
    hk.detect({ careers_url: "https://evil.com/x.heckler-koch.com" }) === null &&
    hk.detect({ careers_url: "https://heckler-koch.com.evil.com/x" }) === null
  ) {
  ...
  const hkHtml =
    "<html>" +
    hkCard(
      "d1be4446a082dd289578456f38fb82473beedb350",
      "Maschineneinrichter (m/w/d) - Fr&#228;sen"
    ) +
  ...
  if (
    hkRows[0]?.title === "Maschineneinrichter (m/w/d) - Fräsen" &&
    hkRows[0]?.url ===
      "https://karriere.heckler-koch.com/jobposting/d1be4446a082dd289578456f38fb82473beedb350"
  ) {
  ...
  if (parseListing("<html>no jobs</html>").length === 0 && parseListing(undefined).length === 0)
    pass("hecklerkoch.parseListing() returns [] for job-less / non-string input");
  ...
  if (badRows.length === 1 && badRows[0].title === "Bad&#xD800;Entity")
    pass("hecklerkoch.parseListing() tolerates an invalid numeric entity (no RangeError crash)");
  else
    fail(
      `hecklerkoch.parseListing() should degrade a malformed entity to literal text, got: ${JSON.stringify(badRows)}`
    );

  // fetch — single request, jobs normalized (location empty by design).
  const hkJobs = await hk.fetch(
    { name: "Heckler & Koch", api: "https://www.heckler-koch.com/de/Karriere/Stellenangebote" },
    { fetchText: async () => hkHtml }
  );
  if (hkJobs.length === 2 && hkJobs[0].company === "Heckler & Koch" && hkJobs[0].location === "")
    pass("hecklerkoch.fetch() returns normalized jobs from one request");
