...
const mod = await import(
  pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/icims.mjs")).href
);
...
const fixture = readFileSync(
  join(ROOT, "tests/fixtures/career-ops/icims-search-page.html"),
  "utf-8"
);
...
// detect(): any *.icims.com https careers_url resolves to the search URL.
{
  ...
  if (hit && hit.url === `${ORIGIN}/jobs/search?ss=1&pr=0&in_iframe=1`)
    pass("icims.detect() resolves portal search URL");
  else fail(`icims.detect() returned ${JSON.stringify(hit)}`);

  if (icims.detect({ name: "X", careers_url: "https://example.com/jobs" }) === null)
    pass("icims.detect() null for non-icims URL");
  else fail("icims.detect() accepted a non-icims URL");

  if (icims.detect({ name: "X", careers_url: "http://careers-a.icims.com/jobs" }) === null)
    pass("icims.detect() rejects plain http");
...
// parseIcimsSearchPage(): two same-origin cards parsed; foreign-host card dropped.
{
  ...
  const [dir, fork] = jobs;
  if (dir.title === "Director, Revenue Operations & Strategy")
    pass("title extracted with entities decoded");
  else fail(`title: ${JSON.stringify(dir.title)}`);
  if (dir.url === `${ORIGIN}/jobs/1234/director%2c-revenue-operations/job`)
    pass("url extracted with query stripped");
  ...
  if (dir.postedAt === undefined)
    pass("no postedAt on list-page jobs (enrichDate supplies it later)");
  else fail(`unexpected postedAt: ${dir.postedAt}`);
  if (fork.location === "US-CA-Fontana" && fork.title === "Forklift Operator")
    pass("second card parsed");
...
// Relative posting hrefs must resolve against the origin, not be dropped.
{
  ...
  if (jobs.length === 1 && jobs[0].url === `${ORIGIN}/jobs/7777/relative-role/job`)
    pass("relative href resolved against origin");
  ...
  if (parseIcimsSearchPage(`<ul>${offHost}</ul>`, ORIGIN, "acmefreight").length === 0)
    pass("resolved off-host href still dropped");
...
{
  if (
    parseIcimsSearchPage("", ORIGIN, "x").length === 0 &&
    parseIcimsSearchPage("<html>no cards</html>", ORIGIN, "x").length === 0
  ) {
...
// Multi-page tenant: stops on the first empty page.
{
  ...
  const jobs = await icims.fetch(
    { name: "acmefreight", careers_url: `${ORIGIN}/jobs/search?ss=1` },
    ctx
  );
  if (jobs.length === 3 && mkCtx.calls.join(",") === "0,1,2")
...
// Tenant that repeats the last page for out-of-range pr: repeat-content stop.
{
  ...
  if (jobs.length === 2 && mkCtx.calls.length === 3)
    pass("fetch stops when a page repeats the previous first URL");
...
// every page holds distinct jobs, so neither natural stop condition fires.
{
  ...
  const jobs = await icims.fetch(
    { name: "bigtenant", careers_url: `${ORIGIN}/jobs/search?ss=1` },
    ctx
  );
...
// ── enrichDate(): detail-page JSON-LD ───────────────────────────────
{
  ...
  if (job.postedAt === Date.parse("2026-07-20"))
    pass("enrichDate sets postedAt from JSON-LD datePosted");
...
// @graph document: JobPosting node nested under @graph is still found.
{
  ...
  await icims.enrichDate(job, { fetchText: async () => detail });
  if (job.postedAt === Date.parse("2026-07-18"))
    pass("enrichDate reads datePosted from an @graph JobPosting node");
...
// Array-form JSON-LD: pick the JobPosting element, skip the non-JobPosting one.
{
  ...
  await icims.enrichDate(job, { fetchText: async () => detail });
  if (job.postedAt === Date.parse("2026-07-19"))
    pass("enrichDate picks the JobPosting node from an array");
...
}
{
  ...
  await icims.enrichDate(job, {
    fetchText: async () => '<script type="application/ld+json">{broken json</script>',
  });
...
// the date also carries jobLocation.address.
{
  ...
  if (job.location === "Annapolis Junction, MD, United States")
    pass("enrichDate fills an empty location from jobLocation");
...
}
{
  ...
  if (job.location === "Toronto, ON, Canada")
    pass("enrichDate replaces an N/A location, single jobLocation object");
  else fail(`location: ${job.location}`);
}
...
// returned no location at all even though a usable one was in the array.
{
  ...
  await icims.enrichDate(job, { fetchText: async () => detail });
  if (job.location === "Toronto, ON, Canada")
    pass("enrichDate walks past an all-UNAVAILABLE jobLocation entry to a later one");
  else fail(`location: ${job.location}`);
}
// Single-entry jobLocation behavior is unchanged.
{
  ...
  await icims.enrichDate(job, { fetchText: async () => detail });
  if (job.location === "Denver, CO, United States")
    pass("enrichDate still handles a single jobLocation entry");
  else fail(`location: ${job.location}`);
}
