...
try {
  const breezyModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/breezy.mjs")).href
  );
  ...
  const hit = breezy.detect({
    name: "New Incentives",
    careers_url: "https://new-incentives.breezy.hr/",
  });
  ...
  if (
    breezy.detect({ name: "X", careers_url: null }) === null &&
    breezy.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    breezy.detect({ name: "Spoof", careers_url: "https://evil.example/acme.breezy.hr/json" }) ===
    null
  ) {
  ...
  if (
    jobs[0]?.title === "Assistant Field Manager" &&
    jobs[0]?.company === "New Incentives" &&
    jobs[0]?.location === "Niger, Sokoto, NG"
  ) {
  ...
  if (jobs[1]?.location === "Lagos, Lagos, NG, Remote") {
    ...
    fail(
      `row 1 location = ${JSON.stringify(jobs[1]?.location)}, expected "Lagos, Lagos, NG, Remote"`
    );
  }
  ...
  const noDup = parseBreezyResponse(
    [
      {
        name: "R",
        url: "https://acme.breezy.hr/p/r",
  ...
  if (noDup[0]?.location === "Remote, EMEA")
    pass("parseBreezyResponse does not double-append Remote");
