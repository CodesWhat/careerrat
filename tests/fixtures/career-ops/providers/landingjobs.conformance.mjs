...
try {
  const ljModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/landingjobs.mjs")).href
  );
  ...
  const full = normalizeLandingJob(
    {
      ...
      url: "https://landing.jobs/at/inscale/senior-java-dev",
  ...
  if (
    full &&
    full.title === "Senior Java Dev" &&
    full.url === "https://landing.jobs/at/inscale/senior-java-dev" &&
    full.company === "Inscale" &&
    full.location === "Lisbon, PT" &&
    full.postedAt === Date.parse("2025-02-26T09:38:38.127Z")
  ) {
    pass(
      "normalizeLandingJob maps title/url, derives company from slug, builds location, parses published_at"
    );
  } else {
  ...
  const remoteJob = normalizeLandingJob({
    title: "R",
    url: "https://landing.jobs/at/acme/r",
    locations: [{ city: "Berlin", country_code: "DE" }],
    remote: true,
  });
  if (remoteJob?.location === "Berlin, DE, Remote")
    pass('normalizeLandingJob appends "Remote" when remote is true');
  ...
  const coEntry = normalizeLandingJob(
    { title: "T", url: "https://landing.jobs/jobs/99" },
    "Entry Name"
  );
  ...
  if (
    ...
    coDefault?.company === "Landing.jobs" &&
    coBlank?.company === "Landing.jobs"
  ) {
    pass(
      'normalizeLandingJob falls back company → entry name → "Landing.jobs" (whitespace-only entry name ignored)'
    );
  } else {
    fail(
      `normalizeLandingJob company fallbacks = ${JSON.stringify({ a: coEntry?.company, b: coDefault?.company, c: coBlank?.company })}`
    );
  }

  // postedAt falls back to created_at; omitted when both absent.
  const fromCreated = normalizeLandingJob({
    title: "T",
    url: "https://landing.jobs/at/acme/cd",
    created_at: "2025-02-26T08:53:02.254Z",
  });
  const noDate = normalizeLandingJob({ title: "T", url: "https://landing.jobs/at/acme/nd" });
  if (
    fromCreated?.postedAt === Date.parse("2025-02-26T08:53:02.254Z") &&
    noDate &&
    !("postedAt" in noDate)
  ) {
    pass(
      "normalizeLandingJob falls back to created_at and omits postedAt when both dates are absent"
    );
  } else {
    fail(
      `normalizeLandingJob date handling = ${JSON.stringify({ created: fromCreated?.postedAt, none: noDate })}`
    );
  }
  ...
  if (drops.every((r) => r === null)) {
    pass(
      "normalizeLandingJob host-locks url to landing.jobs and drops off-host/non-https/no-url/empty-title/non-object"
    );
  } else {
  ...
  const sample = [
    {
      title: "Role A",
      url: "https://landing.jobs/at/acme/role-a",
      locations: [{ city: "Porto", country_code: "PT" }],
      remote: false,
      published_at: "2025-02-26T09:38:38.127Z",
    },
  ...
  if (capturedUrl === "https://landing.jobs/api/v1/jobs")
    pass("landingjobs.fetch() requests the v1 feed URL");
  else fail(`landingjobs.fetch() requested ${JSON.stringify(capturedUrl)}`);

  if (capturedOpts && capturedOpts.redirect === "error")
    pass('landingjobs.fetch() passes redirect:"error" (SSRF guard)');
  else
    fail(`landingjobs.fetch() should pass redirect:"error", got: ${JSON.stringify(capturedOpts)}`);

  if (fetched.length === 1 && fetched[0]?.company === "Acme") {
    pass(
      "landingjobs.fetch() returns normalized jobs (drops the empty-title row, derives company)"
    );
  } else {
    fail(
      `landingjobs.fetch() returned ${fetched.length} jobs, row 0 = ${JSON.stringify(fetched[0])}`
    );
  }
