...
try {
  const workingnomadsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/workingnomads.mjs")).href
  );
  ...
  if (capturedOpts && capturedOpts.redirect === "error")
    ...
    fail(
      `workingnomads.fetch() should pass redirect:"error", got: ${JSON.stringify(capturedOpts)}`
    );
  ...
  if (fetched[0] && Object.keys(fetched[0]).sort().join(",") === "company,location,title,url")
    ...
    fail(
      `workingnomads.fetch() row 0 keys = ${JSON.stringify(fetched[0] && Object.keys(fetched[0]))}`
    );

  if (
    fetched[0]?.title === "Senior AI Engineer" &&
    ...
    pass(
      "workingnomads.fetch() maps title/url and trims company_name + location into the normalized shape"
    );
  ...
  if (
    ...
    fail(
      `workingnomads.fetch() row 1 title/url = ${JSON.stringify({ title: fetched[1]?.title, url: fetched[1]?.url })}`
    );
  ...
  if (noName[0]?.company === "Working Nomads")
    pass(
      'workingnomads.fetch() defaults company to "Working Nomads" when company_name and entry.name are both missing'
    );
  ...
  if (Array.isArray(empty) && empty.length === 0)
    pass("workingnomads.fetch() returns [] for an empty feed");
