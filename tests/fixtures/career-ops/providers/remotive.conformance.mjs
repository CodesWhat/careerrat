...
try {
  const remotiveModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/remotive.mjs")).href
  );
  ...
  if (fetched[0] && Object.keys(fetched[0]).sort().join(",") === "company,location,title,url")
    ...
    fail(`remotive.fetch() row 0 keys = ${JSON.stringify(fetched[0] && Object.keys(fetched[0]))}`);
  ...
  if (
    ...
    fail(
      `remotive.fetch() row 1 title/url = ${JSON.stringify({ title: fetched[1]?.title, url: fetched[1]?.url })}`
    );
  ...
  const noName = await remotive.fetch(
    ...
    {
      fetchJson: async () => ({
        jobs: [{ title: "Role", url: "https://remotive.com/remote-jobs/x" }],
      }),
    }
  );
  if (noName[0]?.company === "Remotive")
    pass(
      'remotive.fetch() defaults company to "Remotive" when company_name and entry.name are both missing'
    );
