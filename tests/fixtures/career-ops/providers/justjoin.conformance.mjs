...
try {
  const justjoinModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/justjoin.mjs")).href
  );
  ...
  if (
    jj.detect({ name: "JustJoin", careers_url: "https://justjoin.it/job-offers/all-locations" })
      ?.url
  ) {
  ...
  if (
    jj.detect({ name: "X", careers_url: "https://evil.example/justjoin.it/job-offers" }) === null
  ) {
  ...
  if (
    jj.detect({ name: "X", api: "https://justjoin.it/api/candidate-api/offers/count" }) === null
  ) {
  ...
  if (
    parsed[0].title === "Senior Developer" &&
    parsed[0].url === "https://justjoin.it/job-offer/acme-senior-dev-warsaw-javascript"
  ) {
  ...
  if (
    parsed[0].company === "Acme" &&
    parsed[0].location === "remote, Warszawa" &&
    typeof parsed[0].postedAt === "number"
  ) {
  ...
  if (
    fetched.length === 1 &&
    capturedUrl.startsWith("https://justjoin.it/api/candidate-api/offers?")
  ) {
  ...
  if (capturedOpts && capturedOpts.redirect === "error")
    pass('justjoin.fetch() passes redirect:"error"');
