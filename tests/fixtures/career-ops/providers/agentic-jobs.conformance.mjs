...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/agentic-jobs.mjs")).href
  );
  ...
  if (
    agentic.detect({ name: "X", careers_url: "https://agentic-engineering-jobs.com/" }) === null
  ) {
  ...
  if (countryName("US") === "United States" && countryName("de") === "Germany") {
    ...
    fail(
      `countryName('US')=${JSON.stringify(countryName("US"))} countryName('de')=${JSON.stringify(countryName("de"))}`
    );
  }
  if (
    countryName("XX") === "" &&
  ...
  if (normalizeAgenticLocation({ location: "Remote (EU)", countries: ["DE"] }) === "Remote (EU)") {
    ...
    fail(
      `normalizeAgenticLocation() with location wrong: ${JSON.stringify(normalizeAgenticLocation({ location: "Remote (EU)", countries: ["DE"] }))}`
    );
  }
  if (
    normalizeAgenticLocation({ location: "", countries: ["CA", "GB"] }) ===
    "Canada / United Kingdom"
  ) {
    pass(
      "normalizeAgenticLocation() falls back to deduped decoded countries when location is empty"
    );
  } else {
    fail(
      `normalizeAgenticLocation() countries-fallback wrong: ${JSON.stringify(normalizeAgenticLocation({ location: "", countries: ["CA", "GB"] }))}`
    );
  }
  if (
    normalizeAgenticLocation({ location: null, countries: [], geoRegion: "global" }) === "global"
  ) {
    pass("normalizeAgenticLocation() falls back to geoRegion when countries is empty too");
  } else {
    fail(
      `normalizeAgenticLocation() geoRegion-fallback wrong: ${JSON.stringify(normalizeAgenticLocation({ location: null, countries: [], geoRegion: "global" }))}`
    );
  }
  ...
  const bothBounds = normalizeAgenticSalary({
    salaryMin: 196875,
    salaryMax: 246094,
    salaryCurrency: "usd",
  });
  if (
    bothBounds &&
    bothBounds.min === 196875 &&
    bothBounds.max === 246094 &&
    bothBounds.currency === "USD"
  ) {
  ...
  const minOnly = normalizeAgenticSalary({
    salaryMin: 100000,
    salaryMax: null,
    salaryCurrency: "USD",
  });
  ...
  if (
    job &&
    job.salary &&
    job.salary.min === 120000 &&
    job.salary.max === 160000 &&
  ...
  if (
    unsafeSlugs.every(
      (slug) => normalizeAgenticJob({ title: "X", companyName: "Y", slug }) === null
    )
  ) {
    pass(
      "normalizeAgenticJob() rejects path-unsafe / traversal / URL-injection slugs (feeds straight into a URL path)"
    );
  } else {
    fail(
      `normalizeAgenticJob() should reject every unsafe slug: ${JSON.stringify(unsafeSlugs.map((slug) => [slug, normalizeAgenticJob({ title: "X", companyName: "Y", slug })]))}`
    );
  }
  if (
  ...
  const noExtras = normalizeAgenticJob({
    title: "X",
    companyName: "Y",
    slug: "x-y",
    location: "Remote",
  ...
  if (
    noExtras &&
    !("description" in noExtras) &&
    !("postedAt" in noExtras) &&
    !("salary" in noExtras)
  ) {
    ...
    return {
      ...
      ctx: {
        fetchJson: async (url, opts) => {
          ...
          return (
            pages[pageNum - 1] ?? { data: [], meta: { total: 0, page: pageNum, per_page: 50 } }
          );
        },
        ...(sleep ? { sleep: async () => {} } : {}),
      },
  ...
  const onePage = mkCtx([
    { data: [mkJob(1), mkJob(2)], meta: { total: 2, page: 1, per_page: 50 } },
  ]);
  const onePageJobs = await agentic.fetch(
    { name: "Agentic Engineering Jobs", provider: "agentic-jobs" },
    onePage.ctx
  );
  if (
    ...
    fail(
      `agentic-jobs.fetch() one-page: ${onePageJobs.length} jobs, calls=${JSON.stringify(onePage.calls.map((c) => c.url))}`
    );
  }
  ...
  if (twoPageJobs.length === 51 && twoPages.calls.length === 2) {
    ...
    fail(
      `agentic-jobs.fetch() two-page: ${twoPageJobs.length} jobs, ${twoPages.calls.length} calls`
    );
  }

  const dupePage = mkCtx([
    { data: [mkJob(1), mkJob(1)], meta: { total: 2, page: 1, per_page: 50 } },
  ]);
  ...
  const crossPageJobs = await agentic.fetch(
    { name: "X", provider: "agentic-jobs" },
    crossPageDupe.ctx
  );
  if (crossPageJobs.length === 3 && new Set(crossPageJobs.map((j) => j.url)).size === 3) {
    ...
    fail(
      `agentic-jobs.fetch() cross-page dedup: expected 3 unique jobs, got ${JSON.stringify(crossPageJobs.map((j) => j.url))}`
    );
  }
  ...
  try {
    ...
    if (e instanceof Error && e.message.includes("unexpected API response shape on page 2"))
      malformedThrew = true;
    else throw e;
  }
  if (malformedThrew) {
    pass(
      "agentic-jobs.fetch() throws on a malformed mid-pagination page instead of silently truncating"
    );
  } else {
    fail(
      'agentic-jobs.fetch() should throw when a later page has a missing/non-array "data" field'
    );
  }
  ...
  try {
    await agentic.fetch(
      { name: "X", provider: "agentic-jobs" },
      mkCtx([{ data: [], meta: { total: 0, page: 1, per_page: 50 } }]).ctx
    );
  } catch (e) {
    // Match the specific canary message — an unrelated error (e.g. a bug
    // elsewhere) must not make this assertion pass by accident.
    if (e instanceof Error && e.message.includes("parsed 0 jobs from the API"))
      zeroJobsThrew = true;
    else throw e;
  }
  if (zeroJobsThrew) {
    pass(
      "agentic-jobs.fetch() throws when the API yields zero jobs (response-shape-change canary)"
    );
  } else {
