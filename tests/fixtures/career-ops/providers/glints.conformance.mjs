...
try {
  const glintsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/glints.mjs")).href
  );
  ...
  const offDomain = parseGlintsItem({ id: "1", title: "Role" }, "https://evil.example.com", "Co");
  ...
  if (parseGlintsItem(null, "https://glints.com", "Co") === null)
    pass("parseGlintsItem(null) → null");
  else fail("parseGlintsItem(null) should return null");
  if (parseGlintsItem(42, "https://glints.com", "Co") === null)
    pass("parseGlintsItem(number) → null");
  ...
  const wwwItem = parseGlintsItem({ id: "x", title: "Role" }, "https://www.glints.com", "Co");
  ...
  if (jobs.length === 1 && jobs[0].title === "AI PM")
    pass("glints.fetch() returns parsed jobs via searchJobsV3");
  ...
  const singlePageCtx = {
    transport: "http",
    fetchJson: async () => ({
      data: { searchJobsV3: { jobsInPage: [{ id: "1", title: "Dev" }], hasMore: false } },
    }),
    fetchText: async () => {
      throw new Error("should not be called");
    },
