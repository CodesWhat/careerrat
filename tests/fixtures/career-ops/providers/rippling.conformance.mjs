...
try {
  const ripplingModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/rippling.mjs")).href
  );
  ...
  const hit = rippling.detect({
    name: "Acme",
    careers_url: "https://ats.rippling.com/acme-corp/jobs",
  });
  ...
  const hitNoJobs = rippling.detect({
    name: "X",
    careers_url: "https://ats.rippling.com/acme-corp",
  });
  if (
    hitNoJobs &&
    hitNoJobs.url === "https://api.rippling.com/platform/api/ats/v1/board/acme-corp/jobs"
  ) {
  ...
  if (
    rippling.detect({ name: "X", careers_url: null }) === null &&
    rippling.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    rippling.detect({ name: "X", careers_url: "http://ats.rippling.com/acme/jobs" }) === null &&
    rippling.detect({ name: "X", careers_url: "https://ats.rippling.com/" }) === null &&
    rippling.detect({
      name: "X",
      careers_url: "https://evil.example/ats.rippling.com/acme/jobs",
    }) === null
  ) {
  ...
  if (
    rippling.detect({ name: "X", careers_url: "https://ats.rippling.com/a%20b/jobs" }) === null &&
    rippling.detect({ name: "X", careers_url: "https://ats.rippling.com/acme.corp/jobs" }) ===
      null &&
  ...
  if (
    rippling.detect({ name: "X", careers_url: "https://ats.rippling.com/just-appraised-jobs/jobs" })
      ?.url === "https://api.rippling.com/platform/api/ats/v1/board/just-appraised-jobs/jobs"
  ) {
  ...
  const sample = [
    {
      ...
      url: "https://ats.rippling.com/acme/jobs/uuid-1",
      department: { label: "Sales" },
      workLocation: { label: "Remote (United States)", id: "x" },
    },
    {
      uuid: "2",
      name: "  ML Engineer  ",
      url: "  https://ats.rippling.com/acme/jobs/uuid-2  ",
      workLocation: { label: "Canada" },
    },
    {
      uuid: "3",
      name: "String Loc Role",
      url: "https://ats.rippling.com/acme/jobs/uuid-3",
      workLocation: "New York",
    }, // workLocation as bare string
    {
      uuid: "4",
      name: "No Loc Role",
      url: "https://ats.rippling.com/acme/jobs/uuid-4",
      workLocation: null,
    }, // null → ''
  ...
  if (jobs.length === 4)
    pass("parseRipplingResponse keeps 4 valid postings (drops empty-name / no-url / non-https)");
  ...
  if (
    jobs[0]?.title === "Account Executive" &&
    jobs[0]?.url === "https://ats.rippling.com/acme/jobs/uuid-1" &&
    jobs[0]?.company === "Acme" &&
    jobs[0]?.location === "Remote (United States)"
  ) {
    pass(
      "parseRipplingResponse maps name→title, url, company from entry name, workLocation.label→location"
    );
  } else {
  ...
  if (
    jobs[1]?.title === "ML Engineer" &&
    jobs[1]?.url === "https://ats.rippling.com/acme/jobs/uuid-2"
  ) {
    pass("parseRipplingResponse trims whitespace from name and url");
  } else {
    fail(
      `parseRipplingResponse row 1 title/url = ${JSON.stringify({ title: jobs[1]?.title, url: jobs[1]?.url })}`
    );
  }

  if (jobs[2]?.location === "New York" && jobs[3]?.location === "") {
    pass(
      'parseRipplingResponse accepts a bare-string workLocation and yields "" when workLocation is null'
    );
  } else {
    fail(
      `parseRipplingResponse loc fallbacks = ${JSON.stringify({ str: jobs[2]?.location, none: jobs[3]?.location })}`
    );
  }

  if (
    parseRipplingResponse({}, "X").length === 0 &&
    parseRipplingResponse(null, "X").length === 0
  ) {
  ...
  if (
    hostLocked.length === 1 &&
    hostLocked[0]?.title === "Valid Host" &&
    hostLocked[0]?.url === "https://ats.rippling.com/acme/jobs/uuid-9"
  ) {
    pass(
      "parseRipplingResponse host-locks the posting url to ats.rippling.com (drops external https URLs)"
    );
  } else {
  ...
  if (badEntryThrew)
    pass("rippling.fetch() throws when the careers_url is not an ats.rippling.com host");
