...
try {
  const hackernewsModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/hackernews.mjs")).href
  );
  ...
  const fullFmt =
    "Acme Corp | Senior Engineer | Remote | https://acme.com/jobs/123\nWe are hiring…";
  ...
  if (
    parsedFree &&
    parsedFree.title === freeFmt &&
    parsedFree.url === "https://news.ycombinator.com/item?id=1"
  ) {
  ...
  const htmlFmt =
    '<p>Beta &amp; Co | Staff SWE | New York, NY | <a href="https://beta.io/jobs">https://beta.io/jobs</a></p>';
  ...
  const bleedFmt =
    "<p>Linear | Product Engineer | Remote<p>We are hiring at https://linear.app/careers/pe-2025";
  ...
  const mockCtx = {
    async fetchJson(url, _opts) {
      ...
      if (url.includes(`/items/${FAKE_THREAD_ID}`)) {
        itemFetched = true;
        return fakeItemResp;
      }
  ...
  if (
    ...
    fail(
      `hackernews.fetch() search params: tags=${searchParams?.get("tags")} hitsPerPage=${searchParams?.get("hitsPerPage")} query=${searchParams?.get("query")}`
    );
  }
