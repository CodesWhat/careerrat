...
try {
  const gemModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/gem.mjs")).href
  );
  ...
  if (
    gem.detect({ name: "Spoof", api: "https://evil.example/job_board/v0/acme/job_posts/" }) ===
      null &&
  ...
  if (
    ...
    pass(
      "parseRestResponse() normalizes title, canonical jobs.gem.com URL, location, description, and publication date"
    );
  } else {
  ...
  const offPathRows = parseRestResponse(
    [
      ...
      {
        title: "Not a job (three segments)",
        absolute_url: "https://jobs.gem.com/example-company/456/apply",
      },
  ...
  if (offPathRows.length === 1 && offPathRows[0].title === "Real job") {
    pass(
      "parseRestResponse() rejects jobs.gem.com URLs off the canonical /{vanity_path}/{id} posting path"
    );
  } else {
  ...
  const wrappedRows = parseRestResponse(
    {
      job_posts: [
        { title: "Wrapped Role", absolute_url: "https://jobs.gem.com/example-company/789" },
      ],
    },
    "Gem REST"
  );
  ...
  if (emptyEnvelopesOk)
    pass(
      "parseRestResponse() returns [] for empty/contentless envelopes ([], {}, null, undefined)"
    );
  ...
  try {
    ...
    if (/unsupported REST response envelope/.test(e.message)) {
      pass(
        "parseRestResponse() throws a descriptive error for an unsupported nonempty envelope shape"
      );
    } else {
  ...
  if (restContentFallbackRows[0]?.description === "Own & ship the roadmap. Escaped: Role") {
    ...
    fail(
      `parseRestResponse() content fallback description = ${JSON.stringify(restContentFallbackRows[0]?.description)}`
    );
  }
  ...
  const fetchedRest = await gem.fetch(
    { name: "Gem REST", api: "https://api.gem.com/job_board/v0/example-company/job_posts/" },
    {
      fetchJson: async (url, opts) => {
        restUrl = url;
        restOptions = opts;
        return [{ title: "Role", absolute_url: "https://jobs.gem.com/example-company/1" }];
      },
    }
  );
  if (
    restUrl === "https://api.gem.com/job_board/v0/example-company/job_posts/" &&
    restOptions?.redirect === "error" &&
    fetchedRest.length === 1
  ) {
    pass('gem.fetch() reads the pinned REST endpoint with redirect:"error" and makes one request');
  } else {
    fail(
      `gem.fetch() REST url=${JSON.stringify(restUrl)} opts=${JSON.stringify(restOptions)} rows=${JSON.stringify(fetchedRest)}`
    );
  }
  ...
  if (
    gem.detect({ name: "Path spoof", careers_url: "https://evil.example/jobs.gem.com/retool" }) ===
      null &&
    gem.detect({
      name: "Query spoof",
      careers_url: "https://evil.example/careers?x=jobs.gem.com/retool",
    }) === null &&
    gem.detect({
      name: "Suffix spoof",
      careers_url: "https://jobs.gem.com.evil.example/retool",
    }) === null
  ) {
          ...
          jobPostings: [
            {
              ...
              extId: "1001",
              title: "AI Engineer",
              locations: [{ name: "San Francisco", isRemote: false }, { isRemote: true }],
              job: {
  ...
  const detailResponse = [
    {
      data: {
        oatsExternalJobPosting: {
          ...
          descriptionHtml:
            "<p>Build &amp; ship <strong>AI</strong> tools.</p><ul><li>Own the roadmap</li></ul>",
        },
      },
    },
  ];
  ...
  if (
    ...
    calls[0].url === "https://jobs.gem.com/api/public/graphql/batch"
  ) {
    pass(
      "gem.fetch() makes exactly 2 calls, both to the batch endpoint (list, then one batched detail POST)"
    );
  } else {
  ...
  if (
    ...
    listBody.length === 1 &&
    listBody[0].operationName === "JobBoardList" &&
    listBody[0].variables.boardId === "retool"
  ) {
  ...
  if (
    ...
    detailBody.length === 1 &&
    detailBody[0].operationName === "ExternalJobPostingQuery" &&
    detailBody[0].variables.boardId === "retool" &&
    detailBody[0].variables.extId === "1001"
  ) {
    pass(
      "gem.fetch() call 2 batches one ExternalJobPostingQuery op per valid posting (one op for the one valid extId)"
    );
  } else {
  ...
  if (fetched[0]?.postedAt === 1700000000 * 1000)
    ...
    fail(
      `gem.fetch() row 0 postedAt = ${JSON.stringify(fetched[0]?.postedAt)} (expected ${1700000000 * 1000})`
    );
  ...
  const emptyCases = [
    ...
    [{}],
    [{ data: null }],
    [{ data: { oatsExternalJobPostings: null } }],
  ];
  let emptyOk = true;
  for (const body of emptyCases) {
    ...
    if (!Array.isArray(out) || out.length !== 0 || requestCount !== 1) {
      emptyOk = false;
      fail(
        `gem.fetch() body=${JSON.stringify(body)} → ${JSON.stringify(out)}, requests=${requestCount}`
      );
      break;
    }
  }
  ...
  if (
    degraded.length === 1 &&
    degraded[0].postedAt === undefined &&
    degraded[0].description === ""
  ) {
    pass(
      "gem.fetch() degrades gracefully when the detail batch throws: postings still returned, postedAt/description omitted"
    );
  } else {
  ...
  const multi = await gem.fetch(
    { name: "Retool", careers_url: "https://jobs.gem.com/retool" },
    {
      fetchJson: async () => {
        multiCallCount++;
        return multiCallCount === 1 ? multiListResponse : multiDetailResponse;
      },
    }
  );
