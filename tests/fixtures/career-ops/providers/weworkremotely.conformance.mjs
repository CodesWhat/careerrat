...
try {
  const wwrModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/weworkremotely.mjs")).href
  );
...
<rss version="2.0">
  ...
  if (jobs[0]?.company === "Acme & Co" && jobs[0]?.title === "Staff AI Engineer") {
    ...
    fail(
      `row 0 title/company = ${JSON.stringify({ title: jobs[0]?.title, company: jobs[0]?.company })}`
    );
  }
  ...
  if (
    jobs[1]?.company === "WWR Board" &&
    jobs[1]?.title === "Principal Platform Engineer & Tooling"
  ) {
  ...
  if (capturedOpts && capturedOpts.redirect === "error") {
    ...
    fail(
      `weworkremotely.fetch() should pass redirect:"error", got: ${JSON.stringify(capturedOpts)}`
    );
  }
