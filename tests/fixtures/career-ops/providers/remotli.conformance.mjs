...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/remotli.mjs")).href
  );
  ...
  if (misses.every((m) => m === null))
    pass("detect() rejects off-host, look-alike, non-HTTPS and missing careers_url");
  ...
  const row = (jobs, companies) => ({
    jobs: { status: "active", ...jobs },
    companies: companies || {},
  });

  const full = normalizeRemotliJob(
    row({
  ...
  if (
    full &&
    /Own the numbers/.test(full.description) &&
    !/[<>]/.test(full.description) &&
    /FP&A/.test(full.description)
  ) {
    pass(
      "normalizeRemotliJob strips HTML and decodes entities in description (feeds the #1597 fingerprint)"
    );
  } else {
  ...
  const nested = normalizeRemotliJob(
    row({
      title: "T",
      slug: "de",
      status: "active",
  ...
  if (
    nested &&
    !/[<>]/.test(nested.description) &&
    /Own the numbers/.test(nested.description) &&
    /FP&A/.test(nested.description)
  ) {
  ...
  if (
    ...
    pass(
      "normalizeRemotliJob maps salaryMin/Max/Currency to the {min,max,currency} shape scan.mjs expects"
    );
  } else {
  ...
  const inverted = normalizeRemotliJob(
    row({ title: "T", slug: "s1", salaryMin: 200, salaryMax: 100, salaryCurrency: "CHF" })
  );
  if (inverted?.salary?.min === 100 && inverted?.salary?.max === 200)
    pass("normalizeRemotliJob orders inverted salary bounds");
  ...
  if (closed.every((c) => c === null))
    pass("normalizeRemotliJob drops non-active rows (built-in liveness)");
  ...
  if (unknownStatus.every((s) => s === null)) {
    pass(
      "normalizeRemotliJob rejects rows whose status is missing, null, non-string or blank (fails closed)"
    );
  } else {
  ...
  if (active && active.url === "https://remotli.ch/jobs/ns")
    pass('normalizeRemotliJob keeps rows with status "active"');
  ...
  const messyActive = normalizeRemotliJob({
    jobs: { title: "T", slug: "ns", status: "  ACTIVE  " },
    companies: {},
  });
  if (messyActive && messyActive.url === "https://remotli.ch/jobs/ns")
    pass("normalizeRemotliJob trims and lowercases status before comparing");
  ...
  if (
    fromJoin?.company === "Join Co" &&
    fromEntry?.company === "Entry Name" &&
    fallback?.company === "Remotli"
  ) {
    pass('normalizeRemotliJob falls back company → companies.name → entry name → "Remotli"');
  } else {
    fail(
      `normalizeRemotliJob company fallbacks = ${JSON.stringify({ a: fromJoin?.company, b: fromEntry?.company, c: fallback?.company })}`
    );
  }
  ...
  if (urlFailures.length === 0) {
    pass(
      "normalizeRemotliJob emits applyUrl as the canonical URL, falling back to the board page (8 cases)"
    );
  } else {
  ...
  if (badSlugs.every((b) => b === null))
    pass("normalizeRemotliJob drops path-unsafe slugs, empty title and malformed rows");
  ...
  let entityResult;
  try {
    const dec = normalizeRemotliJob(
      row({ title: "T", slug: "ent", status: "active", description: OUT_OF_RANGE })
    );
  ...
  if (entityResult === OUT_OF_RANGE) {
    pass(
      "normalizeRemotliJob passes out-of-range numeric/hex entities through verbatim (no RangeError, no data loss)"
    );
  } else {
  ...
  const decoded = normalizeRemotliJob(
    row({ title: "T", slug: "e2", status: "active", description: "<p>R&amp;D team</p>" })
  );
  ...
  try {
    const mixed = await remotli.fetch(
      { name: "R" },
      {
        fetchJson: async () => ({
          jobs: [
  ...
  if (pageSurvived)
    pass("fetch() keeps the whole page when one description carries a malformed entity");
  ...
  if (noDate && !("postedAt" in noDate))
    pass("normalizeRemotliJob omits postedAt when no date is present");
  else fail(`normalizeRemotliJob postedAt presence = ${JSON.stringify(noDate)}`);

  // --- fetch() ---------------------------------------------------------------
  const mk = (i) => ({
    jobs: { title: `Role ${i}`, slug: `role-${i}`, status: "active" },
    companies: { name: `Co ${i}` },
  });
  ...
  const pagedFetch = async (url, opts) => {
    ...
    if (page === 1)
      return {
        ...
        pagination: { page: 1, limit: 50, total: 60, totalPages: 2 },
      };
    return {
      jobs: Array.from({ length: 10 }, (_, i) => mk(50 + i)),
  ...
  if (requested.every((r) => r.redirect === "error"))
    pass('fetch() passes redirect:"error" on every page (SSRF guard)');
  ...
  if (firstPageShapeThrew)
    pass("fetch() treats a malformed first page as a dead board, not as proof of life");
    ...
    partial = await remotli.fetch(
      { name: "X" },
      {
        fetchJson: async (url) => {
          ...
          return {
            jobs: Array.from({ length: 50 }, (_, i) => mk(page * 100 + i)),
            pagination: { page, limit: 50, total: 400, totalPages: 8 },
          };
        },
      }
    );
  ...
  if (!partialThrew && partial && partial.length === 100) {
    ...
    fail(
      `fetch() mid-scan failure: threw=${partialThrew && partialThrew.message} length=${partial && partial.length}`
    );
  }
  ...
  try {
    partialShape = await remotli.fetch(
      { name: "X" },
      {
        fetchJson: async (url) => {
          const page = Number(new URL(url).searchParams.get("page"));
          ...
          return {
            jobs: Array.from({ length: 50 }, (_, i) => mk(i)),
            pagination: { page: 1, limit: 50, total: 400, totalPages: 8 },
          };
        },
      }
    );
