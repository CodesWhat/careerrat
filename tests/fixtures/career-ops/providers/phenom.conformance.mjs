...
import { captureConsoleErrors, fail, pass, ROOT } from "../helpers.mjs";

console.log("\nProvider — phenom (Phenom People CareerConnect widgets API)");
try {
  const phenomModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/phenom.mjs")).href
  );
  const phenom = phenomModule.default;
  const {
    resolveConfig: phConfig,
    slugify,
    parsePhenomDate,
    jobLocation,
    parseRefineSearch,
  ...
  const phCfg = phConfig({
    api: "https://careers.exampleco.com",
    phenom: {
      lang: "en_global",
      country: "global",
      urlPrefix: "global/en",
  ...
  if (
    phCfg &&
    phCfg.widgetsApi === "https://careers.exampleco.com/widgets" &&
    phCfg.urlPrefix === "global/en" &&
    phCfg.selectedFields.country[0] === "Germany"
  ) {
  ...
  if (
    phDefault &&
    phDefault.lang === "en_global" &&
    phDefault.country === "global" &&
    phDefault.urlPrefix === "global/en"
  )
    pass("phenom.resolveConfig() applies en_global/global/urlPrefix defaults");
  ...
  if (typeof phenom.detect !== "function")
    pass("phenom has no detect() — always selected via explicit provider: phenom");
  else fail("phenom should not define detect()");

  // slugify — strips umlauts and specials, collapses to hyphens.
  if (slugify("Sr Economic & Financial Analyst") === "Sr-Economic-Financial-Analyst")
    pass("phenom.slugify() collapses specials to hyphens");
  else fail(`phenom.slugify() wrong: ${slugify("Sr Economic & Financial Analyst")}`);
  if (slugify("München HR (m/w/d)") === "Munchen-HR-m-w-d")
    pass("phenom.slugify() strips umlaut diacritics");
  else fail(`phenom.slugify() umlaut wrong: ${slugify("München HR (m/w/d)")}`);
  if (slugify("###") === "job")
    pass('phenom.slugify() falls back to "job" for an all-symbol title');
  else fail(`phenom.slugify() fallback wrong: ${slugify("###")}`);

  if (
    parsePhenomDate("2026-05-07T18:25:30.000+0000") ===
      Date.parse("2026-05-07T18:25:30.000+0000") &&
    parsePhenomDate("") === undefined
  )
    pass("phenom.parsePhenomDate() reads ISO instants, rejects empty");
  else fail("phenom.parsePhenomDate() wrong");

  // jobLocation — prefers explicit location, else assembles city/state/country.
  if (jobLocation({ location: "Munich, Germany" }) === "Munich, Germany")
    pass("phenom.jobLocation() prefers the explicit location field");
  else fail("phenom.jobLocation() should prefer location");
  if (
    jobLocation({ city: "Munich", state: "Bavaria", country: "Germany" }) ===
    "Munich, Bavaria, Germany"
  )
    pass("phenom.jobLocation() assembles from city/state/country");
  else
    fail(
      `phenom.jobLocation() assembly wrong: ${jobLocation({ city: "Munich", state: "Bavaria", country: "Germany" })}`
    );
  ...
  const { total: phTotal, rows: phRows } = parseRefineSearch(phJson, {
    origin: "https://careers.exampleco.com",
    urlPrefix: "global/en",
  });
  if (phTotal === 42 && phRows.length === 2)
    pass("phenom.parseRefineSearch() reads totalHits and drops id/title-less records");
  else fail(`phenom.parseRefineSearch() wrong: total=${phTotal} rows=${phRows.length}`);
  if (phRows[0]?.url === "https://careers.exampleco.com/global/en/job/98098/Sr-Analyst")
    pass("phenom.parseRefineSearch() builds the {origin}/{prefix}/job/{id}/{slug} URL");
  else fail(`phenom.parseRefineSearch() url wrong: ${JSON.stringify(phRows[0]?.url)}`);
  if (phRows[1]?.url === "https://careers.exampleco.com/global/en/job/12%2F34/Slash-Id")
    pass("phenom.parseRefineSearch() percent-encodes a jobId path segment");
  ...
  const phPage = (ids) => ({
    refineSearch: { status: 200, totalHits: 150, data: { jobs: ids.map(mkJob) } },
  });
  ...
  const phCtx = {
    ...
    fetchJson: async (url, opts) => {
      ...
      phSawFacet = b.selected_fields;
      if (b.from !== phCalls * 100) throw new Error("bad from offset");
      return phPages[phCalls++] ?? phPage([]);
    },
  };
  const phJobs = await phenom.fetch(
    {
      name: "ExampleCo",
      api: "https://careers.exampleco.com",
      phenom: { selectedFields: { country: ["Germany"] } },
    },
    phCtx
  );
  if (phJobs.length === 102 && phCalls === 2 && new Set(phJobs.map((j) => j.url)).size === 102)
    pass("phenom.fetch() paginates via from/size and dedups across pages");
  else fail(`phenom.fetch() returned ${phJobs.length} jobs after ${phCalls} calls`);
  if (phSawFacet && phSawFacet.country?.[0] === "Germany")
    pass("phenom.fetch() forwards selected_fields facet filters");
  ...
  if (partialJobs.length === 100 && partialCalls === 4)
    pass(
      "phenom.fetch() preserves jobs from earlier pages when a later page fetch exhausts retries"
    );
  else
    fail(
      `phenom.fetch() partial-failure handling wrong: ${partialJobs.length} jobs after ${partialCalls} calls`
    );
  if (!partialWarnings.some((w) => /raise max_pages/.test(w))) {
    ...
    fail(
      `phenom fetch-error stop should not also warn about max_pages: ${JSON.stringify(partialWarnings)}`
    );
  }
  if (
    partialWarnings.some((w) =>
      /truncated at page 2 of \d+ \(100 of 150 jobs\): network blip on page 2/.test(w)
    )
  ) {
    pass(
      "phenom.fetch() warns (console.error) when a page fetch fails mid-pagination, instead of failing silently"
    );
  } else {
    fail(
      `phenom fetch-error stop should log a truncation warning: ${JSON.stringify(partialWarnings)}`
    );
  }
  ...
  if (recoverJobs.length === 102 && recoverCalls === 3) {
    ...
    fail(
      `phenom fetch retry-recovery wrong: ${recoverJobs.length} jobs after ${recoverCalls} calls`
    );
  }
  ...
  await captureConsoleErrors(async () => {
    try {
      await phenom.fetch(
        { name: "ExampleCo", api: "https://careers.exampleco.com" },
        nonRetryableCtx
      );
    } catch {
  ...
  if (nonRetryableThrew && nonRetryableCalls === 1) {
    pass(
      "phenom.fetch() does not retry a non-retryable 4xx and rethrows when no page ever succeeded"
    );
  } else {
    fail(
      `phenom fetch non-retryable error wrong: threw=${nonRetryableThrew} after ${nonRetryableCalls} calls`
    );
  }
  ...
  const { result: hugeJobs, errors: hugeWarnings } = await captureConsoleErrors(() =>
    phenom.fetch(
      { name: "HugeCo", api: "https://careers.hugeco.com" },
      {
        sleep: async () => {},
        fetchJson: async () => {
          hugeCalls++;
          return {
            refineSearch: {
              status: 200,
              totalHits: 1_000_000,
              data: { jobs: Array.from({ length: 100 }, (_, i) => mkJob(`${hugeCalls}-${i}`)) },
            },
          };
        },
      }
  ...
  if (hugeCalls === 20 && hugeJobs.length === 2000) {
    ...
    fail(
      `phenom fetch pagination cap: requests=${hugeCalls}, total=${hugeJobs.length} (expected 20/2000)`
    );
  }
  ...
  const { result: overriddenJobs } = await captureConsoleErrors(() =>
    phenom.fetch(bigEntry, {
      sleep: async () => {},
      fetchJson: async () => {
        overriddenCalls++;
        return {
          refineSearch: {
            ...
            totalHits: 1_000_000,
            data: {
              jobs: Array.from({ length: 100 }, (_, i) => mkJob(`o${overriddenCalls}-${i}`)),
            },
          },
        };
      },
  ...
  if (overriddenCalls === 50 && overriddenJobs.length === 5000) {
    ...
    fail(
      `phenom fetch max_pages override: requests=${overriddenCalls}, total=${overriddenJobs.length} (expected 50/5000)`
    );
  }
  ...
  const { result: absurdJobs } = await captureConsoleErrors(() =>
    phenom.fetch(absurdEntry, {
      sleep: async () => {},
      fetchJson: async () => {
        absurdCalls++;
        return {
          refineSearch: {
            status: 200,
            totalHits: 10_000_000,
            data: { jobs: Array.from({ length: 100 }, (_, i) => mkJob(`a${absurdCalls}-${i}`)) },
          },
        };
      },
    })
  ...
  if (absurdCalls === 300 && absurdJobs.length === 30_000) {
    ...
    fail(
      `phenom fetch max_pages hard cap: requests=${absurdCalls}, total=${absurdJobs.length} (expected 300/30000)`
    );
  }
  ...
  for (const invalidMaxPages of [-5, 0, "abc", NaN, null]) {
    ...
    const invalidEntry = {
      name: "InvalidCo",
      api: "https://careers.invalidco.com",
      max_pages: invalidMaxPages,
    };
    const { result: invalidJobs } = await captureConsoleErrors(() =>
      phenom.fetch(invalidEntry, {
        sleep: async () => {},
        fetchJson: async () => {
          invalidCalls++;
          return {
            refineSearch: {
              status: 200,
              totalHits: 1_000_000,
              data: { jobs: Array.from({ length: 100 }, (_, i) => mkJob(`i${invalidCalls}-${i}`)) },
            },
          };
        },
      })
    ...
    if (invalidCalls === 20 && invalidJobs.length === 2000) {
      ...
      fail(
        `phenom fetch invalid max_pages=${label}: requests=${invalidCalls}, total=${invalidJobs.length} (expected 20/2000)`
      );
    }
  }
  ...
  const { result: ctxCapJobs, errors: ctxCapWarnings } = await captureConsoleErrors(() =>
    phenom.fetch(
      { name: "ProbeCo", api: "https://careers.probeco.com" },
      {
        maxPages: 1,
        sleep: async () => {},
        fetchJson: async () => {
          ...
          return {
            refineSearch: {
              status: 200,
              totalHits: 1_000_000,
              data: { jobs: Array.from({ length: 100 }, (_, i) => mkJob(`p${ctxCapCalls}-${i}`)) },
            },
          };
        },
      }
  ...
  if (ctxCapCalls === 1 && ctxCapJobs.length === 100) {
    ...
    fail(
      `phenom fetch ctx.maxPages: requests=${ctxCapCalls}, total=${ctxCapJobs.length} (expected 1/100)`
    );
  }
  if (!ctxCapWarnings.some((w) => /raise max_pages/.test(w))) {
    pass(
      'phenom.fetch() does NOT fire the "raise max_pages" warning when ctx.maxPages (not the entry cap) stopped pagination'
    );
  } else {
    fail(
