...
try {
  const jobstreetModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/jobstreet.mjs")).href
  );
  ...
  if (
    parsed &&
    parsed.title === "Facility Engineer" &&
  ...
  if (offDomain === null) pass("parseJobstreetItem rejects off-domain base URLs");
    ...
    fail(`parseJobstreetItem should reject off-domain base URLs, got ${JSON.stringify(offDomain)}`);

  // parseJobstreetItem — handles null/malformed input safely
  if (parseJobstreetItem(null, "https://id.jobstreet.com", "Co") === null)
    pass("parseJobstreetItem(null) → null");
  else fail("parseJobstreetItem(null) should return null");
  if (parseJobstreetItem(7, "https://id.jobstreet.com", "Co") === null)
    pass("parseJobstreetItem(7) → null");
  ...
  const mockCtx = {
    transport: "http",
    fetchJson: async (url) => {
      if (!url.startsWith("https://id.jobstreet.com/api/jobsearch/v5/search"))
  ...
  if (jobs.length === 1 && jobs[0].title === "AI Engineer")
    pass("jobstreet.fetch() returns parsed jobs via v5 API");
    ...
    await jobstreet.fetch(
      {
        name: "Bad",
        provider: "jobstreet",
        api: "https://evil.example.com/api/jobsearch/v5/search",
      },
