...
try {
  const avatureModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/avature.mjs")).href
  );
  ...
  if (avature.detect({ name: "X", careers_url: "https://acme.avature.net/careers/SearchJobs" }))
    pass("avature.detect() claims a *.avature.net URL");
  else fail("avature.detect() should claim *.avature.net");
  if (avature.detect({ name: "X", careers_url: "https://evil.example/x.avature.net/y" }) === null)
    pass("avature.detect() rejects a path-spoofed URL");
  ...
  if (a1 && a1.title === "Senior PLM Engineer & Architect")
    pass("parseArticles decodes the title entity");
  else fail(`parseArticles title wrong: ${JSON.stringify(a1 && a1.title)}`);
  if (
    a1 &&
    a1.url ===
      "https://acme.avature.net/careers/JobDetail/Senior-PLM-Engineer-17304/17304?businessTitle=PLM"
  )
    pass("parseArticles keeps the absolute JobDetail URL");
  ...
  if (a2 && a2.url === "https://acme.avature.net/en_US/searchjobs/JobDetail/Data-Engineer-900/900")
    pass("parseArticles resolves a relative locale-prefixed JobDetail path");
  else fail(`parseArticles relative url wrong: ${JSON.stringify(a2 && a2.url)}`);
  if (a2 && a2.location === "Munich, Germany")
    pass("parseArticles extracts a rendered location when present");
  else fail(`parseArticles location wrong: ${JSON.stringify(a2 && a2.location)}`);
  if (parseArticles("<div>no articles</div>", origin).length === 0)
    pass("parseArticles returns [] when no articles present");
  ...
  if (vSuffix && vSuffix.title === "Head of PLM")
    pass('parseArticles handles the "article--result 1" class suffix (Siemens)');
  else
    fail(
      `parseArticles missed the class-suffix variant: ${JSON.stringify(vArts.map((a) => a.id))}`
    );
  const vNoClass = vArts.find((a) => a.id === "13672");
  if (vNoClass && vNoClass.title === "Director Platform Engineering")
    pass('parseArticles falls back to a JobDetail anchor without class="link" (Rohde & Schwarz)');
  else
    fail(
      `parseArticles missed the no-class-link variant: ${JSON.stringify(vArts.map((a) => a.id))}`
    );
  ...
  if ((await captureFirstUrl({ name: "X", api: base })) === `${base}?jobOffset=0`)
    pass("avature.fetch() defaults pagination to ?jobOffset=N");
  else fail("avature.fetch() should default to jobOffset");
  if (
    (await captureFirstUrl({ name: "X", api: base, offset_param: "offset" })) === `${base}?offset=0`
  )
    pass("avature.fetch() honours offset_param override (Siemens: ?offset=N)");
  else fail("avature.fetch() should use offset_param when set");
  if (
    (await captureFirstUrl({ name: "X", api: base, offset_param: "  " })) === `${base}?jobOffset=0`
  )
    pass("avature.fetch() falls back to jobOffset for a blank offset_param");
  ...
  const mkHtml = (ids) =>
    ids
      .map(
        (id) =>
          `<article class="article article--result"><h3 class="title"><a class="link" href="${originB}/careers/JobDetail/Role-${id}/${id}">Role ${id}</a></h3></article>`
      )
      .join("");
  ...
  if (healed.length === 14)
    pass("avature.fetch() self-heals jobOffset→offset and walks the full board (14 jobs)");
  else fail(`avature.fetch() self-heal wrong count: ${healed.length} (expected 14)`);
  if (heal.calls.some((u) => /[?&]offset=/.test(u)))
    pass("avature.fetch() self-heal retries with ?offset=N");
  ...
  if (worked.length === 14 && workingCtx.calls.every((u) => /[?&]jobOffset=/.test(u)))
    pass("avature.fetch() does not self-heal when jobOffset already advances");
  else
    fail(
      `avature.fetch() spurious self-heal: ${worked.length} jobs, calls ${JSON.stringify(workingCtx.calls.map((u) => u.split("?")[1]))}`
    );
  ...
  if (emptyHealed.length === 14 && emptyP1Ctx.calls.some((u) => /[?&]offset=/.test(u)))
    pass("avature.fetch() self-heals when the inert key returns an empty page 1");
  ...
  // still decoding valid ones (&amp;).
  {
    ...
    let badArts,
      badThrew = null;
    try {
      badArts = parseArticles(badFrag, origin);
    } catch (e) {
      badThrew = e;
    }
    if (badThrew)
      fail(
        `avature.parseArticles() threw ${badThrew.name} on an out-of-range numeric entity (unguarded String.fromCodePoint): ${badThrew.message}`
      );
    else if (
      badArts.length === 1 &&
      badArts[0].title === "Overflow &#99999999; & Hex &#xFFFFFFFF; Surrogate &#xD800;"
    )
      pass(
        "avature.parseArticles() tolerates out-of-range / surrogate entities, degrading them to literal text while still decoding &amp; (no RangeError crash)"
      );
