...
try {
  const {
    ...
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/jobvite.mjs")).href
  );
  ...
  // ── resolveSlug ────────────────────────────────────────────────
  eq(
    "resolveSlug reads a bare careers_url",
    resolveSlug({ careers_url: "https://jobs.jobvite.com/tylertech" }),
    "tylertech"
  );
  eq(
    "resolveSlug reads a careers_url with a trailing path",
    resolveSlug({ careers_url: "https://jobs.jobvite.com/tylertech/search" }),
    "tylertech"
  );
  eq(
    "resolveSlug rejects a foreign host",
    resolveSlug({ careers_url: "https://evil.example.com/tylertech" }),
    null
  );
  eq(
    "resolveSlug rejects http",
    resolveSlug({ careers_url: "http://jobs.jobvite.com/tylertech" }),
    null
  );
  eq(
    "resolveSlug rejects the api path prefix",
    resolveSlug({ careers_url: "https://jobs.jobvite.com/api/company/x/jobs" }),
    null
  );
  ...
  // ── resolveConfiguredEid ───────────────────────────────────────
  eq(
    "resolveConfiguredEid prefers an explicit company_eid",
    resolveConfiguredEid({ company_eid: "q6NaVfwI" }),
    "q6NaVfwI"
  );
  eq(
    "resolveConfiguredEid trims whitespace",
    resolveConfiguredEid({ company_eid: "  q6NaVfwI  " }),
    "q6NaVfwI"
  );
  eq(
    "resolveConfiguredEid reads ?c= from an api: URL",
    resolveConfiguredEid({ api: "https://app.jobvite.com/CompanyJobs/Xml.aspx?c=q6NaVfwI" }),
    "q6NaVfwI"
  );
  eq(
    "resolveConfiguredEid rejects an api: URL on a foreign host",
    resolveConfiguredEid({ api: "https://evil.example.com/Xml.aspx?c=q6NaVfwI" }),
    null
  );
  eq(
    "resolveConfiguredEid returns null when only a slug is known",
    resolveConfiguredEid({ careers_url: "https://jobs.jobvite.com/tylertech" }),
    null
  );

  // ── extractEidFromBoard ────────────────────────────────────────
  eq(
    "extractEidFromBoard finds a single-quoted eId",
    extractEidFromBoard(`var x=1; companyEId: 'q6NaVfwI', foo:2`),
    "q6NaVfwI"
  );
  eq(
    'extractEidFromBoard tolerates double quotes and "="',
    extractEidFromBoard(`companyEId = "AbC123_-"`),
    "AbC123_-"
  );
  eq(
    "extractEidFromBoard returns null when absent",
    extractEidFromBoard("<html>no id here</html>"),
    null
  );
  eq("extractEidFromBoard returns null for non-string input", extractEidFromBoard(null), null);

  // ── detect ─────────────────────────────────────────────────────
  {
    const d = jobvite.detect({ company_eid: "q6NaVfwI", name: "Tyler" });
    eq(
      "detect() uses the feed URL when the eId is configured",
      d && d.url,
      "https://app.jobvite.com/CompanyJobs/Xml.aspx?c=q6NaVfwI"
    );
  }
  {
    const d = jobvite.detect({ careers_url: "https://jobs.jobvite.com/tylertech", name: "Tyler" });
    eq(
      "detect() falls back to the board URL when only a slug is known",
      d && d.url,
      "https://jobs.jobvite.com/tylertech"
    );
  }
  eq(
    "detect() returns null for a non-Jobvite entry",
    jobvite.detect({ careers_url: "https://boards.greenhouse.io/acme" }),
    null
  );
...
<job>
  ...
  const first = jobs[0];
  eq(
    "parseJobviteXml decodes XML entities in the title",
    first.title,
    "Project Manager - Property & Recording"
  );
  eq(
    "parseJobviteXml upgrades http feed URLs to https",
    first.url.startsWith("https://app.jobvite.com/CompanyJobs/Job.aspx"),
    true
  );
  eq("parseJobviteXml prefers detail-url over apply-url", first.url.includes("Job.aspx"), true);
  eq("parseJobviteXml stamps the company from the entry name", first.company, "Tyler Technologies");
  eq("parseJobviteXml carries the location through", first.location, "Lakewood, CO, United States");
  eq(
    "parseJobviteXml parses the M/D/YYYY date to epoch ms",
    typeof first.postedAt === "number" && new Date(first.postedAt).getUTCFullYear() === 2026,
    true
  );

  eq(
    "parseJobviteXml falls back to apply-url when detail-url is absent",
    jobs[1].url,
    "https://careers.example.com/jobs/9"
  );
  eq(
    "parseJobviteXml omits postedAt when the date is absent",
    Object.hasOwn(jobs[1], "postedAt"),
    false
  );
  ...
  eq(
    "parseJobviteXml falls back to apply-url when detail-url is malformed",
    jobs[2].url,
  ...
  // punctuation from the source ATS), so they must decode, not survive raw.
  {
    const numeric = parseJobviteXml(
      "<result><job><title>Sr Manager &#8217;26 &#x2013; Delivery</title>" +
        "<detail-url><![CDATA[https://app.jobvite.com/x]]></detail-url></job></result>",
      "X"
    );
    eq(
      "parseJobviteXml decodes decimal and hex numeric character references",
      numeric[0].title,
      "Sr Manager ’26 – Delivery"
    );
  }
  ...
  // would otherwise emit straight into a job title.
  {
    ...
    eq(
      "parseJobviteXml leaves XML-illegal numeric references undecoded",
      illegal[0].title,
      "Sr&#0;Manager&#xD800;Ops"
    );
  }
  ...
  // for the ReDoS-shaped regex this parser deliberately avoids.
  {
    ...
    eq(
      "parseJobviteXml handles an unterminated tag without backtracking",
      Date.now() - t0 < 1000,
      true
    );
  }
  ...
  {
    const t0 = Date.now();
    const unclosed = parseJobviteXml(
      "<result>" + "<job><title>x</title>".repeat(40000) + "</result>",
      "X"
    );
    eq(
      "parseJobviteXml handles many unterminated <job> tags in linear time",
      Date.now() - t0 < 1000,
      true
    );
  ...
  eq(
    "parseJobviteXml returns [] for an empty feed",
    parseJobviteXml("<result></result>", "X").length,
    0
  );
  ...
  // Happy path: configured eId goes straight to the feed, no board request.
  {
    ...
    eq(
      "fetch() with a configured eId hits the XML feed",
      seen[0],
      "https://app.jobvite.com/CompanyJobs/Xml.aspx?c=q6NaVfwI"
    );
  ...
  // Discovery path: slug only → board page, then feed.
  {
    ...
    const out = await jobvite.fetch(
      { name: "Tyler", careers_url: "https://jobs.jobvite.com/tylertech" },
      ctx
    );
    ...
    eq(
      "fetch() discovery hits the board with the un-redirect params",
      seen[0],
      "https://jobs.jobvite.com/tylertech?fr=true&nl=1"
    );
    eq(
      "fetch() discovery then hits the feed with the scraped eId",
      seen[1],
      "https://app.jobvite.com/CompanyJobs/Xml.aspx?c=q6NaVfwI"
    );
  ...
  // Discovery failure must name the fix rather than silently returning [].
  {
    ...
    eq(
      "fetch() throws a fix-naming error when the eId cannot be discovered",
      msg.includes("company_eid"),
      true
    );
  }
  ...
  // multi-tenant scan return anything at all.
  {
    ...
    const ctx = {
      ...
      sleep: async (ms) => {
        slept.push(ms);
      }, // test clock — never wall-clock waits
    };
    const out = await jobvite.fetch({ name: "Tyler Technologies", company_eid: "q6NaVfwI" }, ctx);
  ...
  // against the request URL rather than string-compared.
  {
    ...
    eq(
      "fetch() reads a NoJobs.htm feed redirect as an empty board",
