...
try {
  const teamtailorModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/teamtailor.mjs")).href
  );
  ...
  const hit = teamtailor.detect({
    name: "Podimo",
    careers_url: "https://podimo.teamtailor.com/jobs",
  });
  ...
  if (
    teamtailor.detect({ name: "X", careers_url: null }) === null &&
    teamtailor.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    teamtailor.detect({
      name: "Spoof",
      careers_url: "https://evil.example/podimo.teamtailor.com/jobs",
    }) === null
  ) {
  ...
  if (
    jobs[0]?.title === "Sales Director & Lead" &&
    jobs[0]?.company === "Podimo" &&
    jobs[0]?.url === "https://careers.podimo.com/jobs/7950030-sales-director"
  ) {
  ...
  if (fetchJobs.length === 2)
    pass("teamtailor.fetch() hits /jobs.rss with redirect:error and returns parsed jobs");
  ...
  if (
    teamtailor.detect({ name: "Podimo", careers_url: "https://careers.podimo.com/jobs" }) === null
  ) {
  ...
  if (brandedJobs.length === 2)
    pass("explicit provider:teamtailor fetches /jobs.rss off a branded careers host");
  ...
  try {
    await teamtailor.fetch(
      { name: "Podimo", careers_url: "https://careers.podimo.com/jobs" },
      {
        transport: "http",
        fetchText: async () => {
          throw new Error("fetchText should not be reached");
        },
        fetchJson: async () => {
          throw new Error("fetchJson should not be called");
        },
  ...
  if (brandedRefused)
    pass("teamtailor.fetch() refuses a branded host without explicit provider:teamtailor");
