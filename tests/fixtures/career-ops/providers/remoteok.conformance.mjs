...
try {
  const remoteokModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/remoteok.mjs")).href
  );
  ...
  if (fetched.length === 2)
    pass(
      "remoteok.fetch() keeps 2 valid jobs (drops metadata row, null, non-object, empty-position, bad-url rows)"
    );
  else
    fail(
      `remoteok.fetch() returned ${fetched.length} jobs (expected 2): ${JSON.stringify(fetched)}`
    );
  ...
  if (fetched[0] && Object.keys(fetched[0]).sort().join(",") === "company,location,title,url")
    ...
    fail(`remoteok.fetch() row 0 keys = ${JSON.stringify(fetched[0] && Object.keys(fetched[0]))}`);
  ...
  if (
    ...
    fail(
      `remoteok.fetch() row 1 title/url = ${JSON.stringify({ title: fetched[1]?.title, url: fetched[1]?.url })}`
    );
  ...
  if (noName[0]?.company === "RemoteOK")
    pass(
      'remoteok.fetch() defaults company to "RemoteOK" when company and entry.name are both missing'
    );
