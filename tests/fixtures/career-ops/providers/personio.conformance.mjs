...
try {
  const personioModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/personio.mjs")).href
  );
  ...
  const comHit = personio.detect({
    name: "Acme",
    careers_url: "https://acme.jobs.personio.com/jobs",
  });
  ...
  if (
    personio.detect({ name: "X", careers_url: null }) === null &&
    personio.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    personio.detect({
      name: "Spoof",
      careers_url: "https://evil.example/acme.jobs.personio.de/xml",
    }) === null
  ) {
  ...
  if (
    personio.detect({
      name: "Spoof",
      careers_url: "https://acme.jobs.personio.de.evil.com/xml",
    }) === null
  ) {
...
<position>
  ...
  if (jobs.length === 2)
    pass("parsePersonioXml keeps 2 positions (drops empty name + non-numeric id)");
  else fail(`parsePersonioXml returned ${jobs.length} positions (expected 2)`);

  if (
    jobs[0]?.title === "Staff Software Engineer, Data & Platform" &&
    jobs[0]?.company === "Acme"
  ) {
  ...
  if (
    parsePersonioXml("", "X", HOST).length === 0 &&
    parsePersonioXml(null, "X", HOST).length === 0
  ) {
  ...
  await personio.fetch(
    { name: "Acme", careers_url: "https://acme.jobs.personio.de/" },
    {
      fetchText: async (_url, opts) => {
        capturedOpts = opts;
        return "<workzag-jobs></workzag-jobs>";
      },
    }
  );
  ...
  if (htmlJobs.length === 2)
    pass("parsePersonioHtml keeps 2 job-box anchors (ignores unrelated links)");
  else fail(`parsePersonioHtml returned ${htmlJobs.length} jobs (expected 2)`);

  if (
    htmlJobs[0]?.title === "ML-QA Engineer" &&
    htmlJobs[0]?.url === "https://acme.jobs.personio.de/job/2378948"
  ) {
  ...
  if (htmlJobs[0]?.location === "Berlin AI Campus, Munich" && htmlJobs[0]?.company === "Acme") {
    ...
    fail(
      `row 0 location/company = ${JSON.stringify({ location: htmlJobs[0]?.location, company: htmlJobs[0]?.company })}`
    );
  }
  ...
  if (
    hrefFirstJobs.length === 1 &&
    hrefFirstJobs[0]?.title === "Ops Lead" &&
    hrefFirstJobs[0]?.url === "https://acme.jobs.personio.de/job/999001"
  ) {
  ...
  if (
    parsePersonioHtml("", "X", HTML_HOST).length === 0 &&
    parsePersonioHtml(null, "X", HTML_HOST).length === 0
  ) {
  ...
  // fetch() falls back to HTML scraping when /xml 404s.
  {
    ...
    if (
      calls.length === 2 &&
      calls[0].endsWith("/xml") &&
      calls[1] === "https://acme.jobs.personio.de/?language=en"
    ) {
