...
try {
  const dassaultModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/dassault.mjs")).href
  );
  ...
  const mkHit = (f) => {
    const meta = (n, v) =>
      v === undefined ? "" : `<Meta name="${n}"><MetaString name="value">${v}</MetaString></Meta>`;
    return (
      `<Hit did="d" url="x">${"<groups>ignored</groups>"}<metas>` +
  ...
  const xmlA =
    ...
    mkHit({
      ...
      cta1: "https://www.3ds.com/careers/jobs/x-111?a=1&amp;b=2",
      cats: "Category/R&amp;D Type/Regular Country/Germany City/Germany, Munich Products/CATIA Year/4 to 5 years",
      update: "2026/07/03 18:22:13",
    }) +
    mkHit({
      id: "222",
      title: "Data Scientist",
      cta1: "https://www.3ds.com/careers/jobs/y-222",
      cats: "Category/Sales Type/Regular Country/France City/France, Vélizy-Villacoublay Products/DELMIA",
  ...
  if (a[0]?.title === "Software Engineer & Data")
    pass("dassault.parseHits() decodes &amp; in title");
  else fail(`title = ${JSON.stringify(a[0]?.title)}`);

  if (a[0]?.url === "https://www.3ds.com/careers/jobs/x-111?a=1&b=2")
    pass("dassault.parseHits() decodes &amp; in url");
  else fail(`url = ${JSON.stringify(a[0]?.url)}`);

  if (a[0]?.location === "Germany, Munich")
    pass("dassault.parseHits() parses City from content_categories");
  else fail(`location = ${JSON.stringify(a[0]?.location)}`);

  if (a[0]?.company === "Dassault Systèmes")
    pass("dassault.parseHits() sets company from entry name");
  ...
  if (a[0]?.postedAt === Date.UTC(2026, 6, 3, 18, 22, 13))
    pass("dassault.parseHits() postedAt falls back to card_update_timestamp");
  else fail(`postedAt[0] = ${JSON.stringify(a[0]?.postedAt)}`);

  if (a[1]?.postedAt === Date.UTC(2026, 5, 1, 9, 0, 0))
    pass("dassault.parseHits() postedAt prefers content_start_datetime");
  else fail(`postedAt[1] = ${JSON.stringify(a[1]?.postedAt)}`);

  if (a[1]?.location === "France, Vélizy-Villacoublay")
    pass("dassault.parseHits() parses multi-word City value");
  ...
  const xmlForeign =
    ...
    mkHit({
      id: "abc",
      title: "External Aggregated Job",
      cta1: "https://careers.bcit.ca/postings/10516",
    }) +
    `</hits></Answer>`;
  const foreign = parseHits(xmlForeign, "Dassault Systèmes");
  if (foreign.length === 1 && foreign[0].title === "Real 3DS Job")
    pass("dassault.parseHits() drops non-3ds.com postings");
  else
    fail(
      `dassault.parseHits() foreign filter returned ${JSON.stringify(foreign.map((j) => j.title))}`
    );

  // Empty / hit-less XML → []
  if (
    parseHits("", "X").length === 0 &&
    parseHits('<Answer nhits="0"><hits></hits></Answer>', "X").length === 0
  ) {
  ...
  if (
    u.includes("start=20") &&
    u.includes("card_content_type%2Fcareer") &&
    u.includes("cards+language%2Fen")
  ) {
  ...
  if (dassault.detect({ careers_url: "https://www.3ds.com/careers/jobs" }))
    pass("dassault.detect() matches www.3ds.com");
  else fail("dassault.detect() should match www.3ds.com");

  if (dassault.detect({ api: "https://talentacquisition.3ds.com/x" }))
    pass("dassault.detect() matches *.3ds.com subdomains");
  else fail("dassault.detect() should match *.3ds.com");

  if (dassault.detect({ careers_url: "https://evil.com/x.3ds.com" }) === null)
    pass("dassault.detect() rejects path-spoofed host");
  else fail("dassault.detect() should reject path-spoofed host");

  if (dassault.detect({ careers_url: "https://3ds.com.evil.com/x" }) === null)
    pass("dassault.detect() rejects suffix-spoofed host");
  else fail("dassault.detect() should reject suffix-spoofed host");

  if (dassault.detect({ careers_url: 42 }) === null && dassault.detect({}) === null)
    pass("dassault.detect() returns null for non-string / missing url");
  ...
  if (fetched.length === 3 && new Set(fetched.map((j) => j.url)).size === 3)
    pass("dassault.fetch() paginates and dedups across pages");
  else
    fail(
      `dassault.fetch() returned ${fetched.length} jobs (${JSON.stringify(fetched.map((j) => j.title))})`
    );

  if (fetched.every((j) => !("_id" in j)))
    pass("dassault.fetch() strips the internal _id from returned jobs");
  ...
  // still decoding valid ones (&amp;).
  {
    const xmlBad =
      ...
      mkHit({
        id: "bad1",
        title: "Overflow &#99999999; &amp; Hex &#xFFFFFFFF; Surrogate &#xD800;",
        cta1: "https://www.3ds.com/careers/jobs/bad-1",
      }) +
    ...
    let bad,
      badThrew = null;
    try {
      bad = parseHits(xmlBad, "Dassault Systèmes");
    } catch (e) {
      badThrew = e;
    }
    if (badThrew)
      fail(
        `dassault.parseHits() threw ${badThrew.name} on an out-of-range numeric entity (unguarded String.fromCodePoint): ${badThrew.message}`
      );
    else if (bad.length === 2)
      pass(
        "dassault.parseHits() tolerates out-of-range / surrogate numeric entities (no RangeError crash)"
      );
    ...
    if (bj && bj.title === "Overflow &#99999999; & Hex &#xFFFFFFFF; Surrogate &#xD800;")
      pass("dassault.parseHits() degrades bad entities to literal text while still decoding &amp;");
    else if (!badThrew)
      fail(`dassault.parseHits() degraded title wrong: ${JSON.stringify(bj && bj.title)}`);
  }
} catch (e) {
