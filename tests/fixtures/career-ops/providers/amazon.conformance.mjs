...
try {
  const amazon = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/amazon.mjs")).href)
  ).default;
  ...
  if (amazon.detect({ name: "X", careers_url: "https://www.amazon.jobs/en/search" }))
    pass("amazon.detect() claims an amazon.jobs URL");
  else fail("amazon.detect() should claim amazon.jobs");
  if (amazon.detect({ name: "X", careers_url: "https://evil.example/www.amazon.jobs/x" }) === null)
    pass("amazon.detect() rejects a path-spoofed URL");
  else fail("amazon.detect() must reject path-spoofed URLs");
  if (amazon.detect({ name: "X", careers_url: 42 }) === null)
    pass("amazon.detect() returns null for non-string careers_url");
    ...
    jobs: [
      {
        ...
        job_path: "/en/jobs/111/automation-engineer",
        normalized_location: "Erfurt, Thuringia, DEU",
      ...
      {
        title: "SDE",
        job_path: "https://www.amazon.jobs/en/jobs/222/sde",
        location: "Berlin, DEU",
  ...
  const mockCtx = {
    ...
    async fetchJson(url) {
      calls.push(url);
      return calls.length === 1 ? page1 : { jobs: [] };
    },
    async fetchText() {
      return "";
    },
  };
  const jobs = await amazon.fetch(
    { name: "Amazon", amazon: { normalized_country_code: ["DEU"], base_query: "engineer" } },
    mockCtx
  );
  ...
  if (calls[0] && calls[0].includes("normalized_country_code%5B%5D=DEU"))
    pass("amazon.fetch bracket-encodes array facets (normalized_country_code[]=DEU)");
  else fail(`amazon.fetch facet encoding wrong: ${calls[0]}`);
  if (calls[0] && calls[0].includes("result_limit=100"))
    pass("amazon.fetch requests result_limit=100");
  ...
  if (j1 && j1.url === "https://www.amazon.jobs/en/jobs/111/automation-engineer")
    pass("amazon.fetch builds an absolute URL from job_path");
  else fail(`amazon.fetch url wrong: ${JSON.stringify(j1 && j1.url)}`);
  if (j1 && j1.postedAt === Date.parse("July 1, 2026"))
    pass("amazon.fetch parses posted_date (ignores relative updated_time)");
  else fail(`amazon.fetch postedAt wrong: ${JSON.stringify(j1 && j1.postedAt)}`);
  const j2 = jobs.find((j) => j.url.includes("/222/"));
  if (j2 && j2.url === "https://www.amazon.jobs/en/jobs/222/sde")
    pass("amazon.fetch keeps an already-absolute job_path");
