...
try {
  const jibeapplyModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/jibeapply.mjs")).href
  );
  ...
  const hit = jibeapply.detect({
    name: "Acme",
    careers_url: "https://acme.jibeapply.com/jobs?location=Germany",
  });
  ...
  const hitApi = jibeapply.detect({
    name: "X",
    careers_url: "https://acme.jibeapply.com/api/jobs",
  });
  ...
  if (
    jibeapply.detect({
      name: "Spoof",
      careers_url: "https://evil.example/acme.jibeapply.com/jobs",
    }) === null
  ) {
  ...
  const sampleJson = {
    jobs: [
      {
        title: "Senior QA",
        slug: "senior-qa-berlin",
        city: "Berlin",
        country: "Germany",
        hiring_organization: "Acme GmbH",
      },
  ...
  const noOrg = parseJibeapplyResponse(
    { jobs: [{ title: "Dev", slug: "dev", city: "Berlin", country: "Germany" }] },
    entry
  );
  ...
  if (parseJibeapplyResponse({}, entry).length === 0)
    pass("parseJibeapplyResponse({}) → empty result");
  ...
  const malformedCareersEntry = {
    name: "Widget Co",
    careers_url: "jobs.widgetco.com",
    api: "https://jobs.widgetco.com/api/jobs",
  };
  const apiOriginFallback = parseJibeapplyResponse(
    { jobs: [{ title: "Dev", slug: "dev-1" }] },
    malformedCareersEntry
  );
  if (apiOriginFallback[0]?.url === "https://jobs.widgetco.com/jobs/dev-1") {
    ...
    fail(
      `parseJibeapplyResponse api-origin fallback: url = ${JSON.stringify(apiOriginFallback[0]?.url)}`
    );
  }
    ...
    fetchJson: async (url) => {
      ...
      if (page === 1) {
        return {
          totalCount: 15,
          count: 10,
          jobs: Array.from({ length: 10 }, (_, i) => ({ title: `Job ${i}`, slug: `job-${i}` })),
        };
      }
      return {
        totalCount: 15,
        count: 10,
        jobs: Array.from({ length: 5 }, (_, i) => ({ title: `Job p2-${i}`, slug: `job-p2-${i}` })),
      };
    },
  });
  if (pageRequests === 2 && fetchedJibe.length === 15) {
    ...
    fail(
      `jibeapply fetch pagination: requests=${pageRequests}, total=${fetchedJibe.length} (expected 2/15)`
    );
  }
  ...
  if (hugeRequests === 50 && fetchedHuge.length === 500) {
    ...
    fail(
      `jibeapply fetch pagination cap: requests=${hugeRequests}, total=${fetchedHuge.length} (expected 50/500)`
    );
  }
  if (jibeCapWarnings.some((w) => /has more postings than max_pages allows/.test(w))) {
    ...
    fail(
      `jibeapply fetch cap: expected a truncation warning, got ${JSON.stringify(jibeCapWarnings)}`
    );
  }
  ...
  const bigEntry = {
    name: "BigCo",
    careers_url: "https://bigco.jibeapply.com/jobs",
    max_pages: 80,
  };
  ...
  if (overriddenRequests === 80 && fetchedOverridden.length === 800) {
    ...
    fail(
      `jibeapply fetch max_pages override: requests=${overriddenRequests}, total=${fetchedOverridden.length} (expected 80/800)`
    );
  }
  ...
  const absurdEntry = {
    name: "AbsurdCo",
    careers_url: "https://absurdco.jibeapply.com/jobs",
    max_pages: 100_000,
  };
  ...
  if (cappedOverrideRequests === 500 && fetchedAbsurd.length === 5000) {
    ...
    fail(
      `jibeapply fetch max_pages hard cap: requests=${cappedOverrideRequests}, total=${fetchedAbsurd.length} (expected 500/5000)`
    );
  }
  ...
  if (flakyRequests === 3 && fetchedFlaky.length === 20) {
    ...
    fail(
      `jibeapply fetch partial failure: requests=${flakyRequests}, total=${fetchedFlaky.length} (expected 3/20)`
    );
  }
  if (jibeFlakyWarnings.some((w) => /page \d+ fetch failed/.test(w))) {
    ...
    fail(
      `jibeapply fetch page failure: expected a fetch-failed warning, got ${JSON.stringify(jibeFlakyWarnings)}`
    );
  }
  ...
  const brandedEntry = {
    name: "Widget Co",
    careers_url: "https://jobs.widgetco.com/jobs",
    api: "https://jobs.widgetco.com/api/jobs?internal=false",
  };
  await jibeapply.fetch(brandedEntry, {
    ...
    fetchJson: async (url) => {
      explicitApiUrl = url;
      return { totalCount: 3, count: 3, jobs: [{ title: "Dev", slug: "dev-1" }] };
    },
  });
    ...
    fetchJson: async (url) => {
      ...
      if (page === 1)
        return {
          totalCount: 15,
          count: 15,
          jobs: Array.from({ length: 10 }, (_, i) => ({ title: `J${i}`, slug: `j-${i}` })),
        };
      return {
        totalCount: 15,
        count: 15,
        jobs: Array.from({ length: 5 }, (_, i) => ({ title: `J2-${i}`, slug: `j2-${i}` })),
      };
    },
  });
  if (brandedRequests === 2 && fetchedBranded.length === 15) {
    ...
    fail(
      `jibeapply fetch iCIMS pattern: requests=${brandedRequests}, total=${fetchedBranded.length} (expected 2/15)`
    );
  }
  ...
  try {
    await jibeapply.fetch(
      { name: "X", careers_url: "https://jobs.example.com/jobs", api: "http://evil.com/api/jobs" },
      {
        fetchText: async () => "",
        fetchJson: async () => {
    ...
    if (/cannot derive API URL/i.test(e2.message))
      pass("jibeapply.fetch() throws when HTTP entry.api and non-jibeapply careers_url");
