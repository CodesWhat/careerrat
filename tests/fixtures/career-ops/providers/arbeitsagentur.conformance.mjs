...
try {
  const arbeitsagenturModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/arbeitsagentur.mjs")).href
  );
  ...
  const cfg = parseArbeitsagenturConfig({
    arbeitsagentur: {
      keywords: ["  ML Engineer  ", "", 7, "NLP"],
      wo: " Berlin ",
      umkreis: 999999,
      size: 0,
      days: -3,
      remoteNationwide: "yes",
    },
  });
  ...
  if (
    ...
    cfg.size === 1 &&
    cfg.days === 1 &&
    cfg.remoteNationwide === false
  ) {
    pass(
      "parseArbeitsagenturConfig clamps umkreis/size/days and treats non-true remoteNationwide as false"
    );
  } else {
  ...
  if (
    buildLocation([{ adresse: { ort: "Berlin", region: "BERLIN", land: "DEUTSCHLAND" } }]) ===
    "Berlin"
  ) {
    pass("buildLocation takes the city and omits Germany and the region enum");
  } else {
    fail(
      `buildLocation DE = ${JSON.stringify(buildLocation([{ adresse: { ort: "Berlin", region: "BERLIN", land: "DEUTSCHLAND" } }]))}`
    );
  }
  if (buildLocation([{ adresse: { ort: "Wien", land: "OESTERREICH" } }]) === "Wien, OESTERREICH") {
    ...
    fail(
      `buildLocation non-DE = ${JSON.stringify(buildLocation([{ adresse: { ort: "Wien", land: "OESTERREICH" } }]))}`
    );
  }
  ...
  if (
    buildLocation([
      { adresse: { ort: "Hamburg", land: "DEUTSCHLAND" } },
      { adresse: { ort: "Bremen" } },
    ]) === "Hamburg"
  ) {
    ...
    fail(
      `buildLocation multi = ${JSON.stringify(buildLocation([{ adresse: { ort: "Hamburg" } }, { adresse: { ort: "Bremen" } }]))}`
    );
  }
  if (
    buildLocation(null) === "" &&
  ...
  if (
    norm &&
    norm.title === "ML Engineer" &&
    norm.company === "ACME" &&
    norm.url ===
      "https://www.arbeitsagentur.de/jobsuche/jobdetail/" + encodeURIComponent("10000-123/4 X") &&
  ...
  if (
    normalizeJob({ stellenangebotsTitel: "No refnr" }) === null &&
    normalizeJob({ referenznummer: "x", stellenangebotsTitel: "" }) === null
  ) {
  ...
  if (fetched.length === 2 && !("refnr" in fetched[0]))
    pass("aa.fetch() dedups by reference number and strips it from output");
    ...
    { name: "AA", arbeitsagentur: { keywords: ["ML"], wo: "Berlin", remoteNationwide: true } },
    {
      fetchJson: async (url) => {
        ...
        return hasWo
          ...
          : page(
              v6("R", "ML Engineer (Remote)", "Hamburg"),
              v6("X", "Onsite ML Engineer", "Hamburg")
            );
      },
    }
  );
  if (
    calls === 2 &&
    remoteFetched.some((j) => j.url.endsWith("R")) &&
    !remoteFetched.some((j) => j.url.endsWith("X"))
  ) {
    pass("aa.fetch() remoteNationwide keeps remote-titled wide hits and drops onsite ones");
  } else {
    fail(
      `aa.fetch() remoteNationwide = ${calls} calls, ${JSON.stringify(remoteFetched.map((j) => j.url))}`
    );
  }

  // parseArbeitsagenturConfig — remoteMatch mode + remoteMaxPages (config-driven remote detection)
  const rcfg = parseArbeitsagenturConfig({
    arbeitsagentur: { keywords: ["ML"], remoteMatch: "filter", remoteMaxPages: 50 },
  });
  if (rcfg.remoteMatch === "filter" && rcfg.remoteMaxPages === 20) {
    ...
    fail(
      `parseArbeitsagenturConfig remoteMatch/maxPages = ${JSON.stringify({ m: rcfg.remoteMatch, p: rcfg.remoteMaxPages })}`
    );
  }
  const rdef = parseArbeitsagenturConfig({
    arbeitsagentur: { keywords: ["ML"], remoteMatch: "bogus" },
  });
  if (rdef.remoteMatch === "title" && rdef.remoteMaxPages === 1) {
    ...
    fail(
      `parseArbeitsagenturConfig remote defaults = ${JSON.stringify({ m: rdef.remoteMatch, p: rdef.remoteMaxPages })}`
    );
  }
    ...
    },
    {
      fetchJson: async (url) => {
        ...
        return Number(sp.get("page")) === 1
          ? page(
              // full page (== size) → pagination continues
  ...
  if (
    ...
    pagesSeen.has("2") &&
    munich &&
    TAG.test(munich.location)
  ) {
    pass(
      "aa.fetch() remoteMatch:filter sends homeoffice=nv_true, paginates, and tags titles that claim remote"
    );
  } else {
    fail(
      `aa.fetch() filter mode = ${JSON.stringify({ usedHomeoffice, pages: [...pagesSeen], munichLoc: munich?.location })}`
    );
  }
  if (stuttgart && !TAG.test(stuttgart.location) && stuttgart.location === "Stuttgart") {
    pass(
      "aa.fetch() leaves a nv_true hit untagged when its title does not claim remote (hybrid stays commute-filtered)"
    );
  } else {
  ...
  let noKw = false;
  try {
    await aa.fetch({ name: "AA", arbeitsagentur: {} }, mkCtx({}));
  } catch {
    noKw = true;
  }
  ...
  try {
    await aa.fetch(
      { name: "AA", arbeitsagentur: { keywords: ["ML"] } },
      {
        fetchJson: async () => {
