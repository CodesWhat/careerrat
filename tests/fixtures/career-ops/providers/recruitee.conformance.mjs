...
try {
  const recruiteeModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/recruitee.mjs")).href
  );
  ...
  const hit = recruitee.detect({
    name: "Channable",
    careers_url: "https://channable.recruitee.com",
  });
  ...
  const sample = {
    offers: [
      {
        title: "Senior PM",
        careers_url: "https://channable.recruitee.com/o/senior-pm",
        city: "Utrecht",
        country: "Netherlands",
        remote: false,
      },
      {
        title: "Backend Eng",
        url: "https://channable.recruitee.com/o/backend",
        city: "Amsterdam",
        country: "Netherlands",
        remote: true,
      },
  ...
  if (
    jobs[0]?.title === "Senior PM" &&
    jobs[0]?.company === "Channable" &&
    jobs[0]?.url === "https://channable.recruitee.com/o/senior-pm"
  ) {
  ...
  if (jobs[1]?.location === "Amsterdam, Netherlands, Remote") {
    ...
    fail(
      `row 1 location = ${JSON.stringify(jobs[1]?.location)}, expected "Amsterdam, Netherlands, Remote"`
    );
  }
  ...
  if (
    recruitee.detect({ name: "X", careers_url: null }) === null &&
    recruitee.detect({ name: "X", careers_url: 7 }) === null
  ) {
  ...
  if (
    recruitee.detect({
      name: "Spoof",
      careers_url: "https://evil.example/channable.recruitee.com/foo",
    }) === null
  ) {
  ...
  if (
    ...
    offerUrlOffers[2]?.url === "" &&
    offerUrlOffers[3]?.url === ""
  ) {
    pass("parseRecruiteeResponse keeps custom-domain https URLs, drops non-https and missing");
  } else {
    fail(
      `URL validation: row0=${JSON.stringify(offerUrlOffers[0]?.url)}, row1=${JSON.stringify(offerUrlOffers[1]?.url)}, row2=${JSON.stringify(offerUrlOffers[2]?.url)}, row3=${JSON.stringify(offerUrlOffers[3]?.url)}`
    );
  }
  ...
  const fetchJobs = await recruitee.fetch(
    { name: "Channable", careers_url: "https://channable.recruitee.com/" },
    {
      fetchJson: async (url, opts) => {
        ...
        return {
          offers: [
            { title: "Senior PM", careers_url: "https://channable.recruitee.com/o/senior-pm" },
          ],
        };
      },
    }
  ...
  if (
    fetchedUrl === "https://channable.recruitee.com/api/offers/" &&
    ...
    fail(
      `recruitee.fetch() url=${JSON.stringify(fetchedUrl)} opts=${JSON.stringify(fetchedOpts)} jobs=${fetchJobs.length}`
    );
  }
      ...
      offers: [
        {
          ...
          description:
            "&lt;p&gt;Build &lt;strong&gt;pipelines&lt;/strong&gt; at Hostaway&amp;rsquo;s HQ&lt;/p&gt;&lt;script&gt;evil()&lt;/script&gt;",
        },
  ...
  if (descOffers[0]?.description === "Build pipelines at Hostaway’s HQ") {
    ...
    fail(
      `parseRecruiteeResponse row 0 description = ${JSON.stringify(descOffers[0]?.description)}`
    );
  }
  if (!("description" in descOffers[1]) && !("description" in descOffers[2])) {
    ...
    fail(`parseRecruiteeResponse rows 1-2 = ${JSON.stringify(descOffers.slice(1))}`);
  }
} catch (e) {
