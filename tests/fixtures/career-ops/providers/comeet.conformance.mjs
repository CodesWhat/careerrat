...
try {
  const comeetModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/comeet.mjs")).href
  );
  ...
  if (
    apiHit &&
    apiHit.url === "https://www.comeet.co/careers-api/2.0/company/30.005/positions?token=REDACTED"
  ) {
  ...
  if (
    cuHit &&
    cuHit.url === "https://www.comeet.co/careers-api/2.0/company/30.005/positions?token=REDACTED"
  ) {
  ...
  if (
    comeet.detect({ name: "X", careers_url: "https://www.comeet.com/jobs/spark-hire/30.005" }) ===
    null
  ) {
  ...
  if (
    comeet.detect({ name: "X", careers_url: null }) === null &&
    comeet.detect({ name: "X", api: 7 }) === null
  ) {
  ...
  if (
    comeet.detect({
      name: "Spoof",
      api: "https://evil.example/www.comeet.co/careers-api/2.0/company/x/positions",
    }) === null
  ) {
  ...
  if (
    comeet.detect({
      name: "Spoof",
      api: "https://www.comeet.com/careers-api/2.0/company/x/positions?token=y",
    }) === null
  ) {
  ...
  if (
    jobs[0]?.title === "AI Engineer" &&
    jobs[0]?.company === "Spark Hire" &&
    jobs[0]?.location === "Tel Aviv, Israel, Remote"
  ) {
  ...
  if (
    jobs[1]?.url === "https://www.comeet.com/jobs/spark-hire/30.005/backend/AB.C12" &&
    jobs[1]?.location === "Berlin, Germany" &&
    jobs[1]?.postedAt === undefined
  ) {
  ...
  const noDup = parseComeetResponse(
    [
      {
        name: "R",
        url_active_page: "https://www.comeet.com/jobs/x/r",
        location: { name: "Remote, EMEA", is_remote: true },
      },
    ],
    "X"
  );
  if (noDup[0]?.location === "Remote, EMEA")
    pass("parseComeetResponse does not double-append Remote");
