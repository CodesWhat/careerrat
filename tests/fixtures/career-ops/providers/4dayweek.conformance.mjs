...
try {
  const fdwModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/4dayweek.mjs")).href
  );
  const { resolveProvider } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/_registry.mjs")).href
  );
  ...
  const careersHit = fourdayweek.detect({
    name: "4 Day Week",
    careers_url: "https://4dayweek.io/jobs",
  });
  const apiHit = fourdayweek.detect({ name: "4 Day Week", api: "https://4dayweek.io/api/jobs" });
  if (
    careersHit?.url === "https://4dayweek.io/api/jobs" &&
    apiHit?.url === "https://4dayweek.io/api/jobs"
  ) {
  ...
  const detectMisses = [
    ...
    fourdayweek.detect({
      name: "Path spoof",
      careers_url: "https://evil.example/4dayweek.io/jobs",
    }),
    fourdayweek.detect({
      name: "Suffix spoof",
      careers_url: "https://4dayweek.io.evil.example/jobs",
    }),
  ...
  const full = normalize4dwJob(
    {
      ...
      company_name: "  PanzerGlass  ",
      locations: [{ city: "Hinnerup", country: "Denmark" }],
  ...
  if (
    full &&
    full.title === "Financial Controller" &&
    full.url === "https://4dayweek.io/job/financial-controller-at-panzerglass-45369c18" &&
    full.company === "PanzerGlass" &&
    full.location === "Hinnerup, Denmark" &&
    full.postedAt === 1782731975 * 1000
  ) {
    pass(
      "normalize4dwJob maps title, builds /job/<slug> url, company_name, location, posted(seconds)→ms"
    );
  } else {
  ...
  const remoteJob = normalize4dwJob({
    title: "R",
    slug: "r-1",
    locations: [{ city: "Berlin", country: "Germany" }],
    work_arrangement: "remote",
  });
  if (remoteJob?.location === "Berlin, Germany, Remote")
    pass('normalize4dwJob appends "Remote" when work_arrangement is "remote"');
  ...
  if (
    coNested?.company === "Nested Co" &&
    coEntry?.company === "Entry Name" &&
    coDefault?.company === "4 Day Week" &&
    coBlank?.company === "4 Day Week"
  ) {
    pass(
      'normalize4dwJob falls back company → company.name → entry name → "4 Day Week" (whitespace-only ignored)'
    );
  } else {
    fail(
      `normalize4dwJob company fallbacks = ${JSON.stringify({ n: coNested?.company, e: coEntry?.company, d: coDefault?.company, b: coBlank?.company })}`
    );
  }
  ...
  const page1 = {
    jobs: Array.from({ length: 25 }, (_, i) => mk(i)),
    total: 50,
    page: 1,
    has_more: true,
  };
  const page2 = {
    jobs: [mk(25), mk(26), { title: "", slug: "bad" }],
    total: 50,
    page: 2,
    has_more: false,
  }; // has_more:false → stop; 1 drop
  const requested = [];
  ...
  if (requested.every((r) => r.redirect === "error"))
    pass('4dayweek.fetch() passes redirect:"error" on every page (SSRF guard)');
  else fail(`4dayweek.fetch() redirect opts = ${JSON.stringify(requested.map((r) => r.redirect))}`);

  if (paged.length === 27)
    pass(
      "4dayweek.fetch() aggregates valid jobs across pages (25 + 2, dropping the empty-title row)"
    );
  ...
  await fourdayweek.fetch(
    { name: "4 Day Week", max_pages: 1 },
    {
      fetchJson: async (url) => {
        capReq.push(url);
        return { jobs: Array.from({ length: 25 }, (_, i) => mk(i)), total: 999, has_more: true };
      },
    }
  );
