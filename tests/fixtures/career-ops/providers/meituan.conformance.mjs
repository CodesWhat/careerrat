...
try {
  const meituan = (
    await import(pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/meituan.mjs")).href)
  ).default;
  const { parseMeituanResponse } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/meituan.mjs")).href
  );

  if (meituan.id === "meituan") pass('meituan.id is "meituan"');
  else fail(`meituan.id is ${JSON.stringify(meituan.id)}`);

  const hit = meituan.detect({
    name: "美团",
  ...
  if (
    meituan.detect({ name: "X", careers_url: "https://evil.example/zhaopin.meituan.com" }) === null
  ) {
  ...
  if (
    meituan.detect({ name: "X", careers_url: "http://zhaopin.meituan.com/web/social" }) === null
  ) {
  ...
  if (jobs.length === 2)
    pass("parseMeituanResponse() keeps titled+ID posts, drops incomplete ones");
  else fail(`parseMeituanResponse() returned ${jobs.length} jobs, expected 2`);

  const j1 = jobs[0];
  if (
    j1 &&
    j1.url === "https://zhaopin.meituan.com/web/position/detail?jobUnionId=abc-123" &&
    j1.location === "北京市/上海市"
  ) {
  ...
  if (
    j1 &&
    j1.description.includes("部门: 核心本地商业-测试部") &&
    j1.description.includes("负责大模型训练。")
  ) {
  ...
  const mkJob = (id, title) => ({
    jobUnionId: String(id),
    name: title,
    cityList: [{ name: "北京市" }],
  });
  const mkCtx = (impl) => {
  ...
  const pagedJobs = await meituan.fetch(
    { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI"] },
    paged.ctx
  );
  ...
  if (paged.sleeps.length === 1 && paged.sleeps[0] > 0) {
    pass(
      "meituan.fetch() paces follow-up requests via ctx.sleep (no delay before the first request)"
    );
  } else {
    fail(`meituan.fetch() ctx.sleep calls: ${JSON.stringify(paged.sleeps)}`);
  }

  const flaky = mkCtx((_call, n) =>
    n === 2
  ...
  const flakyJobs = await meituan.fetch(
    { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI"] },
    flaky.ctx
  );
  if (flakyJobs.length === 120 && flaky.calls.length === 3) {
    pass(
      "meituan.fetch() retries an empty mid-pagination page instead of truncating (flake → retry → 120 jobs)"
    );
  } else {
    fail(
      `meituan.fetch() retry-on-empty: ${flakyJobs.length} jobs, ${flaky.calls.length} requests`
    );
  }
  ...
  const overlapJobs = await meituan.fetch(
    { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI", "大模型"] },
    overlap.ctx
  );
  if (overlapJobs.length === 1 && overlap.calls.length === 2 && overlap.sleeps.length === 1) {
    ...
    fail(
      `meituan.fetch() cross-keyword: ${overlapJobs.length} jobs, ${overlap.calls.length} requests, sleeps ${JSON.stringify(overlap.sleeps)}`
    );
  }
  ...
  const capped = mkCtx(() => ({
    data: {
      page: { totalCount: 500 },
      list: Array.from({ length: 100 }, (_, i) => mkJob(5000 + i, `岗位E${i}`)),
    },
  }));
  await meituan.fetch(
    { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI"], max_pages: 1 },
    capped.ctx
  );
  ...
  const probe = mkCtx(() => ({
    data: {
      page: { totalCount: 500 },
      list: Array.from({ length: 100 }, (_, i) => mkJob(6000 + i, `岗位F${i}`)),
    },
  }));
  ...
  if (probe.calls.length === 1 && probe.calls[0].keywords === "" && probeJobs.length === 100) {
    pass(
      "meituan.fetch() honors the ctx.maxPages probe hint and defaults to a whole-board (empty keyword) query"
    );
  } else {
    fail(
      `meituan.fetch() ctx.maxPages=1: ${probe.calls.length} requests, keywords=${JSON.stringify(probe.calls[0] && probe.calls[0].keywords)}`
    );
  }
  ...
  const blipJobs = await meituan.fetch(
    { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI", "大模型"] },
    blip.ctx
  );
  if (blipJobs.length === 1 && blipJobs[0].title === "幸存岗位") {
    ...
    fail(
      `meituan.fetch() partial results on failure: ${JSON.stringify(blipJobs.map((j) => j.title))}`
    );
  }
  ...
  try {
    zeroThenBlipJobs = await meituan.fetch(
      { name: "美团", careers_url: MEITUAN_URL, keywords: ["AI", "大模型"] },
      zeroThenBlip.ctx
    );
  } catch {
    zeroThenBlipThrew = true;
  }
  if (!zeroThenBlipThrew && Array.isArray(zeroThenBlipJobs) && zeroThenBlipJobs.length === 0) {
    pass(
      "meituan.fetch() treats a later failure as a blip even when earlier keywords matched 0 jobs (board is alive)"
    );
  } else {
    fail(
      `meituan.fetch() 0-result keyword then 503: ${zeroThenBlipThrew ? "threw" : JSON.stringify(zeroThenBlipJobs)}`
    );
  }
  ...
  if (firstFailThrew) {
    pass(
      "meituan.fetch() still throws when the very first request fails (dead board reads as failure)"
    );
  } else {
