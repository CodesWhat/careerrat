...
// board-wide aggregator provider.
//
...
const item = (name, url, description) =>
  ...
  "}";
const pageHtml = (items) =>
  `<html><body><script>{"itemListElement":[${items.join(",")}]}</script></body></html>`;
...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/builtin.mjs")).href
  );
  ...
  const flat = readConfig({
    queries: ["platform engineer"],
    categories: ["dev-engineering"],
    max_pages: 2,
  });
  if (
    flat.queries.join() === "platform engineer" &&
    flat.categories.join() === "dev-engineering" &&
    flat.maxPages === 2
  ) {
  ...
  const nested = readConfig({
    queries: ["flat-loses"],
    builtin: { queries: ["nested-wins"], max_pages: 4 },
  });
  ...
  if (parsed.length === 2) {
    pass(
      "parseListPage() parses valid ItemList rows and skips malformed / titleless ones (2 kept)"
    );
  } else {
    fail(`parseListPage kept ${parsed.length}: ${JSON.stringify(parsed.map((j) => j.title))}`);
  }
  if (
    parsed[0]?.title === "Platform Engineer" &&
    parsed[0]?.url === "https://builtin.com/job/1" &&
    parsed[0]?.description === "Own the platform." &&
    parsed[0]?.company === "" &&
    parsed[0]?.location === ""
  ) {
  ...
  if (
    parseListPage(null).length === 0 &&
    parseListPage(42).length === 0 &&
    parseListPage("<html></html>").length === 0
  ) {
  ...
  if (Array.isArray(nothing) && nothing.length === 0 && noCfg.calls.length === 0) {
    pass(
      "builtin.fetch() with NO queries/categories returns [] and makes ZERO requests (no baked-in personal defaults)"
    );
  } else {
    fail(
      `neutralization: ${nothing.length} jobs in ${noCfg.calls.length} requests (expected 0 / 0)`
    );
  }
  ...
  if (
    built.calls.some((c) => c.url === searchP1) &&
    built.calls.some((c) => c.url === catP1) &&
    jobsBuilt.length === 2
  ) {
    pass("builtin.fetch() builds /jobs?search=&page= and /jobs/<category>?page= URLs");
  } else {
    fail(
      `URL construction: ${built.calls.map((c) => c.url).join(" | ")} → ${jobsBuilt.length} jobs`
    );
  }
  ...
  const dedupCtx = mockCtx({
    [p1]: pageHtml([
      item("X", "https://builtin.com/job/x", "x"),
      item("Y", "https://builtin.com/job/y", "y"),
    ]),
    [p2]: pageHtml([
      item("Y", "https://builtin.com/job/y", "y"),
      item("Z", "https://builtin.com/job/z", "z"),
    ]), // Y dup, Z new
    [p3]: "", // empty → stop
  });
  ...
  const survived = await bi.fetch(
    { name: "B", queries: ["good", "bad"], max_pages: 3 },
    throwCtx.ctx
  );
  ...
  if (
    ...
    pass(
      'resolveHost() canonicalizes a bare/mixed-case market host to its www form (bare 301s, and we fetch redirect:"error")'
    );
  } else {
  ...
  if (
    resolveHost("evil.com") === null &&
    resolveHost("builtin.com.evil.com") === null &&
    resolveHost("notbuiltin.com") === null &&
    resolveHost(42) === null
  ) {
  ...
  if (badJobs.length === 0 && badHost.calls.length === 0) {
    pass(
      "builtin.fetch() REFUSES an unknown host — returns [] and makes zero requests (never falls back to builtin.com)"
    );
  } else {
  ...
  const seaCtx = mockCtx({
    [seaP1]: pageHtml([item("PE", "https://www.builtinseattle.com/job/pe/1", "x")]),
  });
  const seaJobs = await bi.fetch(
    {
      name: "B",
      builtin: { host: "builtinseattle.com", queries: ["platform engineer"] },
      max_pages: 1,
    },
    seaCtx.ctx
  );
  ...
  await bi.fetch(
    {
      name: "B",
      builtin: { scope: "remote", queries: ["a"], categories: ["dev-engineering"] },
      max_pages: 1,
    },
    scopeCtx.ctx
  );
  if (
    scopeCtx.calls.some((c) => c.url === remoteSearch) &&
    scopeCtx.calls.some((c) => c.url === remoteCat)
  ) {
    pass(
      "builtin.fetch() nests queries AND categories under an optional scope path (/jobs/remote/...)"
    );
  } else {
  ...
  if (
    byId["101"]?.company === "Acme Corp" &&
    byId["102"]?.company === "Globex" &&
    byId["103"]?.company === "Initech"
  ) {
    pass(
      "parseListPage() joins each card to its ItemList row on the job id and fills company (unblocks the blacklist gate)"
    );
  } else {
    fail(`company join: ${JSON.stringify(enriched.map((j) => j.company))}`);
  }
  if (
    byId["101"]?.location === "Hybrid · Seattle, WA, USA" &&
    byId["102"]?.location === "Remote · United States"
  ) {
    pass(
      "parseListPage() composes workplace mode + place into location (unblocks location_filter)"
    );
  } else {
  ...
  if (byId["104"]?.location === "") {
    pass(
      "parseCards() leaves an UNRESOLVABLE multi-location card empty — empty passes location_filter, so the row keeps flowing instead of being dropped on a placeholder"
    );
  } else {
    fail(`unresolvable multi-location should be '': ${JSON.stringify(byId["104"]?.location)}`);
  }
  if (
    byId["101"]?.salary?.min === 170000 &&
    byId["101"]?.salary?.max === 230000 &&
    byId["101"]?.salary?.currency === undefined
  ) {
    pass(
      "parseListPage() maps an Annually band to {min,max} and leaves currency unset (buildSalaryFilter only rejects when both sides declare one)"
    );
  } else {
  ...
  if (byId["102"]?.salary === undefined) {
    pass(
      'parseSalary() ignores builtin’s bogus "115K-130K Hourly" bands (their data error; reading it would be 1000x off)'
    );
  } else {
  ...
  if (
    parsePostedAt("Yesterday", NOW) === NOW - day &&
    ...
    fail(
      `parsePostedAt: ${["Yesterday", "Reposted 3 Days Ago", "2 Hours Ago", "30+ Days Ago", "An Hour Ago"].map((t) => parsePostedAt(t, NOW)).join(",")}`
    );
  }
  if (parsePostedAt("Whenever", NOW) === undefined && parsePostedAt(null, NOW) === undefined) {
    pass(
      'parsePostedAt() returns undefined for unrecognised text (scan.mjs then treats it as "no date" and passes)'
    );
  } else {
  ...
  if (
    composeLocation("Remote", []) === "Remote" &&
    composeLocation("Remote or Hybrid", []) === "Remote or Hybrid"
  ) {
  ...
  if (
    parseSalary("170K Annually")?.min === 170000 &&
    parseSalary("170K Annually")?.max === 170000
  ) {
  ...
  if (warnings.some((w) => /ZERO job cards/.test(w) && /INERT/.test(w))) {
    pass(
      "builtin.fetch() warns LOUDLY when a page yields rows but no cards (silent enrichment loss would re-open the off-policy leak)"
    );
  } else {
  ...
  const quietCtx = mockCtx({
    "https://builtin.com/jobs?search=a&page=1": pageHtml([
      item("One", "https://builtin.com/job/one/9", "x"),
    ]),
  });
  const quiet = [];
