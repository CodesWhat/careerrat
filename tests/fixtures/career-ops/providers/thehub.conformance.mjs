...
try {
  const thehubModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/thehub.mjs")).href
  );
  ...
  if (
    cfgDefault.countryCode === "EU" &&
    cfgDefault.includeRemote === false &&
    cfgOverride.countryCode === "DK" &&
    cfgOverride.includeRemote === true &&
    cfgBlank.countryCode === "EU" &&
    cfgBlank.includeRemote === false
  ) {
    pass(
      "parseThehubConfig defaults countryCode/includeRemote and honors portals.yml overrides (strict boolean true)"
    );
  } else {
  ...
  if (
    ...
    full.url === "https://thehub.io/jobs/abc123" &&
    full.company === "Acme Corp" &&
    full.location === "London, UK"
  ) {
  ...
  const assembled = normalizeHubJob(
    { title: "R", id: "r", location: { locality: "Berlin", country: "Germany" }, isRemote: true },
    "X"
  );
  if (assembled?.location === "Berlin, Germany, Remote")
    pass('normalizeHubJob assembles locality/country and appends "Remote" when isRemote');
  ...
  if (noLocRemote?.location === "Remote" && noLocOnsite?.location === "") {
    pass(
      'normalizeHubJob resolves a missing location to "Remote" (isRemote) or "" (not remote), never throwing'
    );
  } else {
    fail(
      `normalizeHubJob missing-location results = ${JSON.stringify({ remote: noLocRemote?.location, onsite: noLocOnsite?.location })}`
    );
  }
  ...
  if (
    ...
    coDefault?.company === "The Hub" &&
    coBlank?.company === "The Hub"
  ) {
    pass(
      'normalizeHubJob falls back company → entry name → "The Hub" (whitespace-only entry name ignored)'
    );
  } else {
    fail(
      `normalizeHubJob company fallbacks = ${JSON.stringify({ a: coEntry?.company, b: coDefault?.company, c: coBlank?.company })}`
    );
  }
  ...
  if (encoded?.url === "https://thehub.io/jobs/a%2Fb%20c")
    pass("normalizeHubJob URL-encodes the id");
  ...
  const hubPage1 = {
    jobs: {
      docs: Array.from({ length: 15 }, (_, i) => mk(i)),
      page: 1,
      pages: 3,
      total: 33,
      limit: 15,
  ...
  const hubPage2 = {
    jobs: { docs: [mk(15), mk(16), { title: "", id: "bad" }], page: 2, pages: 3, limit: 15 },
  }; // short → stop; 1 drop
  const requested = [];
  ...
  if (requested.every((r) => r.redirect === "error"))
    pass('thehub.fetch() passes redirect:"error" on every page (SSRF guard)');
  ...
  await thehub.fetch(
    { name: "The Hub", thehub: { countryCode: "DK" } },
    {
      fetchJson: async (url) => {
        dkReq.push(url);
        return { jobs: { docs: [], page: 1, pages: 1, limit: 15 } };
      },
    }
  );
  ...
  const withRemote = await thehub.fetch(
    { name: "The Hub", thehub: { includeRemote: true } },
    {
      fetchJson: async (url) => {
        remoteReq.push(url);
        return url.includes("isRemote=true") ? remoteDocs : regionDocs;
      },
    }
  );
  if (
    ...
    pass(
      "thehub.fetch() runs an isRemote=true pass after the region pass when includeRemote is set, deduping by url"
    );
  } else {
    fail(
      `thehub.fetch() includeRemote requested = ${JSON.stringify(remoteReq)}, returned ${withRemote.length} jobs (expected 3)`
    );
  }

  if (paged.length === 17)
    pass(
      "thehub.fetch() aggregates valid jobs across pages (15 + 2, dropping the empty-title row)"
    );
  ...
  const fullPage = (page) => ({
    jobs: {
      docs: Array.from({ length: 15 }, (_, i) => mk(page * 100 + i)),
      page,
      pages: 2,
      limit: 15,
    },
  ...
  if (
    capReq.length === 1 &&
    capReq[0] === "https://thehub.io/api/v2/jobsandfeatured?page=1&countryCode=EU"
  ) {
  ...
  await thehub.fetch(
    ...
    {
      fetchJson: async (url) => {
        defReq.push(Number(new URL(url).searchParams.get("page")));
        return fullDeepPage(defReq.length);
      },
    }
  );
  ...
  await thehub.fetch(
    ...
    {
      fetchJson: async (url) => {
        cap67Req.push(Number(new URL(url).searchParams.get("page")));
        return fullDeepPage(cap67Req.length);
      },
    }
  );
  ...
  if (firstFailThrew) {
    pass(
      "thehub.fetch() still throws when the very first request fails (dead board reads as failure)"
    );
  } else {
  ...
  if (blipReq.length === 2 && blipJobs.length === 15) {
    ...
    fail(
      `thehub.fetch() region-pass partial failure: requested ${blipReq.length}, returned ${blipJobs.length} jobs (expected 2 requests, 15 jobs)`
    );
  }
  ...
  if (remoteFailReq.length === 2 && remoteFailJobs.length === 1) {
    pass(
      "thehub.fetch() keeps region-pass jobs when the remote pass fails after the region pass already succeeded"
    );
  } else {
    fail(
      `thehub.fetch() remote-pass failure: requested ${remoteFailReq.length}, returned ${remoteFailJobs.length} jobs (expected 2 requests, 1 job)`
    );
  }
} catch (e) {
