...
try {
  const ashbyModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/ashby.mjs")).href
  );
  ...
  if (
    hit &&
    hit.url === "https://api.ashbyhq.com/posting-api/job-board/acme?includeCompensation=true"
  ) {
    pass(
      "ashby.detect() resolves jobs.ashbyhq.com/<slug> → posting-api URL with includeCompensation=true"
    );
  } else {
  ...
  if (
    ashby.detect({ name: "X" }) === null &&
    ashby.detect({ name: "X", careers_url: null }) === null
  ) {
  ...
  if (
    hitApi &&
    hitApi.url === "https://api.ashbyhq.com/posting-api/job-board/openai?includeCompensation=true"
  ) {
  ...
  if (
    ashby.detect({ name: "Evil", api: "https://evil.example/posting-api/job-board/acme" }) === null
  ) {
  ...
  if (
    ashby.detect({ name: "Insecure", api: "http://api.ashbyhq.com/posting-api/job-board/acme" }) ===
    null
  ) {
  ...
  const annual = parseCompensation({
    compensation: { interval: "1 YEAR", minValue: 90000, maxValue: 120000, currency: "usd" },
  });
  ...
  const hourly = parseCompensation({
    compensation: { interval: "1 HOUR", minValue: 50, maxValue: 70, currency: "USD" },
  });
  ...
  const noInterval = parseCompensation({
    compensation: { minValue: 80000, maxValue: 100000, currency: "EUR" },
  });
  ...
  if (
    ...
    pass(
      "parseCompensation() returns null when compensation is absent (job {}, null job, null comp)"
    );
  } else {
  ...
  if (
    parseCompensation({
      compensation: { interval: "1 YEAR", minValue: "", maxValue: null, currency: "USD" },
    }) === null
  ) {
  ...
  const coerced = parseCompensation({
    compensation: { interval: "1 YEAR", minValue: "-5", maxValue: "110000", currency: "usd" },
  });
  if (coerced && coerced.min === 110000 && coerced.max === 110000) {
    pass(
      "parseCompensation() coerces numeric strings, drops negatives, and collapses to the surviving bound"
    );
  } else {
    fail(`parseCompensation(coerced) = ${JSON.stringify(coerced)}`);
  }

  const swapped = parseCompensation({
    compensation: { interval: "1 YEAR", minValue: 120000, maxValue: 90000, currency: "USD" },
  });
  ...
  if (
    capturedUrl === "https://api.ashbyhq.com/posting-api/job-board/acme?includeCompensation=true"
  ) {
  ...
  if (fetched.length === 2) pass("ashby.fetch() returns one normalized row per job");
  else fail(`ashby.fetch() returned ${fetched.length} rows (expected 2)`);

  if (
    fetched[0]?.title === "Head of Applied AI" &&
    ...
    fetched[0]?.salary &&
    fetched[0].salary.min === 150000 &&
    fetched[0].salary.max === 180000 &&
  ...
  if (fetched[0]?.location === "Canada · Europe · Berlin · Germany")
    pass(
      'ashby.fetch() folds secondaryLocations (region/locality/country) into location, deduped, " · "-joined'
    );
  ...
  if (
    ...
    fetched[1]?.url === "" &&
    fetched[1]?.location === "" &&
    fetched[1]?.salary === null &&
    fetched[1]?.postedAt === undefined
  )
    pass(
      "ashby.fetch() tolerates a sparse job (empty strings, null salary, undefined postedAt for a bad date)"
    );
  ...
  for (const body of emptyCases) {
    ...
    if (!Array.isArray(out) || out.length !== 0) {
      emptyOk = false;
      fail(`ashby.fetch() body=${JSON.stringify(body)} → ${JSON.stringify(out)}`);
      break;
    }
  }
  ...
  if (
    recoverAttempts === 2 &&
    recovered.length === 1 &&
    recovered[0].title === "Recovered Role" &&
    recoverSleeps.length === 1 &&
    recoverSleeps[0] >= 1000
  ) {
    pass(
      "ashby.fetch() retries a failed request and recovers transparently (2 attempts, 1 backoff via ctx.sleep)"
    );
  } else {
    fail(
      `ashby.fetch() retry recovery: attempts=${recoverAttempts}, sleeps=${JSON.stringify(recoverSleeps)}, jobs=${JSON.stringify(recovered)}`
    );
  }
  ...
  try {
    await ashby.fetch(
      { name: "Acme", careers_url: "https://jobs.ashbyhq.com/acme" },
      {
        ...
        fetchJson: async () => {
          exhaustAttempts++;
          throw new Error(`boom #${exhaustAttempts}`);
        },
      }
    );
  ...
  if (
    pinnedUrl === "https://api.ashbyhq.com/posting-api/job-board/openai?includeCompensation=true"
  ) {
  ...
  try {
    ...
    if (!underiveFetchCalled && /cannot derive API URL for NoBoard/.test(e.message)) {
      pass(
        'ashby.fetch() throws "cannot derive API URL" before fetching for an undetectable entry'
      );
    } else {
      fail(
