...
try {
  ...
  const badRows = parseResults(
    "<html>" + badEntityCard + "</html>",
  ...
  const radPages = [
    html,
    "<html>" + card("111", "C", "Kiel, Germany") + "</html>",
    "<html></html>",
  ];
  let radCalls = 0;
  const radSeen = [];
...
<li>
  ...
  if (overshootJobs.length === 4 && warned === "4") {
    ...
    fail(
      `radancy truncation warning: returned ${overshootJobs?.length}, warned "${warned}" (expected 4 / "4")`
    );
  }
} catch (e) {
