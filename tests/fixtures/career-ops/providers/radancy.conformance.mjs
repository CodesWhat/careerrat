...
try {
  const radancyModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/radancy.mjs")).href
  );
  ...
  if (
    radListUrl({ api: "https://careers.munichre.com/en/search-jobs" }) ===
    "https://careers.munichre.com/en/search-jobs"
  )
    pass("radancy.resolveListUrl() keeps an explicit search-jobs URL");
  else fail("radancy.resolveListUrl() should keep the search-jobs URL");
  if (
    radListUrl({ careers_url: "https://careers.munichre.com/de/some-page" }) ===
    "https://careers.munichre.com/de/search-jobs"
  )
    pass("radancy.resolveListUrl() defaults to /{lang}/search-jobs");
  else
    fail(
      `radancy.resolveListUrl() default wrong: ${radListUrl({ careers_url: "https://careers.munichre.com/de/some-page" })}`
    );

  // detect — never auto-claims (branded hosts, wire explicitly).
  if (radancy.detect({ careers_url: "https://careers.munichre.com/en/search-jobs" }) === null)
    pass("radancy.detect() returns null (explicit wiring only)");
  ...
  const html =
    "<html>" +
    card("40548453568", "Innendienst f&#252;r Versicherungsagentur", "Bingen am Rhein, Germany") +
    card("40546200896", "Category Manager", "London, United Kingdom") +
    "</html>";
  ...
  if (rows[0]?.title === "Innendienst für Versicherungsagentur")
    pass("radancy.parseResults() decodes entities in titles");
  else fail(`radancy.parseResults() title wrong: ${JSON.stringify(rows[0]?.title)}`);
  if (
    rows[0]?.url === "https://careers.munichre.com/en/job/city/40548453568-slug/3193/40548453568" &&
    rows[0]?.location === "Bingen am Rhein, Germany"
  )
    pass("radancy.parseResults() builds absolute URLs and extracts the location span");
  else fail(`radancy.parseResults() url/loc wrong: ${JSON.stringify(rows[0])}`);
  if (
    parseResults("<html>no items</html>", "https://x").length === 0 &&
    parseResults(undefined, "https://x").length === 0
  )
    pass("radancy.parseResults() returns [] for item-less / non-string input");
  ...
  if (badRows.length === 1 && badRows[0].title === "Bad&#xD800;Entity")
    pass("radancy.parseResults() tolerates an invalid numeric entity (no RangeError crash)");
  else
    fail(
      `radancy.parseResults() should degrade a malformed entity to literal text, got: ${JSON.stringify(badRows)}`
    );
  ...
  const radCtx = {
    ...
    fetchText: async (url) => {
      radSeen.push(url);
      return radPages[radCalls++] ?? "<html></html>";
    },
  };
  const radJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    radCtx
  );
  if (radJobs.length === 3 && radCalls === 3)
    pass("radancy.fetch() paginates and stops on the first empty page");
  else fail(`radancy.fetch() returned ${radJobs.length} jobs after ${radCalls} calls`);
  if (radSeen[0]?.endsWith("?p=1") && radSeen[1]?.endsWith("?p=2"))
    pass("radancy.fetch() pages via ?p=N (1-based)");
  ...
  const partialJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    partialCtx
  );
  if (partialJobs.length === 2 && partialCalls === 2)
    pass("radancy.fetch() preserves jobs from earlier pages when a later page fetch throws");
  else
    fail(
      `radancy.fetch() partial-failure handling wrong: ${partialJobs.length} jobs after ${partialCalls} calls`
    );
  ...
  if (radFirstErr?.message === "tenant down")
    pass("radancy.fetch() throws when the first page fetch fails (dead board ≠ empty board)");
  ...
  const dupCtx = {
    sleep: async () => {},
    fetchText: async () => dupPages[dupCalls++] ?? "<html></html>",
  };
  const dupJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    dupCtx
  );
  if (dupJobs.length === 2 && dupCalls === 2)
    pass(
      "radancy.fetch() stops when a page brings only already-seen ids (fresh === 0), without appending duplicates"
    );
  else
    fail(
      `radancy.fetch() duplicate-page stop wrong: ${dupJobs.length} jobs after ${dupCalls} calls`
    );
  ...
  // ══════════════════════════════════════════════════════════════════════════
  const { parseLegacyResults, parseModernResults, buildFragmentUrl, readFragmentTotals } =
    radancyModule;
...
<li>
  ...
  if (parseModernResults(html, "https://careers.munichre.com").length === 2)
    pass("radancy.parseModernResults() still parses the search-results-list__item markup");
  else fail("radancy.parseModernResults() regressed on modern markup");
  if (parseModernResults(LEGACY_KP, "https://x").length === 0)
    pass("radancy.parseModernResults() does not claim legacy markup");
  ...
  if (uhgRows.length === 2)
    pass("radancy.parseLegacyResults() yields 2 UHG rows (save-job button not double-counted)");
  else
    fail(`radancy.parseLegacyResults() UHG count = ${uhgRows.length}: ${JSON.stringify(uhgRows)}`);
  if (uhgRows[0]?.title === "Patient Service Representative")
    pass("radancy.parseLegacyResults() takes the title from <h2>, excluding the req-number span");
  else fail(`radancy.parseLegacyResults() UHG title = ${JSON.stringify(uhgRows[0]?.title)}`);
  if (uhgRows[0]?.location === "Acton, Massachusetts")
    pass(
      'radancy.parseLegacyResults() reads .job-location (tolerates the trailing " 1" class token)'
    );
  else fail(`radancy.parseLegacyResults() UHG location = ${JSON.stringify(uhgRows[0]?.location)}`);
  if (
    uhgRows[0]?.url ===
    "https://careers.unitedhealthgroup.com/job/acton/patient-service-representative/34088/98479156752"
  )
    pass("radancy.parseLegacyResults() resolves the relative href against origin");
  else fail(`radancy.parseLegacyResults() UHG url = ${JSON.stringify(uhgRows[0]?.url)}`);
  if (uhgRows[1]?.title === "Principal Architect, Interoperability & Integration")
    pass("radancy.parseLegacyResults() decodes entities in legacy titles");
  ...
  if (
    kpRows.length === 1 &&
    kpRows[0].title === "Sales Representative II - Large Group" &&
    kpRows[0].location === "Denver, CO, Flexible, Full-time, Day"
  )
    pass("radancy.parseLegacyResults() handles the Kaiser variant (no div / class / req span)");
  ...
  if (parseResults(LEGACY_KP, "https://www.kaiserpermanentejobs.org").length === 1)
    pass("radancy.parseResults() falls back to the legacy parser");
  ...
  const junkCases = [
    ...
    () =>
      parseLegacyResults(
        '<a href="/not-a-job/x" data-job-id="1"><h2>T</h2></a>',
        "https://x.example"
      ),
    () =>
      parseLegacyResults(
        '<a href="/job/a/b/1/2" data-job-id="1"><h2></h2></a>',
        "https://x.example"
      ),
  ];
  let junkThrew = null;
  ...
  for (const f of junkCases) {
    try {
      junkCounts.push(f().length);
    } catch (e2) {
      junkThrew = e2.message;
      break;
    }
  }
  if (!junkThrew && junkCounts.every((n) => n === 0))
    pass("radancy.parseLegacyResults() rejects malformed rows without throwing");
  else
    fail(
      `radancy.parseLegacyResults() malformed: threw=${junkThrew} counts=${JSON.stringify(junkCounts)}`
    );

  if (
    parseLegacyResults(LEGACY_KP + LEGACY_KP, "https://www.kaiserpermanentejobs.org").length === 1
  )
    pass("radancy.parseLegacyResults() dedupes a repeated data-job-id");
  ...
  if (fragUrl.pathname === "/search-jobs/results")
    pass("radancy.buildFragmentUrl() targets /search-jobs/results");
  else fail(`radancy.buildFragmentUrl() path = ${fragUrl.pathname}`);
  if (fragUrl.searchParams.get("SearchResultsModuleName") === "Search Results")
    pass(
      "radancy.buildFragmentUrl() sends SearchResultsModuleName (omitting it silently returns an empty result set)"
    );
  else fail("radancy.buildFragmentUrl() must send SearchResultsModuleName");
  if (!fragUrl.searchParams.has("SearchFiltersModuleName"))
    pass(
      "radancy.buildFragmentUrl() omits SearchFiltersModuleName (sending it re-attaches an ~8MB facet blob per page)"
    );
  else fail("radancy.buildFragmentUrl() must NOT send SearchFiltersModuleName");
  if (
    fragUrl.searchParams.get("CurrentPage") === "3" &&
    fragUrl.searchParams.get("RecordsPerPage") === "100"
  )
    pass("radancy.buildFragmentUrl() sets CurrentPage and RecordsPerPage=100");
  else
    fail(
      `radancy.buildFragmentUrl() paging = ${fragUrl.searchParams.get("CurrentPage")}/${fragUrl.searchParams.get("RecordsPerPage")}`
    );

  // Totals drive pagination bounds.
  const totals = readFragmentTotals(LEGACY_UHG);
  if (totals.totalResults === 5889 && totals.totalPages === 59)
    pass("radancy.readFragmentTotals() reads data-total-results / data-total-pages");
  else fail(`radancy.readFragmentTotals() = ${JSON.stringify(totals)}`);
  if (
    readFragmentTotals("<div/>").totalPages === null &&
    readFragmentTotals(null).totalResults === null
  )
    pass("radancy.readFragmentTotals() returns nulls for missing / non-string input");
  ...
  const fragCtx = {
    ...
    fetchText: async () => {
      throw new Error("fetchText must not run when the fragment works");
    },
  };
  const fragJobs = await radancy.fetch(
    { name: "Optum", careers_url: "https://careers.unitedhealthgroup.com/search-jobs" },
    fragCtx
  );
  if (
    fragJobs.length === 2 &&
    fragJobs[0].company === "Optum" &&
    fragJobs[0].title === "Patient Service Representative"
  )
    pass("radancy.fetch() uses the JSON fragment transport and stamps company");
  else fail(`radancy.fetch() fragment jobs = ${JSON.stringify(fragJobs)}`);
  if (fragCalls.length && fragCalls.every((u) => u.includes("/search-jobs/results")))
    pass("radancy.fetch() never requests the heavy ?p=N page when the fragment works");
  ...
  const noJsonCtx = {
    sleep: async () => {},
    fetchText: async () => (noJsonCalls++ === 0 ? html : "<html></html>"),
  };
  const noJsonJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    noJsonCtx
  );
  if (noJsonJobs.length === 2)
    pass("radancy.fetch() works when ctx has no fetchJson (HTML transport)");
  ...
  const fbCtx = {
    sleep: async () => {},
    fetchJson: async () => {
      throw new Error("no fragment endpoint on this tenant");
    },
    fetchText: async () => (fbText++ === 0 ? html : "<html></html>"),
  };
  const fbJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    fbCtx
  );
  if (fbJobs.length === 2 && fbText > 0)
    pass("radancy.fetch() falls back to ?p=N when the fragment endpoint throws");
  else
    fail(`radancy.fetch() fragment-throw fallback = ${fbJobs.length} jobs, ${fbText} text calls`);
  ...
  const emptyFragJobs = await radancy.fetch(
    { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
    emptyFragCtx
  );
  if (emptyFragJobs.length === 2 && emptyText > 0)
    pass("radancy.fetch() falls back when the fragment parses to zero rows");
  else
    fail(
      `radancy.fetch() empty-fragment fallback = ${emptyFragJobs.length} jobs, ${emptyText} text calls`
    );
  ...
  // "unreachable" for a tenant it just talked to — it returns what it has.
  {
    ...
    try {
      zeroJobs = await radancy.fetch(
        { name: "Munich Re", api: "https://careers.munichre.com/en/search-jobs" },
        {
          ...
          fetchText: async () => {
            throw new Error("403 on the HTML page");
          },
        }
      );
    } catch (err) {
    ...
    if (!zeroErr && Array.isArray(zeroJobs) && zeroJobs.length === 0) {
      pass(
        "radancy.fetch() does not throw when the fragment resolved (zero rows) and the HTML fallback fails (fragment success = proof of life)"
      );
    } else {
      fail(
        `radancy.fetch() fragment proof-of-life wrong: ${zeroErr ? `threw "${zeroErr.message}"` : JSON.stringify(zeroJobs)}`
      );
    }
  }
  ...
  const rowFor = (id) =>
    `<li><a href="/job/c/s/1/${id}" data-job-id="${id}"><h2>T${id}</h2><span class="job-location">X</span></a></li>`;
  let overshootPage = 0;
  ...
  const overshootCtx = {
    ...
    fetchJson: async () => {
      ...
      return {
        results: `<section data-total-results="99" data-total-pages="9">${ids.map(rowFor).join("")}</section>`,
        hasJobs: true,
      };
    },
    fetchText: async () => {
  ...
  try {
    ...
    overshootJobs = await radancy.fetch(
      { name: "Overshoot", careers_url: "https://x.example/search-jobs", max_jobs: 4 },
      overshootCtx
    );
  } finally {
    console.error = realConsoleError;
  }
  const warned = (overshootWarnings.join(" ").match(/truncated at (\d+)/) || [])[1];
  if (overshootJobs.length === 4 && warned === "4") {
    pass("radancy truncation warning reports the returned count, not the pre-slice buffer");
  } else {
    fail(
      `radancy truncation warning: returned ${overshootJobs?.length}, warned "${warned}" (expected 4)`
    );
  }
  ...
  let malformedJobs;
  try {
    malformedJobs = await radancy.fetch(
      { name: "MalformedResults", careers_url: "https://t.example/search-jobs" },
      malformedCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  if (
    malformedJobs.length === 1 &&
    malformedWarnings.some((m) => m.includes("unexpected fragment response shape"))
  ) {
    pass(
      "radancy.fetch() throws on a non-string json.results instead of reading it as a natural end"
    );
  } else {
    fail(
      `radancy.fetch() malformed-results handling: jobs=${malformedJobs?.length} warnings=${JSON.stringify(malformedWarnings)}`
    );
  }
  ...
  let probeCapJobs;
  try {
    probeCapJobs = await radancy.fetch(
      { name: "ProbeCapped", careers_url: "https://s.example/search-jobs" },
      probeCapCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  if (probeCapJobs.length === 1 && probeCapPage === 1 && probeCapWarnings.length === 0) {
    ...
    fail(
      `radancy.fetch() probe-cap handling: jobs=${probeCapJobs?.length} pages=${probeCapPage} warnings=${JSON.stringify(probeCapWarnings)}`
    );
  }
  ...
  try {
    await radancy.fetch(
      { name: "ProbeError", careers_url: "https://r.example/search-jobs" },
      probeErrorCtx
    );
  } catch (err) {
  ...
  if (probeErrorCaught instanceof FakeProbeBudgetReached) {
    pass(
      "radancy.fetch() propagates a ctx.fetch* rejection unwrapped while probing (JSON transport)"
    );
  } else {
    fail(
      `radancy.fetch() probe-error propagation (JSON): ${probeErrorCaught?.constructor?.name || probeErrorCaught}`
    );
  }
  ...
  try {
    await radancy.fetch(
      { name: "ProbeErrorHtml", api: "https://qh.example/search-jobs" },
      probeErrorHtmlCtx
    );
  } catch (err) {
  ...
  if (probeErrorHtmlCaught instanceof FakeProbeBudgetReached) {
    pass(
      "radancy.fetch() propagates a ctx.fetch* rejection unwrapped while probing (HTML transport)"
    );
  } else {
    fail(
      `radancy.fetch() probe-error propagation (HTML): ${probeErrorHtmlCaught?.constructor?.name || probeErrorHtmlCaught}`
    );
  }
  ...
  let exactJobs;
  try {
    exactJobs = await radancy.fetch(
      { name: "ExactFit", careers_url: "https://u.example/search-jobs", max_jobs: 3 },
      exactCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  if (exactJobs.length === 3 && exactWarnings.length === 0) {
    ...
    fail(
      `radancy.fetch() exact-max_jobs handling: jobs=${exactJobs?.length} warnings=${JSON.stringify(exactWarnings)}`
    );
  }
  ...
  const redirectJsonCtx = {
    ...
    fetchJson: async (u, o) => {
      ...
      return page === 1
        ? {
            results: `<section data-total-results="4" data-total-pages="2">${rowFor(1)}${rowFor(2)}</section>`,
            hasJobs: true,
          }
        : {
            results: `<section data-total-results="4" data-total-pages="2">${rowFor(3)}</section>`,
            hasJobs: true,
          };
    },
    fetchText: async () => {
  ...
  await radancy.fetch(
    { name: "RedirectCheckJson", careers_url: "https://redirect-json.example/search-jobs" },
    redirectJsonCtx
  );
  ...
  await radancy.fetch(
    { name: "RedirectCheckHtml", careers_url: "https://redirect-html.example/search-jobs" },
    redirectHtmlCtx
  );
  ...
  if (
    cbUrl1.searchParams.get("_") &&
    cbUrl1.searchParams.get("_") !== cbUrl2.searchParams.get("_")
  ) {
    pass("radancy.buildFragmentUrl() appends a per-call random cache-buster");
  } else {
    fail(
      `radancy.buildFragmentUrl() cache-buster missing or not unique: ${cbUrl1.searchParams.get("_")} / ${cbUrl2.searchParams.get("_")}`
    );
  }
  ...
  let overstateJobs;
  try {
    overstateJobs = await radancy.fetch(
      { name: "Understated", careers_url: "https://y.example/search-jobs" },
      overstateCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  if (overstateJobs.length === 1 && overstateWarnings.length === 0) {
    pass(
      "radancy.fetch() does not warn when a natural page-1 end falls short of an inflated totalResults"
    );
  } else {
    fail(
      `radancy.fetch() overstated-total handling: ${overstateJobs?.length} jobs, warnings=${JSON.stringify(overstateWarnings)}`
    );
  }
  ...
  let genuineCapJobs;
  try {
    genuineCapJobs = await radancy.fetch(
      { name: "CappedShort", careers_url: "https://z.example/search-jobs", max_pages: 2 },
      genuineCapCtx
    );
  } finally {
    console.error = realConsoleError;
  }
  const genuineCapWarned = capWarnings.some((m) => /raise max_jobs\/max_pages/i.test(m));
  if (genuineCapJobs.length === 4 && capPage === 2 && genuineCapWarned) {
    pass(
      "radancy.fetch() warns when max_pages caps the walk short of a larger source-reported totalPages"
    );
  } else {
    fail(
      `radancy.fetch() genuine cap handling: jobs=${genuineCapJobs?.length} pages=${capPage} warned=${genuineCapWarned}`
    );
  }
  ...
  let jsonErrorJobs;
  try {
    jsonErrorJobs = await radancy.fetch(
      { name: "MidWalkError", careers_url: "https://w.example/search-jobs" },
      jsonErrorCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  const noCapAdvice = !errWarnings.some((m) => /raise max_jobs\/max_pages/i.test(m));
  if (jsonErrorJobs.length === 1 && errLogged && noCapAdvice) {
    pass(
      "radancy.fetch() keeps earlier pages and logs the fetch error on a mid-walk JSON failure, without cap advice"
    );
  } else {
    fail(
      `radancy.fetch() mid-walk JSON error handling: jobs=${jsonErrorJobs?.length} warnings=${JSON.stringify(errWarnings)}`
    );
  }
  ...
  let transientJobs;
  try {
    transientJobs = await radancy.fetch(
      { name: "TransientBlip", careers_url: "https://v.example/search-jobs" },
      transientCtx
    );
  } finally {
    console.error = realConsoleError;
  ...
  if (transientJobs.length === 3 && attemptsOnPage2 === 2 && transientWarnings.length === 0) {
    pass(
      "radancy.fetch() recovers a page that fails once transiently, via fetchJsonWithRetry, with no data lost and no warning"
    );
  } else {
    fail(
      `radancy.fetch() transient-recovery handling: jobs=${transientJobs?.length} attempts=${attemptsOnPage2} warnings=${JSON.stringify(transientWarnings)}`
    );
  }
} catch (e) {
