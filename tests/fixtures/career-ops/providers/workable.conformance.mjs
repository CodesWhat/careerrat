...
try {
  const workableModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/workable.mjs")).href
  );
  ...
  const hit = workable.detect({
    name: "TestCo",
    careers_url: "https://apply.workable.com/acme-widgets",
  });
  if (
    hit &&
    hit.url === "https://apply.workable.com/api/v1/widget/accounts/acme-widgets?details=true"
  ) {
  ...
  if (
    jobs[0]?.title === "Senior AI PM" &&
    jobs[0]?.location === "Ghent, Belgium" &&
    jobs[0]?.company === "AcmeWidgets"
  ) {
  ...
  const widgetPayload = {
    ...
    jobs: [
      ...
      {
        title: "Evil Role",
        shortlink: "https://evil.example/j/X",
        url: "https://evil.example/j/X",
      },
      // dropped: no title
  ...
  if (widgetJobs.length === 2) pass("parseWorkableWidget keeps only valid, on-domain, titled jobs");
    ...
    fail(
      `parseWorkableWidget returned ${widgetJobs.length} jobs, expected 2: ${JSON.stringify(widgetJobs.map((j) => j.title))}`
    );
  ...
  if (widgetJobs[0]?.postedAt === Date.parse("2026-04-01"))
    pass("parseWorkableWidget maps published_on → postedAt");
    ...
    { name: "Smoke", careers_url: "https://apply.workable.com/acme-widgets" },
    {
      ...
      fetchJson: async (url, opts) => {
        ...
        if (opts?.redirect !== "error")
          throw new Error(`expected redirect:'error', got ${JSON.stringify(opts?.redirect)}`);
        return widgetPayload;
      },
      fetchText: async () => {
        throw new Error("markdown feed should not be used when the API succeeds");
      },
    }
  );
  ...
  if (
    apiCallHeaders?.["user-agent"] &&
    apiCallHeaders.referer === "https://apply.workable.com/acme-widgets/"
  ) {
  ...
  const recoveredJobs = await workable.fetch(
    { name: "Smoke", careers_url: "https://apply.workable.com/acme-widgets" },
    {
      ...
      fetchText: async () => {
        throw new Error("markdown feed should not be used once the API recovers");
      },
    }
  );
  ...
  if (giveUpAttempts === 1 && giveUpJobs.length === 1 && giveUpJobs[0].title === "Feed Role") {
    pass(
      "workable.fetch() gives up immediately on an hours-long Retry-After and falls back to the markdown feed"
    );
  } else {
  ...
  const makeSlowCtx = () => ({
    ...
    fetchText: async () => {
      throw new Error("should not reach markdown feed");
    },
  });
  await Promise.all([
    workable.fetch(
      { name: "A", careers_url: "https://apply.workable.com/acme-widgets" },
      makeSlowCtx()
    ),
    workable.fetch(
      { name: "B", careers_url: "https://apply.workable.com/acme-widgets" },
      makeSlowCtx()
    ),
  ]);
  ...
  if (
    workable.detect({
      name: "Spoof",
      careers_url: "https://evil.example/apply.workable.com/slug",
    }) === null
  ) {
  ...
  if (
    strayJobs.length === 1 &&
    strayJobs[0].url === "https://apply.workable.com/x/jobs/view/PIPE"
  ) {
    pass(
      "parseWorkableMarkdown extracts URL from line-level regex (survives stray pipes in title)"
    );
  } else {
  ...
  if (filteredJobs.length === 1 && filteredJobs[0].title === "Good Role") {
    ...
    fail(
      `expected only "Good Role" through, got ${JSON.stringify(filteredJobs.map((j) => j.title))}`
    );
  }
} catch (e) {
