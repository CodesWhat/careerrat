...
try {
  const manfredModule = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/manfred.mjs")).href
  );
  ...
  if (manfred.detect({ name: "X" }) === null)
    pass("manfred.detect() returns null without provider:manfred");
  ...
  if (
    resolveLang({ lang: "es" }) === "ES" &&
    resolveLang({ lang: "FR" }) === "EN" &&
    resolveLang({}) === "EN"
  ) {
    pass("resolveLang accepts EN/ES case-insensitively and falls back to EN");
  } else {
    fail(
      `resolveLang drift: ${resolveLang({ lang: "es" })}/${resolveLang({ lang: "FR" })}/${resolveLang({})}`
    );
  }
  ...
  if (
    ...
    fail(
      `placeless location drift: ${JSON.stringify([placeless(100), placeless(60), placeless(40), placeless(0)])}`
    );
  }
  if (
    ...
    fail(
      `multi-location join => ${JSON.stringify(resolveLocation({ locations: ["Paris, France", "Barcelona, Spain"], remotePercentage: 100 }))}`
    );
  }
  ...
  if (
    ...
    normalizeCurrency(" €") === "EUR" &&
    normalizeCurrency("£") === "GBP" &&
    normalizeCurrency("US$") === "USD" &&
    normalizeCurrency("MXN$") === "MXN" &&
    normalizeCurrency("EUR") === "EUR"
  ) {
    pass("normalizeCurrency maps the observed symbols (spacing variants included) to ISO codes");
  } else {
    fail(
      `normalizeCurrency drift: ${JSON.stringify(["€", " €", "£", "US$", "MXN$", "EUR"].map(normalizeCurrency))}`
    );
  }
  ...
  if (
    JSON.stringify(parseCompensation({ salaryFrom: 40000, currency: "€" })) ===
    JSON.stringify({ min: 40000, max: 40000, currency: "EUR" })
  ) {
    pass("a one-sided salary range is mirrored into min and max");
  } else {
    fail(
      `one-sided salary => ${JSON.stringify(parseCompensation({ salaryFrom: 40000, currency: "€" }))}`
    );
  }
  if (
    parseCompensation({ salaryFrom: 0, salaryTo: 0 }) === null &&
  ...
  if (normalizeManfredOffer({ ...activeOffer, position: "   " }) === null)
    pass("normalizeManfredOffer drops a title-less offer");
  else fail("title-less offer should be dropped");
  if (normalizeManfredOffer({ ...activeOffer, id: undefined }) === null)
    pass("normalizeManfredOffer drops an offer with no id");
  else fail("id-less offer should be dropped");
  if (normalizeManfredOffer({ ...activeOffer, slug: "" }) === null)
    pass("normalizeManfredOffer drops an offer with no slug");
  ...
  if (bare?.company === "EntryName")
    pass("normalizeManfredOffer falls back to the entry name for company");
  ...
  const ctx = {
    ...
    fetchJson: async (url, options) => {
      ...
      if (new URL(url).hostname !== "www.getmanfred.com")
        throw new Error(`fetchJson off-host: ${url}`);
  ...
  if (
    calls.length === 1 &&
    calls[0] === "https://www.getmanfred.com/api/v2/public/offers?lang=EN"
  ) {
  ...
  try {
    await manfred.fetch(
      { name: "getManfred", provider: "manfred" },
