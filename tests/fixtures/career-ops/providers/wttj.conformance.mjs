...
try {
  const mod = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/wttj.mjs")).href
  );
  ...
  if (
    wttj.detect({ name: "X", careers_url: "https://www.welcometothejungle.com/en/jobs" }) === null
  ) {
    pass(
      "wttj.detect() does not URL-autodetect (explicit provider: wttj only — the board is global)"
    );
  } else {
  ...
  if (secured.apiKey === securedKey) {
    pass(
      "parseEnvPayload() accepts a long non-hex (secured/base64) client key — length bounds only"
    );
  } else {
  ...
  if (
    ...
    j1.url ===
      "https://www.welcometothejungle.com/en/companies/example-sas/jobs/senior-data-engineer_abc123" &&
  ...
  if (j1 && j1.location === "Paris, France, Remote") {
    pass(
      "normalizeWttjHit() joins first-office city+country and appends Remote for fulltime-remote posts"
    );
  } else {
  ...
  if (monthly && monthly.salary && monthly.salary.min === 50000 && monthly.salary.max === 50000) {
    pass(
      "normalizeWttjHit() ignores a non-yearly salary_maximum (keeps only the annualized minimum)"
    );
  } else {
  ...
  if (
    bare &&
    bare.company === "Welcome to the Jungle" &&
    bare.location === "" &&
    bare.salary === undefined &&
    bare.postedAt === undefined
  ) {
  ...
  if (
    q1.hitsPerPage === "100" &&
    happy.jsonCalls.map((c) => c.query).join(",") === "finops,snowflake"
  ) {
    pass("wttj.fetch() defaults to 100 hits per query and passes each search term through");
  } else {
    fail(
      `wttj.fetch() hitsPerPage=${q1.hitsPerPage}, queries=${happy.jsonCalls.map((c) => c.query).join(",")}`
    );
  }

  const capped = mkCtx(ENV_OK, () => ({ hits: [] }));
  await wttj.fetch(
    { name: "WTTJ", provider: "wttj", wttj: { queries: ["x"], max_hits: 500 } },
    capped.ctx
  );
  ...
  if (noQueriesErr.includes("wttj: the WTTJ board is global")) {
    pass(
      "wttj.fetch() throws with neither wttj.queries nor wttj.filters (never scans the whole board)"
    );
  } else {
  ...
  if (
    filtered.jsonCalls.length === 1 &&
    filtered.jsonCalls[0].filters === FILTER &&
  ...
  if (
    both.jsonCalls.length === 2 &&
    both.jsonCalls.every((c) => c.filters === FILTER) &&
    both.jsonCalls.map((c) => c.query).join(",") === "a,b"
  ) {
    pass("wttj.fetch() applies wttj.filters to every configured query");
  } else {
    fail(
      `wttj.fetch() filters+queries = ${JSON.stringify(both.jsonCalls.map((c) => [c.query, c.filters]))}`
    );
  }
  ...
  await wttj.fetch(
    { name: "WTTJ", provider: "wttj", wttj: { queries: ["x"], max_hits: 5000 } },
    unfilteredCap.ctx
  );
  if (unfilteredCap.jsonCalls[0].hitsPerPage === "200") {
    ...
    fail(
      `wttj.fetch() unfiltered max_hits=5000 → hitsPerPage=${unfilteredCap.jsonCalls[0].hitsPerPage}`
    );
  }
  ...
  if (filteredCap.jsonCalls[0].hitsPerPage === "1000") {
    ...
    fail(
      `wttj.fetch() filtered max_hits=5000 → hitsPerPage=${filteredCap.jsonCalls[0].hitsPerPage}`
    );
  }
  ...
  if (blankFilter.jsonCalls[0].hitsPerPage === "200" && blankFilter.jsonCalls[0].filters === null) {
    pass(
      "wttj.fetch() treats a blank wttj.filters as absent (cap stays 200, no filters param sent)"
    );
  } else {
    fail(
      `wttj.fetch() blank filters → hitsPerPage=${blankFilter.jsonCalls[0].hitsPerPage}, filters=${JSON.stringify(blankFilter.jsonCalls[0].filters)}`
    );
  }
  ...
  try {
    ...
    badShapeErr = err.message;
  }
  if (
    badShapeErr.includes("wttj: unexpected Algolia response") &&
    badShapeErr.includes("expected { hits: [...] }")
  ) {
    pass("wttj.fetch() throws on an Algolia response without a hits array");
  } else {
    fail(
      `wttj.fetch() malformed-response error = ${JSON.stringify(badShapeErr) || "did not throw"}`
    );
  }
} catch (e) {
