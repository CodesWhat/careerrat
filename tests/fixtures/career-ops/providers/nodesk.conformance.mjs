...
try {
  const { default: nodesk, parseNodeskFeed } = await import(
    pathToFileURL(join(ROOT, "src/core/providers/career-ops/vendor/nodesk.mjs")).href
  );
  ...
  if (nodesk.detect({ provider: "nodesk" })?.url === "https://nodesk.co/remote-jobs/index.xml") {
  ...
  const fetched = await nodesk.fetch(
    {},
    {
      async fetchText(url, opts) {
        ...
        if (url !== "https://nodesk.co/remote-jobs/index.xml" || opts?.redirect !== "error") {
