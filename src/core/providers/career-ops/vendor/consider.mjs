...
//
// CareerRat-local divergence: goes through ctx.fetchResponse (the same
// SSRF-guarded transport career-ops-registry.mjs injects for the POST below)
// instead of the global `fetch`. resolveOrigin() above only screens the
// hostname as a string — literal IPs, localhost, .internal/.local suffixes —
// it never resolves DNS, so a public-looking hostname that resolves to a
// private/loopback/metadata address would otherwise reach that address
// unchecked. Routing through ctx.fetchResponse gets the full guard: DNS
// resolution, non-public-address screening, and address-pinned connection,
// same as every other Career Ops provider request. The registry's own
// request() already reads and discards the body on a non-ok response before
// throwing (see career-ops-registry.mjs), so a non-2xx handshake response
// never leaks an undrained body — that thrown error is exactly what the catch
// below degrades to a null/null handshake, same observable behaviour as
// before.
async function acquireCsrfHandshake(origin, ctx) {
  try {
    ...
    // followed — and the catch below treats the resulting error as a
