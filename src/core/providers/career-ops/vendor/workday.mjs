...
// CareerRat-local extension (not part of the upstream provider): resolves a
// specific posting URL (`.../job/{site}/.../job/{title-slug}_{reqId}`) to its
// CXS detail endpoint, so fetchDetail below can hydrate the exact posting
// instead of re-scanning the whole board for a matching row.
...
// CareerRat-local extension (not part of the upstream provider): the same
// browser-clearing header set fetch() below builds inline, shared so
// fetchDetail's exact-posting request clears the same Cloudflare bot
// management (see fetch()'s doc comment) without drifting from it.
...
export default {
  ...
  // CareerRat-local extension (not part of the upstream provider): exact
  // Workday requisition detail hydration for the URL-intake path
  // (src/core/intake/resolve.mjs), returning the canonical company/title/
  // location/full JD body for one already-matched board row instead of the
  // board-listing fields alone.
