...
export function packetPromptFromContext(context) {
  ...
  return [
    ...
    "Before scoring, build requirements: a table of the distinct requirements and qualifications actually named in the job description (max 20 rows, deduped). For each row set requirement (a short noun phrase), importance (critical, high, meaningful, preferred, or low_signal), evidence (stated when the job description says must/required/need, structural when it is implied by the title, level, or team, or inferred when it is your judgment call), jdSignal (the exact job-description phrase, verbatim, under 160 characters, that justified the importance tier), match (strong, partial, missing, or na for how well the candidate context evidences it), and note (one plain sentence on how the candidate's evidence maps to the requirement, or why match is na). Never invent a requirement the job description does not name.",
    "Derive fitRisks from the requirements table, not separately: every fitRisks entry must name a requirements row whose match is missing or partial and whose importance is critical or high.",
