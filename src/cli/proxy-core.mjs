...
// testers are configured or which one matched. Returns the matched entry's
// label + the raw provided token (for reportingUserId() downstream), or null
// on failure — writing the 401 response is each front end's own job. Never
// logs the token.
export async function authenticate(headers, tokenEntries) {
  ...
  if (matchedLabel) return { label: matchedLabel, token: provided };
