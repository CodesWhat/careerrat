...
| `coach-gaps` | REVIEW-gate fit risks open a typed coaching plan with evidence-grounded claim proposals or an explicit no-close path; confirmed evidence still crosses the canonical evidence firewall before re-evaluation | native | Source routing and contract coverage are current. Exact Claude/Codex click, ordinary-text, restart, and packaged acceptance remain release gates rather than historical passes. |
...
| `apply-job` | Ask, deterministic/public and rendered question capture, automatic Orca or Playwright fill, supervised Submit boundary, confirmation evidence, and manual fallback | native | Multi-step advancement is portal-agnostic: one step loop drives LinkedIn Easy Apply's modal and generic paginated ATS forms (Workday-style wizards, SmartRecruiters' stepped upload) alike, confirming a real page-to-page advance from the page's own re-rendered fields rather than a click that merely didn't error, and blocking (never guessing) on a validation stall, a page-specific required field with no resolvable answer, or a review page with no safe advance control. The Playwright executor bridge (PRs #106, #112) and honest extension not-available messaging (PR #108) already shipped. Preserve the manual Submit boundary and verified-only Applied write-back. |
...
| `ingest-mail` | Originally receipt-only; the current in-app path now reads Apple Mail on macOS and permitted Gmail/Outlook webmail, captures relevant communications and local body artifacts, advances source watermarks, and returns visible auth/permission retry states | native | Browser fixtures and source contracts cover the native write boundary. Exact packaged Apple Mail plus signed-in Gmail/Outlook acceptance remains unverified; single candidate-reported messages stay with email-comms. |
| `ingest-messages` | Originally receipt-only; the current in-app path now reads permitted LinkedIn/Wellfound messages, captures relevant communications and local body artifacts, advances platform watermarks, and returns visible auth/permission retry states | native | Browser fixtures and source contracts cover the native write boundary. Exact packaged signed-in LinkedIn/Wellfound acceptance remains unverified; reply drafting stays with email-comms. |
| `sync-status` | Originally a portal-read handoff; the current in-app path now reads permitted Greenhouse/Workday/Ashby/Lever dashboards, normalizes deterministically, atomically applies only `autoApplicable` transitions, and leaves regressions or low-confidence labels for review | native | Source and browser-fixture coverage proves application/CTA/draft/activity/analytics atomicity. Packaged signed-in ATS acceptance remains unverified. Terminal/external runs still hand confirmed transitions to track-outcomes, which owns candidate-reported learning, coaching, and follow-up semantics. |
| `relationship-sourcing` | Originally a durable handoff; the current in-app path now searches permitted LinkedIn/Wellfound surfaces and writes review-only leads into Network while preserving the durable retry CTA on auth blockers | native | Source and browser-fixture coverage proves review-only capture and consent handling. Exact packaged signed-in people-search acceptance remains unverified; candidate-found contacts remain ungated and outreach stays with email-comms. |
| `research-company` | Ask resolves natural company requests ("research Acme") to a fresh cached dossier or an embedded research session, cited and linked to the company | native | Headed isolated-home acceptance passed 2026-08-15. Packaged/live-AI acceptance on 2026-08-17 found and fixed the installed-CLI skill-load regression (`F-103`/PR #84); the session was then blocked by the installed-runtime's 120s timeout on long turns until PR #92 landed. Unblocked and re-run 2026-08-19 against a live installed Claude Code CLI: a real WebSearch/WebFetch research run completed inside the 9-minute session timeout with no truncation, saved through "Save to workspace" (`workspace/research/*.md`, the same `research.record` intent the UI calls), and read back correctly after a full server kill-and-restart. |
| `research-comp` | Ask resolves natural comp requests ("market comp for a nurse in Denver") to a fresh cached benchmark or an embedded research session, cited to role and location | native | Headed isolated-home acceptance passed 2026-08-15. Packaged/live-AI acceptance on 2026-08-17 found and fixed the installed-CLI skill-load regression (`F-103`/PR #84); missing-role/location recovery copy passed cleanly (deterministic, pre-session), and the full-benchmark session was blocked by the same installed-runtime timeout as research-company until PR #92 landed. Unblocked and re-run 2026-08-19 against a live installed CLI, saved and read back across a restart. One footnote, not a defect: the AskBar's free-text classifier did not reliably surface an ACTION chip for research.comp-shaped phrasing during that retest, worked around by calling the `research.comp` intent directly (the same code path the chip triggers, which correctly reused the live session instead of spawning a second one). |
| `optimize-linkedin` | Originally a proposal handoff; the current in-app path now reads the permitted LinkedIn profile, creates an evidence-grounded proposal batch, and supports stale-safe per-surface approve/reject review. Approval stays local | native | Source and browser-fixture coverage proves profile read, proposal creation, and local review. Exact packaged signed-in profile-read acceptance remains unverified. Live edits still require separate `profile_apply` permission, per-field confirmation, and verification on the terminal/external STEP 5–6 path. |
...
new evidence against the write path). research-company and research-comp reached
real six-axis research instead of failing immediately, but were separately blocked
by the installed-runtime's 120s default timeout on long research turns, observed
directly via SSE ("Installed AI request timed out.") on two consecutive
full-research turns and one full-benchmark turn. PR #92 widened the chat-session
timeout to 9 minutes and has landed. All three rows are now closed: the
2026-08-19 re-run completed real WebSearch/WebFetch turns inside the widened
timeout with no truncation, wrote through the same `research.record` intent the
UI calls, and read every result back after a full server kill-and-restart. The
demo workspace's fictional company name correctly failed to resolve on the open
web, which is the honesty firewall refusing to fabricate rather than a bug, so
the research turns were redirected to a real public company while candidate
identity and comp figures stayed synthetic.
