...
All content in this section is copied from `.planning/phases/ROL-API-06-canonical-db-app-shell/06-CONTEXT.md`. [VERIFIED: 06-CONTEXT.md]
...
- In DB workspaces, `rolester data status` exit 0 means tracker-visible mutations must go through `rolester data <verb>` and generated files must not be hand-edited. [VERIFIED: AGENTS.md]
...
| `rolester` global CLI | Manual workspace commands | no | - | Use `node src/cli/*.mjs` or run `npm link` outside this research step if planner needs global CLI. [VERIFIED: environment command + AGENTS.md] |
| `.rolester/db/rolester.db` in current checkout | Live workspace state | no | - | Tests should create temp DBs with `importFromTracker()` or `data init`; product route should 409 when DB missing. [VERIFIED: `node src/cli/data.mjs status --json` + tests/data-route.test.mjs] |

**Missing dependencies with no fallback:**
- None for planning or route/test implementation. [VERIFIED: environment command]

**Missing dependencies with fallback:**
- `rolester` global command is not on PATH in this shell; repository-local `node src/cli/*.mjs` commands are available. [VERIFIED: environment command]
...
### Known Threat Patterns for Rolester Phase 6
...
- `.planning/phases/ROL-API-06-canonical-db-app-shell/06-CONTEXT.md` - locked Phase 6 decisions, discretion, deferred scope. [VERIFIED: 06-CONTEXT.md]
