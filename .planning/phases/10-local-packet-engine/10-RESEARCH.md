...
Copied verbatim from `.planning/phases/10-local-packet-engine/10-CONTEXT.md`. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
- EEO/default form data can exist for user-supervised form work, but Phase 10's locked decision excludes EEO, disability, veteran, and demographic self-identification from generated-answer automation. [CITED: AGENTS.md] [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
**Primary recommendation:** Build `src/core/packet/*` plus local `POST /api/packet/gate`, `POST /api/packet/generate`, `POST /api/packet/questions`, and `POST /api/packet/answers` routes; reuse existing DB verbs, bounded AI helpers, form-question extraction, document validators, and export helpers; make `/api/skill/run` a non-default handoff only. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
| EEO/demographic exclusion | API / Backend | Browser / Client display | The backend must filter excluded sections before generation; the UI can show that a section was detected and excluded. [CITED: config/form-questions.schema.json] [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md] |
...
|------------|-----------|----------|
| Local packet APIs | Full `evaluate-job`, `tailor-application`, `answer-question` skill runtime | Violates Phase 10 product boundary for ordinary packet work; retain only explicit watched/tool-heavy handoffs. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md] |
...
| Internal markdown source + PDF/DOCX outputs | User-facing markdown-only packet | Locked decision says markdown/source is internal and PDF is the normal user-facing format. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md] |
...
- **Defaulting to `/api/skill/run`:** Violates the Phase 10 local API boundary for ordinary packet work. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
- **Saving PDF only and discarding source markdown:** Locked decisions require internal markdown/source persistence as source of truth/build input. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
- **Generating DOCX for every packet:** DOCX is required only for board/upload workflows or explicit user choice. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
**Key insight:** Phase 10 is an orchestration and boundary migration phase. The risky work is not rendering documents; it is ensuring ordinary product actions no longer route through full skills, while preserving evidence, privacy, no-AI, and self-identification safeguards. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]

## Runtime State Inventory

> Included because Phase 10 migrates the packet product path from full skill-runtime defaults to local API ownership. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
**Nothing found in category:** No renamed string or globally registered service name is part of this phase. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
...
**Why it happens:** Phase 10 permits AI for judgment, but local AI configuration may be absent. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]  
...
**How to avoid:** Default to PDF; generate DOCX only when captured form/file requirements or explicit export selection require it. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]  
...
| A2 | A packet manifest can be stored as an internal artifact and referenced from the existing application `artifacts` JSON without a schema migration. [RESOLVED] | Architecture Patterns, Open Questions (RESOLVED) | If implementation tests prove the artifact JSON cannot hold the manifest path and summary metadata, execution must fail the focused packet tests instead of silently dropping manifest state. |
...
1. **RESOLVED: Store a packet manifest artifact, not a dedicated manifest table.**
   - Decision: Phase 10 plans use the existing application `artifacts` JSON plus DB verbs. `appRegisterPacketArtifacts()` writes `artifacts.packetManifest` as an internal manifest artifact path and stores summary siblings for source/export/question metadata on the same application row.
   - Rationale: Existing packet routes already read artifact maps, and the phase does not require a new relational query shape. [CITED: src/core/db/verbs/app.mjs] [CITED: src/cli/packet-route.mjs]
   - Required plan coverage: Plans 10-03 and 10-04 must persist captured-question metadata and source/export lineage into this manifest before packet UI integration in Plan 10-06.

2. **RESOLVED: Keep `POST /api/packet/gate` separate and make `POST /api/packet/generate` gate-aware.**
   - Decision: `POST /api/packet/gate` remains available for explicit review, while `generatePacket()` calls the gate step when no fresh passed or review-approved gate exists.
   - Rationale: This preserves a visible manual/review state for ambiguity and still delivers the locked happy path: passed gate plus apply intent generates the packet in one flow. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
   - Required plan coverage: Plan 10-04 must make gate status part of `packetManifestSchema` and block upload-ready packet state when gate, evidence, or prose generation has unresolved gaps.

3. **RESOLVED: DOCX requirement detection is advisory plus explicit selection; PDF remains default.**
   - Decision: Captured file-question accept metadata and Phase 07 document-format preferences can request DOCX, but generation defaults to PDF and only emits DOCX when captured metadata requires it or the user selects DOCX.
   - Rationale: Provider metadata is useful but not complete across all boards; the explicit option satisfies D-16 without weakening the PDF default from D-15. [CITED: config/form-questions.schema.json] [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md]
   - Required plan coverage: Plan 10-05 must test no-format, explicit-DOCX, and captured-upload-requirement paths.

4. **RESOLVED: Provider scope is Greenhouse, Ashby, and manual paste for Phase 10.**
   - Decision: Phase 10 uses existing Greenhouse/Ashby extraction and manual paste fallback. Lever, Workday, and other provider-specific extraction are outside Phase 10 unless already normalized by existing local helpers.
   - Rationale: D-14 requires supported provider extraction with manual fallback, and current support is already Greenhouse/Ashby/manual. [CITED: src/cli/questions.mjs] [CITED: src/core/apply/form-questions.mjs]
   - Required plan coverage: Plan 10-03 must return a durable manual-needed state for unsupported providers instead of launching browser automation or full skill runtime.
...
| Unauthorized external submission | Elevation of Privilege | Packet engine must prepare files only; no submit/click browser automation. [CITED: .planning/phases/10-local-packet-engine/10-CONTEXT.md] |

## Sources

### Primary (HIGH confidence)

- `.planning/phases/10-local-packet-engine/10-CONTEXT.md` - locked phase decisions, discretion, deferred scope.
