// Next.js App Router client instrumentation entry point (auto-loaded, no
// provider component needed). Same wiring as apps/website/instrumentation-client.ts
// (itself copied from CodesWhat/codeswhat.com's house PostHog standard) — kept
// in sync by hand since apps/docs and apps/website are separate builds.
...
// Next.js calls this automatically on every App Router client-side route
// transition (static export, so all navigation after the first load is
// client-side).
