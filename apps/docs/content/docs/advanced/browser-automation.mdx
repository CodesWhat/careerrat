...
compatible extension session is selected otherwise. Playwright keeps persistent profiles
at `~/.careerrat/board-profiles/<platform>`. If the selected provider cannot execute a
workflow, CareerRat shows a supervised/manual handoff instead of claiming it ran.
