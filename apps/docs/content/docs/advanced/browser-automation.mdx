...
Orca workspace; the compatible extension session is selected otherwise. Playwright
persistent profiles remain an explicit fallback at
`~/.careerrat/board-profiles/<platform>`. If the selected provider cannot execute a
