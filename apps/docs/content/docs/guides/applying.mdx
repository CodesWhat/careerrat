...
4. Shows the generated résumé, cover letter, answers, unresolved facts, and safe site link.
5. Uses the session browser to fill safe, confirmed fields and attach generated documents when
...
6. Keeps voluntary demographic and self-identification questions blank by default. The local-only
...
7. **Halts before final submission** so you can review the form and press submit.
8. Halts on CAPTCHAs, two-factor authentication, and unsupported auth prompts.
9. Records the application through the canonical data layer only after submission is
...
the session browser. This requires the `authenticated_apply_preparation`
capability to be enabled in `careerrat automation status`. It fills and advances
the modal but never presses the final submit control.
