...
5. Uses an authenticated session browser to fill the form when that executor is available.
   Otherwise, it gives you a concrete supervised handoff instead of claiming automation ran.
6. **Halts before final submission** so you can review the form and press submit.
7. Halts on CAPTCHAs and unsupported auth prompts.
8. Records the application through the canonical data layer only after submission is
...
packet, reports filled, uploaded, and unresolved fields in Ask, and never records
Applied until a confirmation page and screenshot are captured or the user reports completion.
