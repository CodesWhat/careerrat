...
CareerRat ships 21 skills under `.agents/skills/`. Each is a `SKILL.md` file — a
...
| `ingest-mail` | Read job-search email updates from Apple Mail on macOS or, with separate mail_access consent, Gmail/Outlook webmail through the session browser; fold recruiter replies and status changes into CareerRat tracker communication state. Opt-in, local-only, read-only. |
