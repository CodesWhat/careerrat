...
2. **Verify** with `careerrat tracker --verify`.
3. **Re-render** with `careerrat tracker` so the dashboard hot-reloads.
4. **Log one Activity Pulse event** via `careerrat activity append`.
